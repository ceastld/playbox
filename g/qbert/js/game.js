...
var STEP = 1 / 60;
var TAU = Math.PI * 2;
var BEST_KEY = 'playbox-qbert-best';
var MUTE_KEY = 'playbox-qbert-mute';
...
var KEY_DIR = {
  KeyA: UL, ArrowLeft: UL,
  KeyW: UR, ArrowUp: UR,
  KeyD: DR, ArrowRight: DR,
...
var PALS = [
  {
    L: [92, 44, 18],
    R: [52, 24, 12],
    tops: [[86, 58, 118], [255, 154, 50], [255, 220, 96]]
  },
  {
    L: [70, 28, 62],
    R: [40, 14, 38],
    tops: [[58, 46, 122], [255, 72, 168], [0, 240, 220]]
  },
  {
    L: [24, 52, 78],
    R: [12, 30, 50],
    tops: [[42, 72, 128], [255, 154, 50], [72, 255, 176]]
  },
  {
    L: [62, 22, 86],
    R: [34, 12, 52],
    tops: [[74, 38, 128], [255, 84, 196], [255, 228, 88]]
  }
];
...
function spawnInterval(round, chase) {
  var t = chase ? 1.28 : 2.7;
  t /= 1 + Math.max(0, round - 1) * 0.12;
  return t < (chase ? 0.64 : 1.35) ? (chase ? 0.64 : 1.35) : t;
}
function coilySpawnDelay(chase) {
  return chase ? 0.62 : 1.25;
}
...
function selfCheck() {
  var n, d, f, tiles, dest, dist, spots;
  ...
  if (spots.length !== 2) throw new Error('2 discs classic');
  spots = discSpots(1, true);
  if (spots.length !== 1) throw new Error('1 disc chase');

  if (KEY_DIR.KeyA !== UL || KEY_DIR.KeyW !== UR || KEY_DIR.KeyD !== DR || KEY_DIR.KeyS !== DL) {
    throw new Error('WASD map');
  }
  if (KEY_DIR.ArrowLeft !== UL || KEY_DIR.ArrowUp !== UR || KEY_DIR.ArrowRight !== DR || KEY_DIR.ArrowDown !== DL) {
    throw new Error('arrows iso');
  }
  if (KEY_DIR.KeyQ != null) throw new Error('Q not a hop');
}
...
var audio = {
  ...
  hop: function (combo) {
    ...
    this.beep(318 * p, 0.055, 'square', 0.055, 580 * p);
  ...
  flip: function (combo) {
    ...
    this.beep(540 * p, 0.07, 'square', 0.06, 880 * p);
    this.beep(810 * p, 0.1, 'triangle', 0.04, 1240 * p);
  },
  disc: function () {
    this.ensure();
  ...
  sting: function () {
    ...
    this.beep(420, 0.08, 'square', 0.08, 90);
    this.beep(90, 0.22, 'sawtooth', 0.07, 40);
  },
  die: function () {
    this.ensure();
...
function hudPlay() {
  ...
  renderPips();
  syncTiles();
  modeLabel.textContent = G.chase ? '追蛇' : '翻色';
...
function startRun(kind) {
  ...
  audio.start();
  hudPlay();
  toast(G.chase ? '追蛇' : '开跳', false, !G.chase);
...
function showTitle() {
  ...
  panelEl.className = 'panel';
  ovTitle.textContent = '方跳';
  ovLead.textContent = '跳上相邻方块翻色。红球往下蹦，紫蛋孵出科伊来追。边上飞碟能送回顶。跳下去会掉命。';
  ovOps.textContent = '点相邻块 · WASD / 方向键等距跳 · R 重开 · M 静音';
  ovStart.classList.remove('gone');
  ovEnd.classList.add('gone');
  hintEl.textContent = 'WASD 对角跳翻色 · 科伊会追 · 飞碟回顶 · 跳下塔掉命';
  G.round = 1;
  G.chase = false;
  G.tiles = makeTiles();
  G.flash = makeFlash();
...
function kill(why) {
  ...
  dieShake();
  shake(why === 'coily' ? 11 : 9);
  rgb = why === 'coily' ? [196, 92, 255] : why === 'ball' ? [255, 61, 184] : [255, 154, 50];
...
function landPlayer() {
  ...
  if (res.flipped) {
    ...
    hitStop(0.042);
    spark(v.x, v.y, col, 7);
    burst(v.x, v.y, 14, col, 78, 0.34, 16);
...
function drawPlayerAt(x, y, e) {
  ...
  ctx.beginPath();
  ctx.ellipse(0, -1 * s - hop * sc(2), 8.8 * s, 8.4 * s, 0, 0, TAU);
  ctx.fill();

  ctx.fillStyle = '#ffd08a';
  ctx.beginPath();
  ctx.ellipse(-1.6 * s, -3.6 * s, 4.8 * s, 3.8 * s, -0.2, 0, TAU);
  ctx.fill();

  ctx.fillStyle = '#ffd8a0';
  ctx.beginPath();
  ctx.ellipse(7.2 * s, 1.1 * s, 6.2 * s, 3.5 * s, 0.15, 0, TAU);
  ctx.fill();
  ctx.fillStyle = '#ff9a32';
  ctx.beginPath();
  ctx.ellipse(11 * s, 1.3 * s, 2.05 * s, 2.3 * s, 0.2, 0, TAU);
  ctx.fill();

  ctx.fillStyle = '#00f0ff';
  ctx.fillRect(-6.6 * s, -2.6 * s, 9.4 * s, 1.5 * s);

  ctx.fillStyle = '#05030c';
  ctx.beginPath();
  ctx.ellipse(2.2 * s, -4.4 * s, 1.35 * s, 1.7 * s, 0, 0, TAU);
  ctx.ellipse(5.5 * s, -4.55 * s, 1.2 * s, 1.55 * s, 0, 0, TAU);
  ctx.fill();
  ctx.fillStyle = '#fff';
  ctx.beginPath();
  ctx.arc(2.5 * s, -4.9 * s, 0.45 * s, 0, TAU);
  ctx.arc(5.75 * s, -5 * s, 0.4 * s, 0, TAU);
  ctx.fill();
...
function drawBg() {
  ...
  g.addColorStop(0, 'rgba(255,154,50,0.16)');
  g.addColorStop(1, 'rgba(255,154,50,0)');
  ctx.fillStyle = g;
  ctx.fillRect(0, 0, cssW, cssH);
