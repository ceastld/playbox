(function () {
  ...
  function syncHud() {
    ...
    if (s.limit) stepLabel.textContent = S.steps + " / " + s.limit + " 步";
    else stepLabel.textContent = S.steps + " 步";
  ...
  function tryMove(dirKey) {
    ...
    S.anim = {
      ...
      youOk: a.ok,
      mirOk: b.ok,
      overLimit: !!(s.limit && S.steps > s.limit),
    };
    S.you = { x: a.x, y: a.y };
    S.mir = { x: b.x, y: b.y };
  ...
  function finishAnim() {
    ...
    if (a.overLimit) {
      beginFall({ youOk: false, mirOk: false, outOfSteps: true });
      return;
    }
  ...
  function showDeath() {
    const f = S.fall;
    let msg = "双双踏入虚空。";
    if (f.outOfSteps) msg = "步数用尽。镜面不同情犹豫。";
    else if (f.youOk && !f.mirOk) msg = "你落在实砖上，镜中的你却踏空了。";
