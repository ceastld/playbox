...
(function () {
  ...
  const HIT_R = 6.0;
  const SHOT_V = 640;
  const BEST_KEY = 'playbox-shikigami-best';
  const MUTE_KEY = 'playbox-shikigami-mute';
  const OPS = '方向 / WASD 飞 · 空格射击 · Shift / Z 锁定敌机 · R 重开 · M 静音';
  const REDUCE = typeof window !== 'undefined' && window.matchMedia
    ? window.matchMedia('(prefers-reduced-motion: reduce)').matches
    : false;

  const MAG = [255, 61, 154];
  const VIO = [225, 77, 255];
  const CYN = [94, 232, 255];
  const GOLD = [255, 227, 107];
  const PAPER = [255, 244, 220];
  const VERM = [255, 77, 74];
  const HOT = [255, 138, 212];
  const FOX = [255, 154, 90];
  const DEEP = [28, 8, 20];
  const SKIN = [255, 214, 224];
  const HAIR = [232, 80, 160];
  ...
  const STAGES = [
    {
      id: 0, name: '朱门', boss: '门守', bossKind: 'gate', bossHp: 94, hue: 312,
      waves: [
        { t: 0.4, kind: 'cranes', n: 5, y: 0.3 },
        { t: 1.5, kind: 'lanterns', n: 3 },
        { t: 2.6, kind: 'cranes', n: 6, y: 0.62 },
        { t: 3.8, kind: 'masks', n: 2 },
        { t: 5.0, kind: 'ofudas', n: 3 },
        { t: 6.2, kind: 'dive', n: 4 },
        { t: 7.2, kind: 'cart' },
        { t: 8.0, kind: 'komainu', n: 2 },
        { t: 9.2, kind: 'foxfires', n: 4 },
        { t: 10.4, kind: 'lanterns', n: 4 },
        { t: 11.6, kind: 'cranes', n: 7, y: 0.46 },
        { t: 12.8, kind: 'tengu', n: 2 },
        { t: 14.0, kind: 'masks', n: 3 },
        { t: 15.2, kind: 'foxfires', n: 5, rain: true },
        { t: 16.4, kind: 'boss' }
      ]
    },
    {
      id: 1, name: '纸廊', boss: '符姬', bossKind: 'ofuda', bossHp: 120, hue: 328,
      waves: [
        { t: 0.35, kind: 'ofudas', n: 4 },
        { t: 1.4, kind: 'lanterns', n: 4 },
        { t: 2.6, kind: 'tengu', n: 2 },
        { t: 3.8, kind: 'foxfires', n: 5 },
        { t: 5.0, kind: 'masks', n: 3 },
        { t: 6.2, kind: 'dive', n: 5 },
        { t: 7.2, kind: 'cart' },
        { t: 8.0, kind: 'komainu', n: 3 },
        { t: 9.2, kind: 'ofudas', n: 5 },
        { t: 10.4, kind: 'lanterns', n: 4, rain: true },
        { t: 11.6, kind: 'tengu', n: 3 },
        { t: 12.8, kind: 'cranes', n: 6, y: 0.34 },
        { t: 14.0, kind: 'masks', n: 3 },
        { t: 15.2, kind: 'foxfires', n: 6 },
        { t: 16.6, kind: 'boss' }
      ]
    },
    {
      id: 2, name: '式城', boss: '大式', bossKind: 'kami', bossHp: 158, hue: 300,
      waves: [
        { t: 0.3, kind: 'cranes', n: 6, y: 0.26 },
        { t: 1.2, kind: 'tengu', n: 2 },
        { t: 2.4, kind: 'masks', n: 3 },
        { t: 3.5, kind: 'ofudas', n: 4 },
        { t: 4.6, kind: 'foxfires', n: 6 },
        { t: 5.8, kind: 'dive', n: 6 },
        { t: 6.8, kind: 'cart' },
        { t: 7.6, kind: 'komainu', n: 3 },
        { t: 8.8, kind: 'lanterns', n: 5 },
        { t: 10.0, kind: 'tengu', n: 3 },
        { t: 11.2, kind: 'masks', n: 3 },
        { t: 12.4, kind: 'ofudas', n: 5, rain: true },
        { t: 13.6, kind: 'foxfires', n: 6 },
        { t: 14.8, kind: 'cranes', n: 7, y: 0.52 },
        { t: 16.0, kind: 'dive', n: 5 },
        { t: 17.0, kind: 'boss' }
      ]
    }
  ];
  ...
  const tensWrap = document.getElementById('tens-wrap');
  const tensBar = document.getElementById('tens-bar');
  ...
  function plySpd() {
    return (isSea() ? 322 : 280) + G.wpnLv * 8;
  }
  function scrollSpd() {
    if (G.bossIn) return isSea() ? 32 : 24;
    const base = isSea() ? 126 : 90;
    const rush = G.combo >= 8 ? 14 : G.combo >= 4 ? 7 : 0;
    return base + rush + G.stage * 6 + G.tension * 10;
  }
  function hpMul() {
    return isSea() ? 1.28 : 1;
  }
  ...
  function wpnText() {
    return G.wpnLv <= 0 ? '符' : G.wpnLv === 1 ? '双符' : G.wpnLv === 2 ? '扇符' : '满开';
  }
  function kindName() {
    return isSea() ? '式海' : '夜城';
  }
  function tensText() {
    return 1 + Math.floor(G.tension * 7);
  }
  ...
  const audio = {
    ...
    shoot() {
      this.ensure();
      this.beep(860 + G.wpnLv * 48, 0.04, 'square', 0.025, 1720);
    },
    lockOn() {
      this.ensure();
      this.beep(680, 0.07, 'triangle', 0.04, 1540);
      this.beep(1240, 0.09, 'sine', 0.028, 1880);
    },
    lockShot() {
      this.ensure();
      this.beep(1180 + G.lockN * 36, 0.034, 'sine', 0.022, 2040);
    },
    graze() {
      this.ensure();
      this.beep(1480 + G.tension * 420, 0.045, 'sine', 0.03, 2100);
    },
    hit(combo) {
      this.ensure();
      const lift = 1 + Math.min(0.5, combo * 0.04);
      this.noise(0.028, 0.026, 1600);
      this.beep(660 * lift, 0.052, 'square', 0.036, 1180 * lift);
    },
    boom(big) {
      this.ensure();
    ...
    lockKill() {
      ...
      this.beep(1320, 0.14, 'sine', 0.034, 2100);
      this.noise(0.08, 0.03, 1100);
    },
    pow() {
      this.ensure();
  ...
  function syncHud() {
    ...
    if (tensBar) tensBar.style.transform = 'scaleX(' + clamp(G.tension, 0, 1) + ')';
    if (tensWrap) tensWrap.classList.toggle('full', G.tension >= 0.9);
    if (hintEl) {
      hintEl.classList.toggle('hot', G.bossIn || G.tension >= 0.7);
      hintEl.classList.toggle('warn', G.lives <= 1 && G.mode === 'play');
      if (G.mode === 'title') hintEl.textContent = '空格连射符纸 · Shift 锁敌打式矢 · 贴身叠临 · 撞上掉命';
      else if (G.bossIn) hintEl.textContent = st.boss + ' · 锁敌贴身叠临 · 撞上掉命';
      else hintEl.textContent = st.name + ' · 空格射击 · Shift 锁定敌机';
    }
    if (comboEl) {
      if (G.mode === 'play' && (G.combo >= 2 || G.lockChain >= 2 || G.tension >= 0.35)) {
        comboEl.hidden = false;
        const bits = [];
        if (G.lockChain >= 2) bits.push('锁 ×' + G.lockChain);
        else if (G.combo >= 2) bits.push('连击 ×' + G.combo);
        if (G.tension >= 0.35) bits.push('临 ×' + tensText());
        comboEl.textContent = bits.join('  ·  ') || '连击';
  ...
  function showOverlay(kind, title, lead) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'SHIK';
    ovTitle.textContent = title;
    ovLead.textContent = lead;
    ovOps.textContent = OPS;
    if (ovStart) ovStart.classList.toggle('gone', kind !== 'title');
    if (ovEnd) ovEnd.classList.toggle('gone', kind === 'title');
    if (btnOvRetry) btnOvRetry.textContent = '再飞';
    if (btnOvModes) {
      if (kind === 'win' && !isSea()) btnOvModes.textContent = '式海';
  ...
  function seedDecor() {
    ...
    for (let i = 0; i < 36; i++) {
      motes.push({
        ...
        v: rand(22, 64),
        a: rand(0.16, 0.5),
        rgb: i % 3 === 0 ? GOLD : i % 3 === 1 ? MAG : PAPER
      });
    }
    for (let i = 0; i < 6; i++) {
      torii.push({
  ...
  function makeEShot(x, y, vx, vy, r, rgb) {
    return { x: x, y: y, vx: vx, vy: vy, r: r || 3.5, life: 2.7, rgb: rgb || MAG, dead: false };
  }
  ...
  function spawnPow(x, y, kind) {
    G.pows.push({
      x: x, y: y, vx: -36, vy: rand(-34, 34), kind: kind || 'power',
      t: 0, dead: false
    });
  }

  function makeCrane(x, y, amp) {
    return {
      type: 'crane', x: x, y: y, baseY: y, w: 22, h: 12, hp: 1, score: SCORE.crane,
      vx: isSea() ? -162 : -128, amp: amp || 22, bob: rand(0, TAU),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }

  function makeLantern(x, y) {
    return {
      type: 'lantern', x: x, y: y, baseY: y, w: 16, h: 20, hp: 2, score: SCORE.lantern,
      vx: isSea() ? -92 : -72, bob: rand(0, TAU), amp: 30, cd: rand(0.5, 1.2),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }

  function makeMask(x, y) {
    return {
      type: 'mask', x: x, y: y, w: 24, h: 22, hp: 3, score: SCORE.mask,
      vx: isSea() ? -74 : -58, vy: 0, cd: rand(0.6, 1.3), bob: rand(0, TAU),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }

  function makeOfuda(x, y) {
    return {
      type: 'ofuda', x: x, y: y, w: 14, h: 22, hp: 2, score: SCORE.ofuda,
      vx: isSea() ? -110 : -88, vy: rand(-40, 40), spin: rand(0, TAU),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }

  function makeFoxfire(x, y) {
    return {
      type: 'foxfire', x: x, y: y, baseY: y, w: 14, h: 14, hp: 1, score: SCORE.foxfire,
      vx: isSea() ? -122 : -96, bob: rand(0, TAU), amp: 40, vy: rand(-60, 60),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }

  function makeKomainu(x, y) {
    return {
      type: 'komainu', x: x, y: y, w: 26, h: 22, hp: 4, score: SCORE.komainu,
      vx: isSea() ? -86 : -68, ground: true, cd: rand(0.4, 1.0),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }

  function makeTengu(x, y) {
    return {
      type: 'tengu', x: x, y: y, w: 22, h: 26, hp: 4, score: SCORE.tengu,
      vx: isSea() ? -60 : -48, cd: rand(0.7, 1.4),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }
  ...
  function makeCart(x, y) {
    return {
      type: 'cart', x: x, y: y, w: 32, h: 20, hp: 5, score: SCORE.cart,
      vx: isSea() ? -64 : -50, drop: true, bob: rand(0, TAU),
      dead: false, hitT: 0, locked: false, lockI: 0
    };
  }
  ...
  function makeBoss(kind, name, hp) {
    return {
      ...
      w: kind === 'kami' ? 112 : kind === 'ofuda' ? 96 : 100,
      h: kind === 'kami' ? 86 : kind === 'ofuda' ? 78 : 84,
  ...
  function spawnWave(w) {
    ...
    const y = (w.y || 0.5) * VH;
    if (w.kind === 'cranes') {
      for (let i = 0; i < n; i++) G.ents.push(makeCrane(VW + 24 + i * 28, y + rand(-16, 16), 18 + i * 3));
    } else if (w.kind === 'lanterns') {
      for (let i = 0; i < n; i++) G.ents.push(makeLantern(VW + 22 + i * 34, 70 + (i % 4) * 70));
    } else if (w.kind === 'masks') {
      for (let i = 0; i < n; i++) G.ents.push(makeMask(VW + 30 + i * 44, 80 + i * 70));
    } else if (w.kind === 'ofudas') {
      for (let i = 0; i < n; i++) G.ents.push(makeOfuda(VW + 18 + i * 26, rand(50, VH - 50)));
    } else if (w.kind === 'foxfires') {
      for (let i = 0; i < n; i++) G.ents.push(makeFoxfire(VW + 16 + i * 24, 60 + (i % 5) * 60));
    } else if (w.kind === 'komainu') {
      for (let i = 0; i < n; i++) G.ents.push(makeKomainu(VW + 40 + i * 70, VH - 34));
    } else if (w.kind === 'tengu') {
      for (let i = 0; i < n; i++) G.ents.push(makeTengu(VW + 36 + i * 50, 90 + i * 80));
    } else if (w.kind === 'dive') {
      for (let i = 0; i < n; i++) {
        const b = makeCrane(VW + 16 + i * 22, 36 + (i % 2) * (VH - 80), 8);
        b.dive = true;
        G.ents.push(b);
      }
    } else if (w.kind === 'cart') {
  ...
  function explodeEnt(e, big) {
    const rgb = e.locked
      ? GOLD
      : e.type === 'boss' ? GOLD
        : e.type === 'tengu' || e.type === 'komainu' ? VIO
          : e.type === 'mask' ? VERM
            : e.type === 'foxfire' ? FOX : MAG;
    emit(big ? 30 : e.locked ? 22 : 12, {
      x: e.x, y: e.y, j: big ? 28 : 10,
      vx0: -170, vx1: 170, vy0: -170, vy1: 170,
      r0: 1.6, r1: big ? 6.2 : 3.4, life: big ? 0.56 : 0.32, rgb: rgb, g: 70
    });
    popSpark(e.x, e.y, rgb, big ? 36 : e.locked ? 24 : 16);
    if (e.locked) audio.lockKill();
    else audio.boom(big);
  }
  ...
  function killEnt(e) {
    ...
    const tens = 1 + G.tension * 2.5;
    let pts = (e.score || 50) * G.mult * tens;
    if (locked) {
      bumpLockChain();
      pts *= (1 + G.lockChain);
    }
    ...
    bumpCombo();
    const label = locked
      ? (G.tension >= 0.55 ? '临锁 +' + pts : '锁 +' + pts)
      : (G.tension >= 0.55 ? '临 +' + pts : '+' + pts);
    floatText(e.x, e.y - 10, label, locked || G.tension >= 0.55 ? GOLD : WHT, locked || e.type === 'boss' || pts >= 280);
    explodeEnt(e, e.type === 'boss' || e.type === 'cart' || locked);
    if (locked) {
      if (G.lockChain % 3 === 0) spawnPow(e.x, e.y, 'power');
      hitStop(clamp(0.046 + G.lockChain * 0.004 + G.tension * 0.02, 0.046, 0.08));
      kick(3.6 + G.tension * 1.4);
    } else {
      if (e.type === 'cart' && e.drop) spawnPow(e.x, e.y, nextDrop());
      if (e.type === 'boss') onBossDown(e);
      hitStop(e.type === 'boss' ? 0.08 : clamp(0.032 + G.combo * 0.0026 + G.tension * 0.018, 0.032, 0.072));
  ...
  function hurtEnt(e, dmg, hx, hy, fromLock) {
    ...
    emit(4, {
      ...
      r0: 1, r1: 2.4, life: 0.18, rgb: fromLock ? MAG : GOLD, g: 0
    });
  ...
  function onBossDown(e) {
    G.bossIn = false;
    addScore(Math.round(1500 * (G.stage + 1) * G.mult * (1 + G.tension)));
  ...
  function fire() {
    ...
    if (!REDUCE) {
      emit(2, {
        ...
        r0: 1, r1: 2, life: 0.12, rgb: PAPER, g: 0
      });
    }
  }
  ...
  function acquireLocks() {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      ...
      if (e.x < G.px - 8 || e.x > VW + 10) continue;
    ...
    best.lockI = n;
    G.lockN = n;
    popSpark(best.x, best.y, MAG, 18);
    floatText(best.x, best.y - 14, '锁', MAG, false);
    audio.lockOn();
    hitStop(0.022);
    kick(1.6);
    if (stageEl && !REDUCE) {
  ...
  function fireLock() {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      const e = G.ents[i];
      if (e.locked && !e.dead) locked.push(e);
    }
    if (!locked.length) return;
    if (G.shots.length > 110) return;
    G.lockCd = isSea() ? 0.07 : 0.082;
    G.muzzle = Math.max(G.muzzle, 0.04);
    audio.lockShot();
    const cap = Math.min(locked.length, maxLocks());
    for (let i = 0; i < cap; i++) {
      const e = locked[i];
      const ang = Math.atan2(e.y - G.py, e.x - G.px);
      const spread = (i - (cap - 1) * 0.5) * 0.18;
      G.shots.push(makeShot(
        G.px + 14, G.py,
        Math.cos(ang + spread) * 360, Math.sin(ang + spread) * 360,
        e.type === 'boss' ? 0.38 : 1,
        { r: 3.2, life: 1.4, lock: true, target: e }
      ));
    }
  }
  ...
  function collectPow(p) {
    ...
    } else addScore(Math.round(500 * G.mult * (1 + G.tension)));
    audio.pow();
  ...
  function hurtPlayer(why) {
    ...
    G.lockChain = 0;
    G.lockChainT = 0;
    for (let i = 0; i < G.ents.length; i++) G.ents[i].locked = false;
    emit(34, {
      x: G.px, y: G.py, j: 18,
  ...
  function goLose() {
    G.mode = 'lose';
    audio.lose();
    showOverlay('lose', '式散了', '纸狐散了，夜城把你留下了。分数 ' + G.score + '。');
    syncHud();
  }
  ...
  function goWin() {
    ...
    addScore(8000);
    audio.win();
    showOverlay('win', isSea() ? '式海退了' : '城破了', '三夜打穿。大式倒下了。分数 ' + G.score + '。');
    syncHud();
  }

  function updateTension(dt) {
    if (G.deadT > 0) {
      G.tension = Math.max(0, G.tension - dt * 0.8);
      G.tensMul = 1 + G.tension * 2.5;
      return;
    }
    let nearest = 999;
    for (let i = 0; i < G.ents.length; i++) {
      const e = G.ents[i];
      if (e.dead) continue;
      const pad = e.type === 'boss' ? 38 : Math.min(e.w || 16, e.h || 12) * 0.42;
      const d = hypot(e.x - G.px, e.y - G.py) - pad;
      if (d < nearest) nearest = d;
    }
    for (let i = 0; i < G.eShots.length; i++) {
      const s = G.eShots[i];
      const d = hypot(s.x - G.px, s.y - G.py) - s.r;
      if (d < nearest) nearest = d;
    }
    if (nearest < 22) {
      G.tension = Math.min(1, G.tension + dt * 0.78);
      if (G.grazeCd <= 0) {
        G.grazeCd = 0.11;
        audio.graze();
        const n = Math.round(SCORE.graze * (1 + G.tension * 3) * G.mult);
        addScore(n);
        emit(3, {
          x: G.px + rand(-8, 8), y: G.py + rand(-8, 8), j: 3,
          vx0: -40, vx1: 40, vy0: -50, vy1: 20,
          r0: 1, r1: 2.2, life: 0.18, rgb: GOLD, g: 0
        });
        if (G.tension > 0.55) floatText(G.px + 16, G.py - 18, '临 ×' + tensText(), GOLD, G.tension > 0.85);
      }
    } else if (nearest < 48) {
      G.tension = Math.min(1, G.tension + dt * 0.34);
    } else if (nearest < 88) {
      G.tension = Math.min(1, G.tension + dt * 0.14);
    } else {
      G.tension = Math.max(0, G.tension - dt * 0.26);
    }
    G.tensMul = 1 + G.tension * 2.5;
  }
  ...
  function updateFx(dt) {
    ...
    if (G.grazeCd > 0) G.grazeCd = Math.max(0, G.grazeCd - dt);
    ...
    if (G.lockChainT > 0) {
      G.lockChainT -= dt;
      if (G.lockChainT <= 0) G.lockChain = 0;
    }
  ...
  function updateBoss(e, dt) {
    ...
    if (e.kind === 'gate') {
      e.y = VH * 0.5 + Math.sin(e.t * 1.35) * 86;
      if (e.phase === 2) e.x = 640 + Math.sin(e.t * 1.05) * 60;
      if (e.cd <= 0) {
        fanShot(e.x - 30, e.y, e.phase === 2 ? 7 : 5, isSea() ? 170 : 140, 0.22, VERM);
        if (e.phase === 2) G.eShots.push(aimShot(e.x - 20, e.y, G.px, G.py, isSea() ? 190 : 150, 4, GOLD));
        e.cd = (e.phase === 2 ? 0.74 : 1.06) * rain;
      }
    } else if (e.kind === 'ofuda') {
      e.y = lerp(e.y, VH * (0.3 + 0.2 * (1 + Math.sin(e.t * 0.72))), 0.04);
      e.x = 630 + Math.sin(e.t * 0.58) * 26;
      if (e.cd <= 0) {
        if (((e.t * 2) | 0) % 2 === 0) {
          ringShot(e.x - 10, e.y, e.phase === 2 ? 14 : 10, isSea() ? 130 : 108, PAPER, e.t);
        } else {
          for (let k = 0; k < (e.phase === 2 ? 5 : 3); k++) {
            G.eShots.push(aimShot(e.x - 24, e.y + 8, G.px, G.py, 150 + k * 18, 3.6, MAG));
          }
        }
      ...
      if (e.cd <= 0) {
        ...
        for (let i = 0; i < n; i++) {
          ...
          G.eShots.push(makeEShot(e.x - 36, e.y, Math.cos(a) * -154, Math.sin(a) * 154, 3.8, VIO));
        }
        if (e.phase === 2) {
          ringShot(e.x, e.y, 12, isSea() ? 120 : 96, FOX, e.t * 0.7);
          G.eShots.push(aimShot(e.x - 20, e.y, G.px, G.py, isSea() ? 200 : 160, 5, GOLD));
        }
  ...
  function updateEnts(dt) {
    for (let i = G.ents.length - 1; i >= 0; i--) {
      ...
      e.x += (e.vx || 0) * dt;
      if (e.type === 'crane') {
        if (e.dive) e.y += clamp(G.py - e.y, -90, 90) * dt * 1.4;
        else {
          e.bob += dt * 3.2;
        ...
        e.cd -= dt;
        if (e.cd <= 0) {
          G.eShots.push(makeEShot(e.x, e.y, -48, 64, 3.2, GOLD));
          e.cd = isSea() ? 1.05 : 1.38;
        }
      } else if (e.type === 'mask') {
        e.bob += dt * 1.6;
        e.y += Math.sin(e.bob) * 18 * dt;
        e.y += (G.py - e.y) * dt * 0.08;
        e.cd -= dt;
        if (e.cd <= 0) {
          G.eShots.push(aimShot(e.x - 8, e.y, G.px, G.py, isSea() ? 154 : 122, 3.5, VERM));
          e.cd = isSea() ? 1.18 : 1.5;
        }
      } else if (e.type === 'ofuda') {
        e.y += e.vy * dt;
        e.spin += dt * 6;
        if (e.y < 18 || e.y > VH - 18) e.vy *= -1;
      } else if (e.type === 'foxfire') {
        e.bob += dt * 4.2;
        e.y = e.baseY + Math.sin(e.bob) * e.amp;
        e.x += Math.cos(e.bob) * 22 * dt;
      } else if (e.type === 'komainu') {
        e.cd -= dt;
        if (e.cd <= 0) {
          G.eShots.push(aimShot(e.x, e.y - 12, G.px, G.py, isSea() ? 156 : 124, 3.6, GOLD));
          e.cd = isSea() ? 1.08 : 1.42;
        }
      } else if (e.type === 'tengu') {
        e.cd -= dt;
        if (e.cd <= 0) {
          fanShot(e.x - 8, e.y, 3, isSea() ? 140 : 114, 0.26, VIO);
      ...
      if (e.locked) {
        e.vx = Math.max(e.vx, isSea() ? -46 : -34);
        e.y += Math.sin(G.t * 5 + e.lockI) * 10 * dt;
      }
  ...
  function updateShots(dt) {
    for (let i = G.shots.length - 1; i >= 0; i--) {
      ...
      if (s.lock && s.target && !s.target.dead) {
        ...
        const spd = 580;
        s.vx = lerp(s.vx, dx / d * spd, 0.24);
        s.vy = lerp(s.vy, dy / d * spd, 0.24);
      }
      s.x += s.vx * dt;
      s.y += s.vy * dt;
      s.life -= dt;
      ...
      for (let j = 0; j < G.ents.length; j++) {
        const e = G.ents[j];
        if (e.dead) continue;
        if (s.lock && s.target && e !== s.target && e.type !== 'boss') continue;
        if (bodyHit(e, s.x, s.y, s.r)) {
          hurtEnt(e, s.dmg, s.x, s.y, !!s.lock);
          s.dead = true;
          hit = true;
          break;
        }
    ...
    for (let i = G.eShots.length - 1; i >= 0; i--) {
      ...
      if (G.deadT <= 0 && hypot(s.x - G.px, s.y - G.py) < HIT_R + s.r * 0.45) {
        s.dead = true;
        G.eShots.splice(i, 1);
        hurtPlayer('shot');
      }
  ...
  function updatePows(dt) {
    for (let i = G.pows.length - 1; i >= 0; i--) {
      ...
      if (G.deadT <= 0) {
        ...
        const d = hypot(dx, dy) || 1;
        if (d < 90) {
          p.vx = lerp(p.vx, dx / d * 260, 0.12);
          p.vy = lerp(p.vy, dy / d * 260, 0.12);
        }
      }
      p.x += p.vx * dt;
      p.y += p.vy * dt;
  ...
  function update(dt) {
    ...
    if (tensWrap) tensWrap.classList.toggle('full', G.tension >= 0.9);
    if (comboEl) {
      if (G.combo >= 2 || G.lockChain >= 2 || G.tension >= 0.35) {
        comboEl.hidden = false;
        const bits = [];
        if (G.lockChain >= 2) bits.push('锁 ×' + G.lockChain);
        else if (G.combo >= 2) bits.push('连击 ×' + G.combo);
  ...
  function drawCastle() {
    ...
    const st = stageOf(G.stage);
    const hue = st.hue;
    const top = hueRgb(hue, 0.58, 0.08);
    const bot = hueRgb(hue + 16, 0.42, 0.04);
    ...
    const moonY = sy(G.stage === 2 ? 76 : 62);
    const moonR = (G.stage === 2 ? 56 : 38) * scale;
    ...
    if (G.stage !== 1) {
      for (let i = 0; i < torii.length; i++) {
        ...
        const w = t.w * scale;
        const h = t.h * scale;
        c.fillStyle = rgba(VERM, 0.72);
        c.fillRect(x - w * 0.5, y - h, w * 0.12, h);
        c.fillRect(x + w * 0.38, y - h, w * 0.12, h);
        c.fillRect(x - w * 0.58, y - h - 6 * scale, w * 1.16, 8 * scale);
        c.fillRect(x - w * 0.48, y - h + 12 * scale, w * 0.96, 5 * scale);
      }
    }
    if (G.stage >= 1) {
      for (let i = 0; i < pagodas.length; i++) {
        const p = pagodas[i];
        c.fillStyle = rgba(hueRgb(st.hue, 0.28, 0.12), 0.72);
        const px = sx(p.x);
        const py = sy(VH);
        const pw = p.w * scale;
        const ph = p.h * scale;
        c.fillRect(px, py - ph, pw, ph);
        c.fillStyle = rgba(MAG, 0.22);
        c.fillRect(px - 4 * scale, py - ph, pw + 8 * scale, 6 * scale);
        c.fillRect(px - 2 * scale, py - ph * 0.55, pw + 4 * scale, 5 * scale);
      }
    }
    for (let i = 0; i < lanterns.length; i++) {
      const L = lanterns[i];
      const lx = sx(L.x);
      const ly = sy(L.y + Math.sin(L.bob) * 4);
      const ls = L.s * scale;
      c.fillStyle = rgba(GOLD, 0.18);
      c.beginPath();
      c.arc(lx, ly, ls * 1.6, 0, TAU);
      c.fill();
      c.fillStyle = rgba(GOLD, 0.78);
      c.fillRect(lx - ls * 0.45, ly - ls * 0.55, ls * 0.9, ls * 1.1);
      c.fillStyle = rgba(VERM, 0.7);
      c.beginPath();
      c.moveTo(lx - ls * 0.4, ly - ls * 0.55);
      c.lineTo(lx, ly - ls * 1.15);
      c.lineTo(lx + ls * 0.4, ly - ls * 0.55);
      c.fill();
    }
  ...
  function drawRider() {
    ...
    if (G.tension > 0.18) {
      const ta = 0.12 + G.tension * 0.38;
      c.strokeStyle = rgba(G.tension > 0.7 ? GOLD : MAG, ta);
      c.lineWidth = Math.max(1.2, (1.4 + G.tension * 1.6) * s);
      c.beginPath();
      c.arc(0, 0, (14 + G.tension * 10 + Math.sin(G.t * 10) * 2) * s, 0, TAU);
      c.stroke();
    }
    if (G.lockHold) {
      c.strokeStyle = rgba(MAG, 0.5 + 0.25 * Math.sin(G.t * 10));
      c.lineWidth = Math.max(1.2, 1.6 * s);
      c.beginPath();
      c.arc(0, 0, (18 + Math.sin(G.t * 8) * 2) * s, 0, TAU);
      c.stroke();
    }

    c.fillStyle = rgba(FOX, 0.95);
    c.beginPath();
    c.moveTo(-6 * s, 4 * s);
    c.quadraticCurveTo(-22 * s, -2 * s, -18 * s, 10 * s);
    c.quadraticCurveTo(-8 * s, 16 * s, 8 * s, 8 * s);
    c.quadraticCurveTo(6 * s, 2 * s, -2 * s, 2 * s);
    c.closePath();
    c.fill();
    ...
    c.beginPath();
    c.arc(-4 * s, 5 * s, 1.1 * s, 0, TAU);
    c.fill();

    c.fillStyle = rgba([90, 18, 58], 0.95);
    c.beginPath();
    c.moveTo(-2 * s, 2 * s);
    c.lineTo(10 * s, 4 * s);
    c.lineTo(6 * s, 16 * s);
    ...
    c.fillRect(0, 5 * s, 8 * s, 1.4 * s);

    c.fillStyle = rgba(SKIN, 1);
    c.beginPath();
    c.arc(5 * s, -6 * s, 6.2 * s, 0, TAU);
    c.fill();

    c.fillStyle = rgba(HAIR, 0.95);
    c.beginPath();
    c.moveTo(-6 * s, -4 * s);
    c.quadraticCurveTo(-16 * s, 4 * s, -12 * s, 12 * s);
    c.quadraticCurveTo(-2 * s, 0, 4 * s, -8 * s);
    c.quadraticCurveTo(12 * s, -18 * s, 13 * s, -7 * s);
    c.lineTo(8 * s, -4 * s);
    c.closePath();
    c.fill();
    ...
    c.fillStyle = rgba(DEEP, 0.92);
    c.beginPath();
    c.arc(7.2 * s, -5.6 * s, 1.05 * s, 0, TAU);
    c.fill();

    c.fillStyle = rgba(PAPER, 0.95);
    c.save();
    c.translate(16 * s, -1 * s);
    c.rotate(-0.2);
    c.fillRect(0, -3 * s, 8 * s, 11 * s);
    c.fillStyle = rgba(MAG, 0.8);
    c.fillRect(2 * s, -1 * s, 4 * s, 2 * s);
    c.restore();

    if (G.muzzle > 0) {
      c.fillStyle = rgba(G.lockHold ? MAG : GOLD, clamp(G.muzzle * 10, 0, 1));
      c.beginPath();
      c.arc(26 * s, 0, (5 + G.muzzle * 40) * s, 0, TAU);
      c.fill();
    }
    c.restore();
    c.globalAlpha = 1;
  ...
  function drawLockBeams() {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      const e = G.ents[i];
      if (!e.locked || e.dead) continue;
      c.strokeStyle = rgba(MAG, 0.38 + 0.22 * Math.sin(G.t * 12 + e.lockI));
  ...
  function drawEnt(e) {
    ...
    if (e.type === 'crane') {
      ...
      c.lineTo(x - 2 * s, y);
      c.lineTo(x - 8 * s, y + 6 * s);
      c.closePath();
      c.fill();
      const flap = Math.sin(G.t * 14 + e.x) * 5;
      c.fillStyle = rgba(HOT, 0.8);
      c.beginPath();
      c.moveTo(x - 2 * s, y);
      c.lineTo(x - 14 * s, y - (6 + flap) * s);
      c.lineTo(x + 2 * s, y);
      c.fill();
      c.fillStyle = rgba(DEEP, 0.9);
      c.beginPath();
      c.arc(x + 4 * s, y - 1 * s, 1 * s, 0, TAU);
      c.fill();
    } else if (e.type === 'lantern') {
      ...
      c.fillRect(x - 6 * s, y - 5 * s, 12 * s, 14 * s);
      c.fillStyle = rgba(VERM, 0.9);
      c.beginPath();
      c.moveTo(x - 5 * s, y - 5 * s);
      c.lineTo(x, y - 14 * s);
      c.lineTo(x + 5 * s, y - 5 * s);
      c.closePath();
      c.fill();
      ...
      c.fillStyle = rgba(VERM, 0.95);
      c.beginPath();
      c.ellipse(x, y, 12 * s, 11 * s, 0, 0, TAU);
      c.fill();
      c.fillStyle = rgba(DEEP, 0.92);
      c.beginPath();
      c.ellipse(x - 4 * s, y - 2 * s, 2.4 * s, 3.2 * s, 0, 0, TAU);
      c.ellipse(x + 4 * s, y - 2 * s, 2.4 * s, 3.2 * s, 0, 0, TAU);
      c.fill();
      c.fillStyle = rgba(GOLD, 0.85);
      c.beginPath();
      c.moveTo(x - 3 * s, y + 5 * s);
      c.lineTo(x, y + 9 * s);
      c.lineTo(x + 3 * s, y + 5 * s);
      c.fill();
      c.fillStyle = rgba(PAPER, 0.7);
      c.fillRect(x - 10 * s, y - 14 * s, 6 * s, 4 * s);
      c.fillRect(x + 4 * s, y - 14 * s, 6 * s, 4 * s);
    } else if (e.type === 'ofuda') {
      c.save();
      c.translate(x, y);
      c.rotate(e.spin || 0);
      c.fillStyle = rgba(PAPER, 0.96);
      c.fillRect(-5 * s, -11 * s, 10 * s, 22 * s);
      c.fillStyle = rgba(MAG, 0.9);
      c.fillRect(-2 * s, -7 * s, 4 * s, 3 * s);
      c.fillRect(-2 * s, 2 * s, 4 * s, 8 * s);
      c.restore();
    } else if (e.type === 'foxfire') {
      c.fillStyle = rgba(FOX, 0.35);
      c.beginPath();
      ...
      c.beginPath();
      c.arc(x, y, 2.2 * s, 0, TAU);
      c.fill();
    } else if (e.type === 'komainu') {
      c.fillStyle = rgba([90, 40, 48], 0.95);
      c.fillRect(x - 12 * s, y - 6 * s, 24 * s, 16 * s);
      c.fillStyle = rgba(VERM, 0.9);
      c.beginPath();
      c.arc(x + 6 * s, y - 8 * s, 8 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(GOLD, 0.8);
      c.fillRect(x - 4 * s, y - 2 * s, 8 * s, 4 * s);
      c.fillStyle = rgba(DEEP, 0.9);
      c.beginPath();
      c.arc(x + 8 * s, y - 9 * s, 1.4 * s, 0, TAU);
      c.fill();
    } else if (e.type === 'tengu') {
      c.fillStyle = rgba(VIO, 0.95);
      c.fillRect(x - 8 * s, y - 8 * s, 16 * s, 18 * s);
      c.fillStyle = rgba(VERM, 0.9);
      c.beginPath();
      c.arc(x, y - 14 * s, 7 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(PAPER, 0.9);
      c.beginPath();
      c.moveTo(x + 4 * s, y - 14 * s);
      c.lineTo(x + 16 * s, y - 12 * s);
      c.lineTo(x + 4 * s, y - 10 * s);
      c.closePath();
      c.fill();
      c.fillStyle = rgba(DEEP, 0.9);
      c.fillRect(x - 3 * s, y - 18 * s, 6 * s, 3 * s);
    } else if (e.type === 'cart') {
      ...
      c.fill();
      c.fillStyle = rgba(PAPER, 0.9);
      c.fillRect(x - 6 * s, y - 10 * s, 8 * s, 14 * s);
      c.fillStyle = rgba(GOLD, 0.85);
      c.beginPath();
      c.arc(x + 8 * s, y, 4 * s, 0, TAU);
      c.fill();
    }
    c.restore();
  }
  ...
  function drawBoss(e) {
    ...
    const s = scale;
    if (e.kind === 'gate') {
      c.fillStyle = rgba(VERM, 0.92);
      c.fillRect(x - 40 * s, y - 36 * s, 10 * s, 72 * s);
      c.fillRect(x + 30 * s, y - 36 * s, 10 * s, 72 * s);
      c.fillRect(x - 48 * s, y - 48 * s, 96 * s, 12 * s);
      c.fillRect(x - 36 * s, y - 28 * s, 72 * s, 8 * s);
      c.fillStyle = rgba(INK, 0.95);
      c.beginPath();
      c.ellipse(x, y + 6 * s, 22 * s, 26 * s, 0, 0, TAU);
      c.fill();
      c.fillStyle = rgba(GOLD, 0.9);
      c.beginPath();
      c.arc(x - 8 * s, y - 2 * s, 5 * s, 0, TAU);
      c.arc(x + 8 * s, y - 2 * s, 5 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(DEEP, 0.9);
      c.beginPath();
      c.arc(x - 8 * s, y - 2 * s, 2 * s, 0, TAU);
      c.arc(x + 8 * s, y - 2 * s, 2 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(VERM, 0.85);
      c.fillRect(x - 10 * s, y + 12 * s, 20 * s, 6 * s);
    } else if (e.kind === 'ofuda') {
      c.fillStyle = rgba(PAPER, 0.22);
      c.beginPath();
      c.arc(x, y, 44 * s, 0, TAU);
      c.fill();
      c.save();
      c.translate(x, y);
      c.rotate(Math.sin(e.t) * 0.12);
      c.fillStyle = rgba(PAPER, 0.96);
      c.fillRect(-18 * s, -36 * s, 36 * s, 72 * s);
      c.fillStyle = rgba(MAG, 0.9);
      c.fillRect(-8 * s, -24 * s, 16 * s, 8 * s);
      c.fillRect(-6 * s, -6 * s, 12 * s, 28 * s);
      c.restore();
      c.fillStyle = rgba(SKIN, 1);
      c.beginPath();
      c.arc(x, y - 8 * s, 10 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(HAIR, 0.95);
      c.beginPath();
      c.arc(x, y - 14 * s, 12 * s, Math.PI, 0);
      c.fill();
      c.fillStyle = rgba(DEEP, 0.9);
      c.beginPath();
      c.arc(x - 3 * s, y - 8 * s, 1.4 * s, 0, TAU);
      c.arc(x + 4 * s, y - 8 * s, 1.4 * s, 0, TAU);
      c.fill();
    } else {
      c.fillStyle = rgba(FOX, 0.28);
      c.beginPath();
      c.arc(x, y, 50 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(FOX, 0.95);
      c.beginPath();
      c.ellipse(x, y + 8 * s, 40 * s, 24 * s, 0, 0, TAU);
      c.fill();
      c.fillStyle = rgba(PAPER, 0.9);
      c.beginPath();
      c.moveTo(x - 18 * s, y - 10 * s);
      c.lineTo(x - 38 * s, y - 40 * s);
      c.lineTo(x - 6 * s, y - 16 * s);
      c.closePath();
      c.moveTo(x + 8 * s, y - 14 * s);
      c.lineTo(x + 28 * s, y - 44 * s);
      c.lineTo(x + 18 * s, y - 10 * s);
      c.closePath();
      c.fill();
      c.fillStyle = rgba(SKIN, 1);
      c.beginPath();
      c.arc(x + 8 * s, y - 8 * s, 12 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(HAIR, 0.95);
      c.beginPath();
      c.arc(x + 8 * s, y - 16 * s, 13 * s, Math.PI, 0);
      c.fill();
      c.fillStyle = rgba(VIO, 0.95);
      c.beginPath();
      c.moveTo(x + 2 * s, y - 18 * s);
      c.lineTo(x + 10 * s, y - 36 * s);
      c.lineTo(x + 18 * s, y - 16 * s);
      c.closePath();
      c.fill();
      c.fillStyle = rgba(DEEP, 0.9);
      c.beginPath();
      c.arc(x + 12 * s, y - 8 * s, 2 * s, 0, TAU);
      c.fill();
      c.fillStyle = rgba(GOLD, 0.9);
      c.beginPath();
      c.arc(x - 16 * s, y + 4 * s, 6 * s, 0, TAU);
      c.fill();
    }
  }
  ...
  function drawShots() {
    ...
    for (let i = 0; i < G.shots.length; i++) {
      ...
      if (s.lock) {
        ...
        c.rotate(Math.atan2(s.vy, s.vx));
        c.fillStyle = rgba(PAPER, 0.96);
        c.fillRect(-6 * scale, -3.2 * scale, 12 * scale, 6.4 * scale);
        c.fillStyle = rgba(MAG, 0.85);
        c.fillRect(-2 * scale, -1.4 * scale, 6 * scale, 2.8 * scale);
        c.restore();
      }
    }
    for (let i = 0; i < G.eShots.length; i++) {
      const s = G.eShots[i];
      c.fillStyle = rgba(s.rgb || MAG, 0.95);
      c.beginPath();
      c.arc(sx(s.x), sy(s.y), s.r * scale, 0, TAU);
      c.fill();
      c.fillStyle = rgba(WHT, 0.55);
      c.beginPath();
      c.arc(sx(s.x - 0.6), sy(s.y - 0.6), s.r * 0.4 * scale, 0, TAU);
      c.fill();
    }
  }
  ...
  function drawPows() {
    ...
    for (let i = 0; i < G.pows.length; i++) {
      ...
      c.fillStyle = rgba(GOLD, 0.28);
      c.beginPath();
      c.arc(x, y, 12 * scale, 0, TAU);
      c.fill();
      c.save();
      c.translate(x, y);
      c.rotate(p.t * 2.4);
      c.fillStyle = rgba(PAPER, 0.96);
      c.fillRect(-5 * scale, -8 * scale, 10 * scale, 16 * scale);
      c.fillStyle = rgba(MAG, 0.9);
      c.fillRect(-2 * scale, -4 * scale, 4 * scale, 3 * scale);
      c.restore();
    }
  }
  ...
  function startGame(kind) {
    ...
    syncHud();
    audio.start();
    toast(isSea() ? '式海 · 更密更快' : '夜城 · 朱门', false, true);
  }
  ...
  function goTitle() {
    ...
    clearField();
    seedDecor();
    G.ents.push(makeBoss('kami', '大式', 158));
    G.ents[0].x = VW - 150;
    G.ents[0].y = VH * 0.55;
    G.ents[0].in = false;
    G.ents.push(makeMask(VW * 0.58, VH * 0.32));
    G.ents.push(makeLantern(VW * 0.64, VH * 0.7));
    G.ents[1].locked = true;
    G.ents[1].lockI = 1;
    showOverlay(
      'title',
      '式神',
      '骑纸狐从左往右穿夜城。空格连射符纸，Shift / Z 锁定敌机放出式矢。贴身叠「临」才高分。撞上掉命。过关才见关底。'
    );
    syncHud();
  }
