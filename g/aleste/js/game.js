...
(function () {
  ...
  const AUTO_SPEED_KEY = 'playbox-aleste-auto-speed';
  ...
  function syncHud() {
    ...
    else setHint('方向移动 · 空格开火 · Shift 爆弹 · 捡 波/扇/穿 · A 自动', '');
    syncPips();
  }
  ...
  function autoDanger(x, y, horizon) {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      ...
      const r = (e.type === 'boss' ? e.r * 0.62 : e.type === 'mid' ? e.r * 0.68 : e.r * 0.72);
      const body = hitR + r;
      if (dist < body + 28) {
        const soon = (look - t) / Math.max(0.08, look);
        const w = e.type === 'dive' ? 36 : e.type === 'boss' ? 14 : e.type === 'mid' ? 16 : 18;
  ...
  function autoWantKind() {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      ...
      if (e.type === 'dive') dives += 1;
      if (e.type === 'stream') streams += 1;
      if (e.type === 'boss' || e.type === 'mid') big = true;
    }
    if (big) return 'pierce';
    if (n >= 6 && (maxX - minX) > 140) return 'wide';
    if (streams >= 3 || dives >= 3) return 'wave';
    if (lined >= 3) return 'pierce';
    if (G.wpn !== 'straight') return G.wpn;
    return 'wide';
  }
  ...
  function autoThink() {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      ...
      else if (e.type === 'turret') w = 88;
      else if (e.type === 'orb') w = 70;
      else if (e.type === 'stream') w = 58;
      else if (e.type === 'mid') w = 240;
      else if (e.type === 'boss') w = 280 + e.hp * 0.35;
      else w = 36 + (e.hp || 1) * 8;
      w += (e.hp || 1) * 4;
    ...
    for (let i = 0; i < G.pows.length; i++) {
      ...
      if (p.kind === 'bomb') {
        ...
        if (G.wpn === 'straight' && G.wpnLv <= 0) w += 88;
