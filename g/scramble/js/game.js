...
/* 攀升 — Scramble remake. No CDN. Distinct from 潜猎 / 泽泽 / 星轨 / 沙罗. */
...
var CORE_X = 10020;
var MIN_GAP = 102;
var MIN_GAP_DENSE = 90;
var BEST_KEY = 'playbox-scramble-best';
var MUTE_KEY = 'playbox-scramble-mute';
var OPS = '方向键 / WASD 飞 · 空格射击 · Shift / Z 投弹 · R 重开 · M 静音';
var SEC_NAME = ['荒岭', '溶腔', '火陨', '高堡', '迷宫', '核门'];
var SEC_END = [RIDGE_END, CAVE_END, FIRE_END, CITY_END, MAZE_END, BASE_END];
...
function sectionOf(x) {
  if (x < RIDGE_END) return 0;
  if (x < CAVE_END) return 1;
  if (x < FIRE_END) return 2;
  if (x < CITY_END) return 3;
  if (x < MAZE_END) return 4;
  return 5;
}
function sectionName(s) {
  return SEC_NAME[s] || SEC_NAME[5];
}
function scrollSpeed(dense, section) {
  var base = dense ? 154 : 116;
  var add = [0, 6, 10, 8, 16, 20][section] || 0;
  return base + add;
}
function fuelDrain(dense) {
  return dense ? 7.0 : 5.4;
}
...
function heightAt(x, s, dense) {
  ...
  if (s === 0) {
    ...
    t = clamp((x - RIDGE_END) / Math.max(1, CAVE_END - RIDGE_END), 0, 1);
    gap = lerp(212, 146, t) + (n - 0.5) * 16;
    if (dense) gap *= 0.88;
    mid = 208 + (n2 - 0.5) * 72;
    c = mid - gap * 0.5 + tooth(x, 22);
    f = mid + gap * 0.5 - tooth(x + 900, 20);
  } else if (s === 2) {
    c = 44 + n * 30 + tooth(x, 10);
    f = 330 + (n2 - 0.5) * 18;
  } else if (s === 3) {
    c = 16 + n * 14;
    f = 326 + (n - 0.5) * 18;
  } else if (s === 4) {
    t = clamp((x - CITY_END) / Math.max(1, MAZE_END - CITY_END), 0, 1);
    gap = lerp(128, 108, t) + (n - 0.5) * 8;
    if (dense) gap *= 0.86;
    mid = 140 + stepNoise(x, 150, 1.7) * 150;
    c = Math.round((mid - gap * 0.5) / 8) * 8;
    f = Math.round((mid + gap * 0.5) / 8) * 8;
  } else {
    t = clamp((x - MAZE_END) / Math.max(1, BASE_END - MAZE_END), 0, 1);
    gap = lerp(120, 104, t) + (n - 0.5) * 8;
    if (dense) gap *= 0.88;
    mid = 176 + stepNoise(x, 132, 4.1) * 90;
...
function fuelXForSlot(slot) {
  return slot * FUEL_EVERY + 110 + hash2(slot * 9.1) * 150;
}
...
function makeShip() {
  return { sx: 108, sy: 210, vx: 0, vy: 0 };
}
function makeShot(x, y) {
  return { x: x, y: y, vx: SHOT_V, r: 3.2, life: 1.5, dead: false };
}
function makeBomb(x, y) {
  return { x: x, y: y, vx: 176, vy: 78, r: 4.4, life: 1.7, dead: false };
}
...
function makeCore(x, floorY, ceilY) {
  var h = clamp(floorY - ceilY - 16, 58, 100);
  return { kind: 'core', x: x, y: floorY - h, w: 78, h: h, hp: 10, score: 1000, dead: false, flash: 0 };
}
function makeEbullet(x, y, vx, vy) {
  return { x: x, y: y, vx: vx, vy: vy, r: 3.1, life: 2.5, dead: false };
}
function shipBox(ship, cam) {
  return { x: cam + ship.sx - 10, y: ship.sy - 5, w: 22, h: 11 };
}
function entBox(e) {
...
function selfCheck() {
  var v, h0, h1, h2, h4, n1, b, s, slot, fx, i, gap0, gap1, gap4, ng;
  var hFire, gapFire, hCity, hBase, gapBase, sealed;
  if (BEST_KEY !== 'playbox-scramble-best') throw new Error('best key');
  if (MUTE_KEY !== 'playbox-scramble-mute') throw new Error('mute key');
  if (LIVES !== 3) throw new Error('3 lives');
  if (!(RIDGE_END < CAVE_END && CAVE_END < FIRE_END && FIRE_END < CITY_END && CITY_END < MAZE_END && MAZE_END < BASE_END)) {
    throw new Error('section order');
  }
  if (CORE_X <= MAZE_END || CORE_X >= BASE_END) throw new Error('core in base');
  if (scrollSpeed(true, 0) <= scrollSpeed(false, 0)) throw new Error('dense faster');
  if (scrollSpeed(false, 5) <= scrollSpeed(false, 0)) throw new Error('base faster');
  ...
  h1 = heights(2500, false);
  hFire = heights(4200, false);
  hCity = heights(5800, false);
  h4 = heights(7600, false);
  hBase = heights(9400, false);
  n1 = heights(2500, true);
  gap0 = h0.f - h0.c;
  gap1 = h1.f - h1.c;
  ...
  if (gap1 >= gap0 - 40) throw new Error('cave tighter');
  if (gapFire <= gap1) throw new Error('fire more open');
  if (hCity.f - hCity.c < 220) throw new Error('city open');
  if (gap4 >= gap1 - 8) throw new Error('maze tighter');
  if (gapBase >= gap1) throw new Error('base tight');
  if (Math.abs((h4.c + h4.f) - (heights(8200, false).c + heights(8200, false).f)) < 16) {
    throw new Error('maze winds');
  }
  ...
  if (sectionOf(100) !== 0 || sectionOf(2500) !== 1 || sectionOf(4200) !== 2) throw new Error('sections');
  if (sectionOf(5800) !== 3 || sectionOf(7600) !== 4 || sectionOf(9400) !== 5) throw new Error('late sections');
  if (sectionName(0) !== '荒岭' || sectionName(2) !== '火陨' || sectionName(5) !== '核门') throw new Error('names');
  b = makeBomb(0, 0);
  ...
  if (i.hp < 8) throw new Error('core hp');
  ...
  (function () {
    ...
    while (x < end) {
      ...
      slotN = Math.floor((x - 110) / FUEL_EVERY);
...
var pointer = { down: false, hover: false, x: 108, y: 210, id: null };
var inputSrc = 'key';
var pips = [];
...
var audio = {
  ...
  bomb: function () {
    this.ensure();
    this.beep(170, 0.09, 'sawtooth', 0.042, 80);
...
function showOverlay(kind, title, lead) {
  ...
  ovKicker.textContent = kind === 'win' ? 'CLEAR' : kind === 'lose' ? 'DOWN' : 'SCRM';
...
function nearFuelWorld(x) {
  var slot = Math.round((x - 110) / FUEL_EVERY);
...
function stampRange(x0, x1) {
  ...
  for (x = Math.ceil(x0 / 82) * 82; x < x1; x += 82) {
    if (x < 160 || x > CORE_X - 60) continue;
    if (nearFuelWorld(x)) continue;
    r = hash2(x * 0.017 + 3.3 + (dense ? 2.1 : 0));
    sec = sectionOf(x);
    h = heights(x, dense);
    mid = (h.c + h.f) * 0.5;
    if (sec === 0) {
      ...
      } else if (r < 0.5) G.ents.push(makeGun(x, h.f));
      else if (r < 0.86) G.ents.push(makeRocket(x, h.f));
      if (hash2(x * 0.09 + 8) > 0.88) G.ents.push(makeUfo(x + 36, 78 + hash2(x) * 48));
    } else if (sec === 1) {
      if (r < 0.2) G.ents.push(makeGun(x, h.f));
      else if (r < 0.52) G.ents.push(makeRocket(x, h.f));
      else if (r < 0.8) G.ents.push(makeUfo(x, mid + (hash2(x + 1) - 0.5) * 28));
      if (dense && hash2(x * 0.13) > 0.72) G.ents.push(makeFire(x + 28, mid + (hash2(x + 4) - 0.5) * 36));
    } else if (sec === 2) {
      if (r < 0.22) G.ents.push(makeRocket(x, h.f));
      else if (r < 0.4) G.ents.push(makeGun(x, h.f));
      if (hash2(x * 0.21 + 2) > 0.28) {
        G.ents.push(makeFire(x + 18, mid + (hash2(x + 6) - 0.5) * (h.f - h.c - 40)));
      }
      if (hash2(x * 0.17) > 0.55) {
        G.ents.push(makeFire(x + 50, h.c + 28 + hash2(x + 9) * (h.f - h.c - 56)));
      }
    } else if (sec === 3) {
      if (r < 0.5) {
        bw = 16 + hash2(x + 2) * 22;
        bh = 28 + hash2(x + 5) * 70;
        G.ents.push(makeBldg(x, h.f, bw, bh));
      } else if (r < 0.7) G.ents.push(makeGun(x, h.f));
      else if (r < 0.9) G.ents.push(makeRocket(x, h.f));
    } else if (sec === 4) {
      if (r < 0.28) G.ents.push(makeRocket(x, h.f));
      else if (r < 0.48) G.ents.push(makeGun(x, h.f));
      else if (r < 0.7) G.ents.push(makeUfo(x, mid));
      if (dense && hash2(x * 0.19) > 0.62) G.ents.push(makeFire(x + 22, mid));
    } else {
      if (r < 0.36) G.ents.push(makeGun(x, h.f));
      else if (r < 0.64) G.ents.push(makeRocket(x, h.f));
      else if (r < 0.8) G.ents.push(makeUfo(x, mid));
      if (dense && hash2(x * 0.15) > 0.58) G.ents.push(makeFire(x + 16, mid));
    }
  }
...
function goTitle() {
  ...
  showOverlay(
    'title',
    '攀升',
    '横穿洞穴，开火打前，投弹砸地，打油桶续航。撞壁、中弹、油尽都扣命。六段之后打核。别当成潜猎、泽泽、星轨或沙罗——这是攀升，不是潜舰鱼雷，不是触手，不是轨束，不是器官武装。'
  );
}
function startGame(dense) {
  ...
  resetRun(dense);
  hideOverlay();
  toast(dense ? '洞核 · 更密更快' : '攀升 · 荒岭', dense ? 'warn' : 'gold', 900);
  setHint('空格射击 · Shift / Z 投弹 · 打油桶续航', '');
}
function goLose() {
  G.mode = 'lose';
  persistBest();
  syncHud();
  showOverlay('lose', '坠机了', whyText(G.why) + ' · ' + (G.dense ? '洞核' : '攀升') + '  ' + G.score + ' 分 · 连击最高 ' + G.maxCombo);
}
function goWin() {
  G.mode = 'win';
  persistBest();
  syncHud();
  audio.win();
  showOverlay('win', '捣穿了', (G.dense ? '洞核' : '攀升') + ' 核门清空 · ' + G.score + ' 分 · 连击最高 ' + G.maxCombo);
}
function restart() {
  audio.ensure();
...
function fireShot() {
  ...
  G.shots.push(makeShot(wx() + 16, G.ship.sy));
  G.shotCd = 0.11;
  G.muzzle = 0.06;
...
function dropBomb() {
  ...
  G.bombs.push(makeBomb(wx() + 4, G.ship.sy + 7));
  G.bombCd = 0.2;
  audio.bomb();
  kick(3.2, 'hit');
  emit(5, {
    x: wx() + 4, y: G.ship.sy + 7, j: 3,
...
function killEnt(e, src) {
  ...
  if (e.dead) return;
  e.dead = true;
  rgb = e.kind === 'fuel' ? GOLD : e.kind === 'ufo' ? CYN : e.kind === 'core' ? MAG : e.kind === 'fire' ? HOT : MINT;
  n = e.kind === 'core' ? 44 : e.kind === 'fuel' ? 22 : 14;
  ...
  bumpCombo();
  extra = e.kind === 'core' ? 1000 : e.score;
...
function tickShip(dt) {
  ...
  nx = wx();
  if (G.invuln <= 0) {
    if (terrainHit(nx - 6, G.ship.sy, G.dense) || terrainHit(nx + 11, G.ship.sy, G.dense) ||
        terrainHit(nx, G.ship.sy - 5, G.dense) || terrainHit(nx, G.ship.sy + 5, G.dense)) {
      crash('wall');
      return;
    }
  }
  ...
  emit(1, {
    x: nx - 13, y: G.ship.sy + rand(-2, 2), j: 1,
    vx0: -90, vx1: -32, vy0: -16, vy1: 16,
    r0: 1, r1: 2.2, life: 0.2, rgb: G.dense ? MAG : MINT, g: 0
  });
}
...
function tickEnts(dt) {
  ...
  for (i = 0; i < G.ents.length; i++) {
    ...
    if (e.kind === 'rocket') {
      ...
      e.ph += dt * 5.2;
      e.x += e.vx * dt;
      e.y += Math.sin(e.ph) * 30 * dt;
      dx = shipY - e.y;
      e.y += clamp(dx, -42, 42) * dt * 0.32;
...
function noteSection() {
  ...
  if (s !== G.section) {
    ...
    if (secLabel) {
      ...
      secLabel.classList.toggle('hot', s >= 4);
    }
    audio.section();
    toast(sectionName(s), s >= 4 ? 'gold' : 'fuel', 900);
  }
}
...
function update(dt) {
  ...
  if (G.mode === 'title') {
    ...
    emit(1, {
      ...
      r0: 1, r1: 2, life: 0.2, rgb: MINT, g: 0
    });
    return;
  }
  if (G.stop > 0) {
...
function drawBg() {
  ...
  if (sec === 0) fill = '#06180e';
  else if (sec === 1) fill = '#05140f';
  else if (sec === 2) fill = '#140c08';
  else if (sec === 3) fill = '#07140c';
  else if (sec === 4) fill = '#04120c';
  else fill = '#0a1012';
  ctx.fillStyle = fill;
  ...
  for (i = 0; i < stars.length; i++) {
    ...
    gy = s.y;
    ctx.fillStyle = rgba(sec === 2 ? AMB : WHT, s.a * tw * (sec === 1 ? 0.45 : 0.9));
  ...
  if (sec === 0 || sec === 3) {
    ...
    ctx.closePath();
    ctx.fillStyle = sec === 3 ? 'rgba(10,36,22,0.5)' : 'rgba(8,40,22,0.42)';
    ctx.fill();
  }
  if (sec === 2) {
...
function drawTerrain() {
  ...
  step = 5;
  fillF = sec === 2 ? '#1a1008' : sec === 5 ? '#101418' : '#0a2014';
  fillC = sec === 2 ? '#140c08' : '#08180f';
  edge = rimColor(sec);
  ctx.beginPath();
  first = true;
...
function drawEnts() {
  ...
  for (i = 0; i < G.ents.length; i++) {
    ...
    if (e.kind === 'bldg') {
      ...
      glow = 5.2 + Math.sin(e.ph) * 1.6;
      ctx.fillStyle = rgba(MAG, 0.55);
      ctx.beginPath();
      ctx.arc(x + w * 0.5, y + h * 0.5, (glow + 2) * L.s, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(HOT, 0.92);
      ctx.beginPath();
      ctx.arc(x + w * 0.5, y + h * 0.5, glow * L.s, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(GOLD, 0.9);
      ctx.beginPath();
      ctx.arc(x + w * 0.5, y + h * 0.5, 2.5 * L.s, 0, TAU);
      ctx.fill();
    } else if (e.kind === 'core') {
      glow = 0.4 + 0.6 * (1 - e.hp / 10);
      ctx.fillStyle = '#101820';
      ctx.strokeStyle = rgba(e.flash > 0 ? WHT : MAG, 0.92);
      ctx.lineWidth = 2.2 * L.s;
      roundRect(x, y, w, h, 8 * L.s);
      ctx.fill();
      ctx.stroke();
      hx = x + w * 0.5;
      ctx.fillStyle = rgba(MAG, 0.22 + glow * 0.5);
      ctx.beginPath();
      ctx.arc(hx, y + h * 0.52, (12 + glow * 10) * L.s, 0, TAU);
      ctx.fill();
      ctx.save();
      ctx.translate(hx, y + h * 0.52);
      ctx.rotate(G.t * 1.4);
      ...
      ctx.moveTo(-10 * L.s, 0);
      ctx.lineTo(10 * L.s, 0);
      ctx.moveTo(-5 * L.s, -8.6 * L.s);
      ctx.lineTo(5 * L.s, 8.6 * L.s);
      ctx.moveTo(-5 * L.s, 8.6 * L.s);
      ctx.lineTo(5 * L.s, -8.6 * L.s);
      ctx.stroke();
      ctx.restore();
...
function drawProjectiles() {
  ...
  for (i = 0; i < G.bombs.length; i++) {
    ...
    ctx.fillStyle = rgba(GOLD, 0.95);
    ctx.beginPath();
    ctx.arc(sx(b.x), sy(b.y), 3.5 * L.s, 0, TAU);
    ctx.fill();
...
function drawShip() {
  ...
  if (G.muzzle > 0) {
    ...
    ctx.fillRect(13 * L.s, -1.6 * L.s, 11 * L.s, 3.2 * L.s);
    ctx.fillStyle = rgba(CYN, 0.85);
    ctx.fillRect(13 * L.s, -3.2 * L.s, 7 * L.s, 6.4 * L.s);
  }
  ctx.fillStyle = rgba(MINT, 0.96);
  ctx.beginPath();
  ctx.moveTo(14 * L.s, 0);
  ctx.lineTo(-4 * L.s, -8 * L.s);
  ctx.lineTo(-11 * L.s, -3.2 * L.s);
  ctx.lineTo(-8 * L.s, 0);
  ctx.lineTo(-11 * L.s, 3.2 * L.s);
  ctx.lineTo(-4 * L.s, 8 * L.s);
  ctx.closePath();
  ...
  ctx.fillStyle = rgba(CYN, 0.95);
  ctx.beginPath();
  ctx.ellipse(-1 * L.s, -1.2 * L.s, 5.2 * L.s, 3.1 * L.s, 0, 0, TAU);
  ctx.fill();
  ctx.fillStyle = rgba(GOLD, 0.95);
  ctx.beginPath();
  ctx.moveTo(8 * L.s, 0);
  ctx.lineTo(1 * L.s, -2.6 * L.s);
  ctx.lineTo(1 * L.s, 2.6 * L.s);
  ctx.closePath();
  ctx.fill();
  ctx.fillStyle = rgba(GOLD, 0.85);
  ctx.fillRect(-12 * L.s, -2.2 * L.s, 5 * L.s, 4.4 * L.s);
  ctx.restore();
}
