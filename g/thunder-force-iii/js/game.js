...
(function () {
  ...
  const CLAW_R = 11;
  const CLAW_MAX = 2;
  const BEST_KEY = 'playbox-thunder-force-iii-best';
  const MUTE_KEY = 'playbox-thunder-force-iii-mute';
  const OPS = '←↑↓→ / WASD 飞 · 空格射击 · Shift / Z 换武器 · R 重开 · M 静音';
  ...
  const WEPS = [
    { name: '双射', cd: 0.090, dmg: 1 },
    { name: '回射', cd: 0.100, dmg: 1.1 },
    { name: '波刃', cd: 0.160, dmg: 1.35 },
    { name: '猎弹', cd: 0.210, dmg: 1.75 }
  ];
  ...
  const STAGES = [
    {
      ...
      waves: [
        { t: 0.6, kind: 'v', n: 5 },
        { t: 2.2, kind: 'carrier' },
        { t: 3.8, kind: 'beetle', n: 3 },
        { t: 5.6, kind: 'turret', n: 2 },
        { t: 7.8, kind: 'v', n: 6 },
        { t: 10.2, kind: 'carrier' },
        { t: 12.6, kind: 'beetle', n: 4 },
        { t: 15.2, kind: 'mix' },
        { t: 18.0, kind: 'v', n: 7 },
        { t: 20.6, kind: 'turret', n: 2 },
        { t: 23.6, kind: 'boss' }
      ]
    },
    {
      ...
      waves: [
        { t: 0.5, kind: 'v', n: 6 },
        { t: 2.0, kind: 'dart', n: 3 },
        { t: 3.4, kind: 'walker', n: 2 },
        { t: 5.2, kind: 'turret', n: 3 },
        { t: 7.6, kind: 'elite' },
        { t: 10.2, kind: 'carrier' },
        { t: 12.6, kind: 'v', n: 8 },
        { t: 15.0, kind: 'dart', n: 4 },
        { t: 17.4, kind: 'walker', n: 3 },
        { t: 20.2, kind: 'mix' },
        { t: 23.4, kind: 'boss' }
      ]
    },
  ...
  function kindBest() {
    return isRail() ? G.best.r : G.best.l;
  }
  ...
  function fireScale() {
    return isRail() ? 0.92 : 1;
  }
  function enemyFire() {
    return isRail() ? 0.76 : 1;
  }

  function groundY(wx) {
    const n = Math.sin(wx * 0.011) * 14 + Math.sin(wx * 0.029 + 1.3) * 9 + Math.sin(wx * 0.0073 + 0.4) * 7;
    const amp = G.stage === 2 ? 1.15 : G.stage === 3 ? 0.45 : 0.75;
    let g = VH - 24 + n * amp;
  ...
  function pylonCell(wx) {
    return Math.floor((wx + 40) / 240);
  }

  function pylonHAt(wx) {
    if (G.stage !== 3 || G.boss) return 0;
    const cell = pylonCell(wx);
    const h = hash2(cell * 19 + 7);
    if (h < 0.58) return 0;
    return 40 + h * 78;
  }

  function pylonCx(cell) {
    return cell * 240 + 120;
  }

  function pylonHit(px, py) {
    if (G.stage !== 3 || G.boss) return false;
    const wx = G.cam + px;
    const cell = pylonCell(wx);
    const cx = pylonCx(cell);
    if (Math.abs(wx - cx) > 16) return false;
    const h = pylonHAt(cx);
    return h > 0 && py < h + 8;
  }
  ...
  const audio = {
    ...
    shoot(w) {
      this.ensure();
      if (w === 1) {
        this.beep(720, 0.045, 'square', 0.028, 280);
        this.beep(360, 0.06, 'triangle', 0.018, 140);
      } else if (w === 2) {
        this.beep(240, 0.08, 'sawtooth', 0.036, 720);
        this.beep(480, 0.07, 'triangle', 0.02, 160);
      } else if (w === 3) {
        this.beep(520, 0.05, 'triangle', 0.032, 880);
        this.beep(260, 0.07, 'sine', 0.02, 140);
      } else {
        this.beep(880, 0.038, 'square', 0.028, 1680);
      }
    },
    hit(combo) {
      this.ensure();
  ...
  function hud() {
    ...
    if (stageLabel) {
      if (G.mode === 'title') stageLabel.textContent = '林海';
    ...
    if (tagLabel) {
      tagLabel.textContent = isRail() ? '核轨' : '雷力';
    ...
    if (clawLabel) {
      clawLabel.textContent = '爪 ' + G.clawN;
      clawLabel.classList.toggle('off', G.clawN <= 0);
    }
    if (comboEl) {
    ...
    else if (G.mode === 'lose') setHint('R 重开 · 空格射击，Shift 换武器', 'warn');
    else if (G.mode === 'win') setHint('R 重开 · 雷核尽碎', 'hot');
    else if (G.lives === 1) setHint('最后一命 · 撞地、撞机、中弹都掉命', 'warn');
    else if (G.wep === 1) setHint('回射 · 打身后火梭与贴地炮台', '');
    else if (G.wep === 2) setHint('波刃 · 宽波穿群', 'hot');
    else if (G.wep === 3) setHint('猎弹 · 雷爪追踪锁核', 'hot');
    else setHint('空格连射 · Shift 换双射、回射、波刃、猎弹 · 雷爪挡弹', '');
    syncPips();
  }
  ...
  function showOverlay(kind, title, lead) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'TF03';
  ...
  function spawnWave(w) {
    ...
    const extra = isRail() && w.kind !== 'boss' && w.kind !== 'carrier' && w.kind !== 'elite' ? 2 : 0;
    const n = (w.n || 1) + extra;
    if (w.kind === 'v') {
      for (let i = 0; i < n; i++) {
        const k = i - (n - 1) * 0.5;
      ...
      for (let i = 0; i < n; i++) {
        spawnBeetle(baseX + i * 30, 86 + i * ((VH - 180) / Math.max(1, n - 1)));
      }
  ...
  function collectDrop(d) {
    if (G.clawN < CLAW_MAX) {
      G.clawN += 1;
      toast(G.clawN >= 2 ? '双爪' : '雷爪', false, true);
      floatText(d.x - G.cam, d.y - 10, G.clawN >= 2 ? '双爪' : '雷爪', GOLD);
      audio.claw();
    } else {
      const n = Math.round(SCORE.clawMax * G.mult);
      addScore(n);
      toast('MAX', false, true);
      floatText(d.x - G.cam, d.y - 10, '+' + n, GOLD);
      audio.up();
    }
    for (let i = 0; i < G.claws.length; i++) G.claws[i].glow = 1;
  ...
  function fire() {
    ...
    G.muzzle = 0.055;
    audio.shoot(G.wep);
    const wx = G.cam + G.px + 14;
    const rgb = wepRgb();
    if (G.wep === 0) {
      pushShot({ kind: 'twin', wx: wx, y: G.py - 5, vx: 640, vy: 0, dmg: w.dmg, r: 3.2, pierce: 0 });
      pushShot({ kind: 'twin', wx: wx, y: G.py + 5, vx: 640, vy: 0, dmg: w.dmg, r: 3.2, pierce: 0 });
      for (let i = 0; i < G.clawN; i++) {
        const c = G.claws[i];
        pushShot({
          kind: 'twin', wx: G.cam + c.x + 8, y: c.y, vx: 620, vy: 0,
          dmg: w.dmg * 0.85, r: 2.8, pierce: 0
        });
      }
    } else if (G.wep === 1) {
      pushShot({ kind: 'back', wx: wx, y: G.py, vx: 620, vy: 0, dmg: w.dmg, r: 3.3, pierce: 0 });
      pushShot({ kind: 'back', wx: G.cam + G.px - 10, y: G.py - 6, vx: -560, vy: 0, dmg: w.dmg, r: 3.3, pierce: 0 });
      pushShot({ kind: 'back', wx: G.cam + G.px - 10, y: G.py + 6, vx: -560, vy: 0, dmg: w.dmg, r: 3.3, pierce: 0 });
      for (let i = 0; i < G.clawN; i++) {
        const c = G.claws[i];
        pushShot({
          kind: 'back', wx: G.cam + c.x - 6, y: c.y, vx: -540, vy: 0,
          dmg: w.dmg * 0.9, r: 2.8, pierce: 0
        });
      }
    } else if (G.wep === 2) {
      pushShot({
        kind: 'wave', wx: wx, y: G.py, y0: G.py, vx: 500, vy: 0,
        amp: 24, ph: 0, dmg: w.dmg, r: 13, pierce: 1
      });
      for (let i = 0; i < G.clawN; i++) {
        const c = G.claws[i];
        pushShot({
  ...
  function killEnemy(e) {
    ...
    if (e.drop || (e.kind !== 'scout' && Math.random() < (e.kind === 'carrier' ? 1 : 0.16))) {
      dropClaw(e.x, e.y);
    }
    if (e.kind === 'boss') {
      audio.boom();
  ...
  function playerHit(why) {
    ...
    audio.death();
    if (G.clawN > 0) {
      dropClaw(G.cam + G.px + 10, G.py);
      if (G.clawN > 1) dropClaw(G.cam + G.px + 4, G.py + 16);
      G.clawN = 0;
    }
    G.eShots.length = 0;
    hud();
  ...
  function loseGame() {
    ...
    saveBest();
    audio.lose();
    showOverlay('lose', '舰毁了', '雷爪没护住。R 立刻重开，或换模式。');
    hud();
  }
  ...
  function winGame() {
    ...
    showOverlay(
      'win',
      isRail() ? '核轨通关' : '雷核尽碎',
      isRail() ? '密轨打穿。核心从行星散了。' : '三关打穿。把雷核从冥核打穿。'
    );
    hud();
  }
  ...
  function updateClaws(dt) {
    for (let i = 0; i < G.claws.length; i++) {
      const c = G.claws[i];
      c.ang += dt * 4.4;
      c.glow = Math.max(0, c.glow - dt * 1.8);
      const on = i < G.clawN && G.deadT <= 0;
      const tx = G.px + 6 + Math.cos(c.ang) * 26;
      const ty = G.py + Math.sin(c.ang) * 16;
      if (on) {
  ...
  function updateEShots(dt) {
    for (let i = G.eShots.length - 1; i >= 0; i--) {
      ...
      if (!playing()) continue;
      let blocked = false;
      if (!sh.fat && G.clawN > 0 && G.deadT <= 0) {
        for (let c = 0; c < G.clawN; c++) {
          const cl = G.claws[c];
          if (hypot(sh.x - (G.cam + cl.x), sh.y - cl.y) < CLAW_R + sh.r) {
            blocked = true;
            cl.glow = 1;
  ...
  function updateEnts(dt) {
    ...
    for (let i = G.ents.length - 1; i >= 0; i--) {
      ...
      if (e.kind === 'boss') {
        ...
        e.x += e.vx * dt;
        e.y += Math.sin(e.ph * 3 + e.ph) * 18 * dt;
        e.fireCd -= dt;
        ...
        if (e.x > pwx - 10 && !e.passed) {
          ...
          e.passed = true;
          e.vx = lerp(e.vx, isRail() ? 90 : 70, 0.1);
          e.fireCd -= dt;
          if (e.fireCd <= 0) {
      ...
      if (e.kind !== 'turret' && e.kind !== 'walker' && e.kind !== 'boss') {
  ...
  function updatePlayer(dt) {
    ...
    if (G.invuln > 0) {
      ...
      if (pylonHit(G.px, G.py)) G.py = Math.max(G.py, pylonHAt(G.cam + G.px) + 12);
    } else {
      if (G.py > gy - 8) playerHit('撞地');
      else if (pylonHit(G.px, G.py)) playerHit('擦柱');
    }
    G.bank = lerp(G.bank, clamp(((keys.r ? 1 : 0) - (keys.l ? 1 : 0)) * 0.4, -1, 1), 0.12);
    if (!REDUCE && playing()) {
      wisps.push({
        x: G.px - 12, y: G.py + rand(-2, 2),
  ...
  function update(dt) {
    ...
    let cruise = isRail() ? 164 : 110;
    if (G.boss) cruise = isRail() ? 42 : 22;
    G.cam += cruise * dt;
    G.clock += dt;
  ...
  function drawGround() {
    ...
    c.strokeStyle = theme === 'volcano'
      ? rgba(LAVA, 0.55)
      : theme === 'core' ? rgba(TEAL, 0.4) : rgba(LEAF, 0.5);
    c.lineWidth = Math.max(1, 1.6 * scale);
    c.stroke();

    if (theme === 'jungle') {
      for (let k = 0; k < 14; k++) {
        const cell = Math.floor(G.cam / 56) + k;
        const wx = cell * 56 + hash2(cell) * 20;
        const x = scrX(wx);
        if (x < -20 || x > VW + 20) continue;
        const g = groundY(wx);
        const h = 18 + hash2(cell + 3) * 36;
        c.fillStyle = rgba(LEAF, 0.35 + hash2(cell + 1) * 0.25);
        c.beginPath();
        c.moveTo(sx(x), sy(g));
        c.lineTo(sx(x - 10), sy(g - h));
        c.lineTo(sx(x + 10), sy(g - h * 0.7));
        c.closePath();
        c.fill();
      }
    } else if (theme === 'volcano') {
      for (let k = 0; k < 10; k++) {
        const cell = Math.floor(G.cam / 80) + k;
        const wx = cell * 80 + 20;
        const x = scrX(wx);
        const g = groundY(wx);
        const glow = 0.25 + 0.2 * Math.sin(G.t * 6 + cell);
        c.fillStyle = rgba(LAVA, glow);
        c.beginPath();
        c.ellipse(sx(x), sy(g - 2), 10 * scale, 4 * scale, 0, 0, TAU);
        c.fill();
      }
    }

    if (G.stage === 3 && !G.boss) {
      const cell0 = pylonCell(G.cam) - 1;
      for (let k = 0; k < 6; k++) {
        const cell = cell0 + k;
        const wx = pylonCx(cell);
        const h = pylonHAt(wx);
        if (h <= 0) continue;
        const x = scrX(wx);
        c.fillStyle = rgba(TEAL, 0.22);
        c.fillRect(sx(x - 10), sy(0), 20 * scale, h * scale);
        c.strokeStyle = rgba(CYN, 0.55);
        c.lineWidth = Math.max(1, 1.4 * scale);
        c.strokeRect(sx(x - 10), sy(0), 20 * scale, h * scale);
        c.fillStyle = rgba(GOLD, 0.7);
        c.fillRect(sx(x - 10), sy(h - 4), 20 * scale, 4 * scale);
      }
    }
  }

  function drawClaw(c, x, y, a, glow) {
    const s = scale;
    ctx.save();
    ctx.translate(sx(x), sy(y));
    ctx.rotate(c.ang);
    ctx.globalAlpha = a;
    ctx.fillStyle = rgba(glow > 0.2 ? WHT : GOLD, 0.95);
    ctx.beginPath();
    ctx.moveTo(7 * s, 0);
    ctx.lineTo(0, 5 * s);
    ctx.lineTo(-6 * s, 0);
  ...
  function drawBeetle(e, x) {
    const c = ctx;
    const s = scale;
    c.fillStyle = rgba(e.flash > 0 ? WHT : LEAF, 0.95);
    c.beginPath();
    c.ellipse(sx(x), sy(e.y), 12 * s, 8 * s, 0, 0, TAU);
    c.fill();
    c.fillStyle = rgba(GOLD, 0.8);
    c.fillRect(sx(x - 4), sy(e.y - 2), 8 * s, 4 * s);
    c.strokeStyle = rgba(LEAF, 0.7);
    c.lineWidth = Math.max(1, 1.2 * s);
    c.beginPath();
    c.moveTo(sx(x - 4), sy(e.y - 8));
  ...
  function drawTurret(e, x) {
    const c = ctx;
    const s = scale;
    c.fillStyle = rgba(e.flash > 0 ? WHT : LAVA, 0.9);
    c.fillRect(sx(x - 10), sy(e.y - 6), 20 * s, 14 * s);
    c.fillStyle = rgba(GOLD, 0.85);
    c.fillRect(sx(x - 3), sy(e.y - 10), 12 * s, 6 * s);
  }
  ...
  function drawWalker(e, x) {
    const c = ctx;
    const s = scale;
    c.fillStyle = rgba(e.flash > 0 ? WHT : [160, 90, 70], 0.95);
    c.fillRect(sx(x - 12), sy(e.y - 8), 24 * s, 12 * s);
    c.fillStyle = rgba(LAVA, 0.8);
    c.fillRect(sx(x - 4), sy(e.y - 4), 10 * s, 5 * s);
    c.fillStyle = rgba(DEEP, 0.8);
    c.fillRect(sx(x - 10), sy(e.y + 4), 6 * s, 8 * s);
    c.fillRect(sx(x + 4), sy(e.y + 4), 6 * s, 8 * s);
  }
  ...
  function drawBoss(e, x) {
    ...
    const rgb = e.flash > 0 ? WHT : (G.stage === 2 ? LAVA : G.stage === 3 ? GOLD : LEAF);
    c.save();
    ...
    c.closePath();
    c.stroke();
    c.fillStyle = rgba(G.stage === 2 ? LAVA : CYN, 0.22);
    c.fill();
    c.rotate(-e.ph * 0.35);
  ...
  function drawEnts() {
    for (let i = 0; i < G.ents.length; i++) {
      ...
      if (e.kind === 'scout') drawScout(e, x);
      else if (e.kind === 'beetle') drawBeetle(e, x);
      else if (e.kind === 'turret') drawTurret(e, x);
      else if (e.kind === 'dart') drawDart(e, x);
      else if (e.kind === 'walker') drawWalker(e, x);
  ...
  function drawShots() {
    ...
    for (let i = 0; i < G.shots.length; i++) {
      ...
      if (sh.kind === 'wave') {
        ...
        c.beginPath();
        c.moveTo(sx(x - 10), sy(sh.y - 10));
        c.quadraticCurveTo(sx(x), sy(sh.y), sx(x + 16), sy(sh.y + 8));
        c.stroke();
  ...
  function drawPlayer() {
    ...
    if (G.clawN > 0) {
      for (let i = 0; i < G.clawN; i++) {
        const cl = G.claws[i];
        drawClaw(cl, cl.x, cl.y, a, cl.glow);
      }
    }
  ...
  function goTitle() {
    resetRun('force');
    G.mode = 'title';
    showOverlay('title', '雷力', '换武器打穿行星。双射、回射、波刃、猎弹随手换。雷爪绕舰挡弹。撞地、撞机、中弹都掉命。');
    hud();
  }
  ...
  function onKey(e, down) {
    ...
    const swapKey = k === 'z' || k === 'Z' || k === 'Shift' || e.code === 'ShiftLeft' || e.code === 'ShiftRight';
    if (down && (k === 'ArrowLeft' || k === 'ArrowRight' || k === 'ArrowUp' || k === 'ArrowDown' || space || k === 'Enter' || swapKey)) {
      e.preventDefault();
    }
    if (!down) {
