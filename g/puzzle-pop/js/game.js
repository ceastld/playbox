...
(function () {
  ...
  var WARN_SHOTS = 3;
  var BEST_KEY = 'playbox-puzzle-pop-best';
  var MUTE_KEY = 'playbox-puzzle-pop-mute';
  var MODE_KEY = 'playbox-puzzle-pop-mode';
  var COMBO_WORD = ['', '', '连消', '连锁', '三连', '爆了', '狂潮', '炸翻', '通天'];
  ...
  var PUZZLES = [
    { name: '初连', sub: 'FIRST', rows: 3, colors: 3, dropEvery: 10, hole: 0, seed: 32011 },
    { name: '对珠', sub: 'PAIR', rows: 4, colors: 3, dropEvery: 9, hole: 0.04, seed: 32024 },
    { name: '垂挂', sub: 'HANG', rows: 4, colors: 4, dropEvery: 8, hole: 0.08, seed: 32037 },
    { name: '四色', sub: 'QUAD', rows: 5, colors: 4, dropEvery: 8, hole: 0.06, seed: 32048 },
    { name: '夹缝', sub: 'SLIT', rows: 5, colors: 5, dropEvery: 7, hole: 0.1, seed: 32055 },
    { name: '密织', sub: 'DENSE', rows: 6, colors: 5, dropEvery: 6, hole: 0.07, seed: 32066 },
    { name: '急坠', sub: 'FALL', rows: 6, colors: 6, dropEvery: 6, hole: 0.09, seed: 32077 },
    { name: '终幕', sub: 'FINALE', rows: 7, colors: 6, dropEvery: 5, hole: 0.07, seed: 32088 }
  ];
  ...
  function biggestGroup(grid) {
    ...
    for (r = 0; r < MAX_ROWS; r++) {
      for (c = 0; c < colsInRow(r); c++) {
        ...
        var i;
        for (i = 0; i < g.length; i++) seen[g[i].c + ',' + g[i].r] = 1;
  ...
  function puzzleAt(round) {
    if (round < PUZZLES.length) return PUZZLES[round];
  ...
  function endlessSpec(wave) {
    ...
    return {
      name: '第 ' + (w + 1) + ' 波',
      sub: 'WAVE',
      rows: Math.min(7, 5 + ((w / 2) | 0)),
      colors: Math.min(6, 4 + ((w / 2) | 0)),
      dropEvery: Math.max(3, 5 - ((w / 2) | 0)),
      hole: clamp(0.04 + w * 0.012, 0.04, 0.12)
    };
  }
  function fillBoard(rows, nColors, hole, rnd) {
    rnd = rnd || Math.random;
    var g = emptyGrid();
    var r, c, i, n, opts, pick, nb, col, group, tries, ok;
    for (r = 0; r < rows; r++) {
      for (c = 0; c < colsInRow(r); c++) {
        if (r >= 1 && rnd() < hole) continue;
        opts = [];
        n = neighbors(c, r);
        for (i = 0; i < n.length; i++) {
          nb = cellAt(g, n[i].c, n[i].r);
          if (nb >= 0 && opts.indexOf(nb) < 0) opts.push(nb);
        }
        pick = -1;
        tries = 10;
        while (tries--) {
          if (opts.length && rnd() < 0.46) col = opts[(rnd() * opts.length) | 0];
          else col = (rnd() * nColors) | 0;
          g[r][c] = col;
          group = findGroup(g, c, r);
          if (group.length < 3) {
            pick = col;
            break;
          }
  ...
  function hasPopMove(grid, nColors) {
    var cols = colorsOn(grid);
    if (!cols.length) return false;
    var r, c, i, n, color;
    var used = cols.length ? cols : [];
    if (!used.length) {
      for (i = 0; i < nColors; i++) used.push(i);
    }
    for (r = 0; r < MAX_ROWS; r++) {
      for (c = 0; c < colsInRow(r); c++) {
  ...
  function makeBoard(rows, nColors, hole, rnd) {
    var g;
    var guard = 28;
    var minCells = Math.max(8, rows * 3);
    do {
      g = fillBoard(rows, nColors, hole || 0, rnd);
      guard -= 1;
    } while (
      guard > 0 &&
      (countCells(g) < minCells || biggestGroup(g) >= 3 || !hasPopMove(g, nColors))
    );
    return g;
  }
  function makePuzzle(spec) {
    return makeBoard(spec.rows, spec.colors, spec.hole, mulberry(spec.seed));
  }
  ...
  function isDeadGrid(grid, dropY) {
    var r, c, y;
    for (r = 0; r < MAX_ROWS; r++) {
      for (c = 0; c < colsInRow(r); c++) {
        if (grid[r][c] < 0) continue;
  ...
  function runSelfTest() {
    ...
    var hang = emptyGrid();
    hang[0][3] = 0; hang[0][4] = 0; hang[1][3] = 0;
    hang[2][3] = 1; hang[2][4] = 1; hang[3][3] = 1;
    eq(findGroup(hang, 3, 0).length, 3, 'stem zeros');
    eq(findGroup(hang, 3, 2).length, 3, 'hang ones');
    var res = resolveAt(cloneGrid(hang), 3, 0);
    eq(res.popped.length, 3, 'pop stem');
    eq(res.fell.length, 3, 'hanging cluster falls');

    var miss = emptyGrid();
    miss[0][0] = 0; miss[0][2] = 1;
    var missRes = resolveAt(cloneGrid(miss), 0, 0);
    eq(missRes.popped.length, 0, 'pair does not pop');
    eq(missRes.fell.length, 0, 'no fall on miss');

    eq(isDeadGrid(g, 0) ? 0 : 1, 1, 'shallow alive');
    var dead = emptyGrid();
    dead[13][0] = 2;
    eq(isDeadGrid(dead, 0) ? 1 : 0, 1, 'row 13 dead');

    var i, spec, board, poppable, groups;
    for (i = 0; i < PUZZLES.length; i++) {
      spec = PUZZLES[i];
      board = makePuzzle(spec);
      groups = biggestGroup(board);
      poppable = hasPopMove(board, spec.colors);
      eq(groups < 3 ? 1 : 0, 1, 'puzzle ' + spec.name + ' no pre-pop');
      eq(countCells(board) >= spec.rows * 3 ? 1 : 0, 1, 'puzzle ' + spec.name + ' filled');
      eq(poppable ? 1 : 0, 1, 'puzzle ' + spec.name + ' has a pop');
      eq(floatersOf(board).length, 0, 'puzzle ' + spec.name + ' attached');
    }

    var end0 = endlessSpec(0);
    var end4 = endlessSpec(4);
    eq(end0.dropEvery > end4.dropEvery ? 1 : 0, 1, 'endless ceiling faster');
    eq(end4.dropEvery, 3, 'endless min drop');
    eq(end0.colors, 4, 'endless start colors');

    var landG = emptyGrid();
    landG[0][3] = 0; landG[0][4] = 0;
    var slot = simLand(landG, 0, 0);
    eq(slot ? 1 : 0, 1, 'straight shot lands');

    if (fail) {
      if (typeof console !== 'undefined') console.error('self-test failures', fail);
      if (typeof process !== 'undefined') process.exit(1);
    } else if (typeof console !== 'undefined') {
      console.log('puzzle-pop self-test ok');
    }
  }
  ...
  var ovPuzzle = document.getElementById('ov-puzzle');
  var ovEndless = document.getElementById('ov-endless');
  var btnMute = document.getElementById('btn-mute');
  var btnRetry = document.getElementById('btn-retry');
  var modePuzzleBtn = document.getElementById('mode-puzzle');
  ...
  var waveBox = document.getElementById('wave-box');
  ...
  var audio = {
    ...
    shoot: function () {
      this.ensure();
      this.noise(0.048, 0.055, 1500, 'bandpass');
      this.beep(480, 0.07, 'triangle', 0.042, 260);
    },
    bounce: function () {
      this.ensure();
      this.beep(920, 0.035, 'square', 0.024, 480);
    },
    dry: function () {
      this.ensure();
      this.noise(0.09, 0.055, 220, 'lowpass');
      this.beep(160, 0.13, 'sawtooth', 0.032, 64);
    },
    pop: function (n, combo) {
      this.ensure();
      var pitch = 240 + Math.min(n, 14) * 42 + Math.min(combo, 8) * 50;
      this.noise(0.07 + n * 0.006, 0.15 + Math.min(0.12, n * 0.01), 780 + n * 36, 'bandpass');
      this.beep(pitch, 0.1, 'square', 0.06, pitch * 1.85);
      this.beep(pitch * 0.5, 0.14, 'triangle', 0.048, pitch * 0.24);
      if (n >= 5) this.beep(pitch * 1.35, 0.16, 'sine', 0.04, pitch * 2.05, 0.03);
      if (combo >= 2) this.beep(740 + combo * 80, 0.12, 'sine', 0.036, 1480, 0.04);
    },
    fall: function (n) {
      this.ensure();
      var i, max = Math.min(n, 8);
      for (i = 0; i < max; i++) {
        this.beep(520 + i * 100, 0.07, 'sine', 0.04, 800 + i * 95, 0.035 + i * 0.04);
      }
    },
    warn: function () {
      this.ensure();
      this.beep(760, 0.07, 'square', 0.04, 400);
      this.beep(400, 0.12, 'sawtooth', 0.03, 200, 0.07);
    },
    ceiling: function () {
      this.ensure();
      this.noise(0.18, 0.11, 140, 'lowpass');
      this.beep(96, 0.24, 'triangle', 0.055, 42);
    },
    clear: function () {
      this.ensure();
  ...
  function loadBest() {
    ...
    try {
      ...
      if (raw.charAt(0) === '{') {
        ...
        G.bestP = Math.max(0, parseInt(o.puzzle, 10) || 0);
        G.bestE = Math.max(0, parseInt(o.endless, 10) || 0);
      } else {
        G.bestP = Math.max(0, parseInt(raw, 10) || 0);
      }
    } catch (e) { /* ignore */ }
  }
  ...
  function saveBest() {
    try {
      localStorage.setItem(BEST_KEY, JSON.stringify({ puzzle: G.bestP, endless: G.bestE }));
    } catch (e) { /* ignore */ }
  }

  function currentBest() {
    return G.kind === 'endless' ? G.bestE : G.bestP;
  }
  ...
  function playHint() {
    if (G.kind === 'endless') {
      setHint('无尽 · 压顶更快 · 清盘进入下一波', '');
    } else {
      setHint('清八关 · 三颗同色就爆 · 天花板会往下压', '');
    }
  }
  ...
  function currentSpec() {
    return G.kind === 'endless' ? endlessSpec(G.round) : puzzleAt(G.round);
  }
  ...
  function renderHud() {
    ...
    if (G.kind === 'endless') {
      stageLabel.textContent = '无尽 · ' + spec.name;
      waveBox.hidden = false;
      waveEl.textContent = String(G.round + 1);
    } else {
      stageLabel.textContent = '第 ' + (G.round + 1) + '/' + PUZZLES.length + ' · ' + spec.name;
      waveBox.hidden = true;
    }
  ...
  function setModeUi(kind) {
    document.body.classList.toggle('mode-endless', kind === 'endless');
    document.body.classList.toggle('mode-puzzle', kind !== 'endless');
    modePuzzleBtn.setAttribute('aria-pressed', kind !== 'endless' ? 'true' : 'false');
    modeEndlessBtn.setAttribute('aria-pressed', kind === 'endless' ? 'true' : 'false');
  }
  ...
  function setOverlay(kind) {
    ...
    if (kind === 'title') {
      ...
      ovTitle.textContent = '泡连';
      ovLead.textContent = '瞄准同色打进去。三颗相连就爆，悬空的整串砸下来。天花板会往下压。';
      ovOps.textContent = '方向键 / WASD 瞄准 · 空格发射 · 点按也可射 · R 重开 · M 静音';
      ovPuzzle.textContent = '清台';
      ovEndless.textContent = '无尽';
    } else if (kind === 'win') {
      maybeBest();
      ...
      ovOps.textContent = 'R 重开 · 弹层在下方不挡重开 · M 静音';
      ovPuzzle.textContent = '再清';
      ovEndless.textContent = '无尽';
    } else if (kind === 'lose') {
      maybeBest();
      ovKicker.textContent = 'FLOOR';
      ovTitle.textContent = '贴线了';
      ovLead.textContent = G.score + ' 分 · ' +
        (G.kind === 'endless' ? ('第 ' + (G.round + 1) + ' 波') : ('第 ' + (G.round + 1) + ' 关')) +
        ' · 炸掉 ' + G.pops + ' 次 · 坠落 ' + G.dropped + ' · 最高 ' + currentBest();
      ovOps.textContent = 'R 重开 · 弹层在下方不挡重开 · M 静音';
      ovPuzzle.textContent = G.kind === 'endless' ? '清台' : '再清';
      ovEndless.textContent = G.kind === 'endless' ? '再冲' : '无尽';
    }
    renderHud();
  }
  ...
  function buildBoard() {
    var spec = currentSpec();
    if (G.kind === 'puzzle') G.grid = makePuzzle(spec);
    else G.grid = makeBoard(spec.rows, spec.colors, spec.hole);
    G.drop = 0;
    G.dropY = 0;
  ...
  function isDead() {
    return isDeadGrid(G.grid, G.dropY);
  }
  ...
  function traceAim() {
    ...
    for (i = 0; i < 88; i++) {
      ...
      if (y - R <= ceil) break;
      if (findHitOn(G.grid, G.dropY, x, y)) break;
      if (y < 0) break;
  ...
  function dropDust() {
    ...
    emit(REDUCE ? 6 : 16, {
      x: (LEFT + RIGHT) * 0.5, y: ceilY + 8, j: 70,
      vx0: -80, vx1: 80, vy0: 30, vy1: 220,
      r0: 1.4, r1: 3.8, life: 0.4, rgb: PUR, g: 420
    });
    capArr(dust, 80);
  }
  ...
  function afterLand(c, r) {
    ...
    for (i = 0; i < group.length; i++) {
      ...
      burstAt(px, py, PAL[col] || pal, REDUCE ? 5 : 12);
    }
    ...
    spark(mid.x, mid.y, WHITE, REDUCE ? 4 : 14);
    ...
    for (i = 0; i < fell.length; i++) {
      ...
      emit(REDUCE ? 2 : 5, {
        ...
        r0: 1.2, r1: 2.8, life: 0.34, rgb: fpal.hi, g: 500
      });
    }
    ...
    word = COMBO_WORD[Math.min(COMBO_WORD.length - 1, G.combo)];
    if (G.combo >= 2 && word) {
      floatText(mid.x, mid.y + 22, word, GOLD, 18, true);
    }
    if (fell.length >= 3) {
      floatText(mid.x, mid.y + 40, '坠落 ' + fell.length, GOLD, 16, true);
  ...
  function onClear() {
    ...
    showToast('清盘', 'gold');
    if (G.kind === 'puzzle' && G.round >= PUZZLES.length - 1) {
      bumpScore(1500);
      floatText(VW * 0.5, VH * 0.36, '通关 +1500', GOLD, 22, true);
      endGame('win');
      return;
    }
    G.phase = 'clear';
  ...
  function nextRound() {
    ...
    G.lock = 0.12;
    var spec = currentSpec();
    showToast((G.kind === 'endless' ? spec.name : ('第 ' + (G.round + 1) + ' 关 · ' + spec.name)), 'gold');
    setHint(G.kind === 'endless' ? '压顶更快了' : (spec.name + ' · 压顶更快了'), 'hot');
    renderHud();
    kick(0, 0.4, 4);
    screenFlash(MAG, 0.2);
  }
  ...
  function landShot(shot, hit) {
    var slot = placeShotOn(G.grid, G.dropY, shot.x, shot.y, hit);
    G.shot = null;
    if (!slot) {
      audio.dry();
      afterShotSettled();
  ...
  function stepShot(dt) {
    ...
    for (i = 0; i < sub; i++) {
      ...
      hit = findHitOn(G.grid, G.dropY, s.x, s.y);
      if (hit) {
        landShot(s, hit);
        return;
      }
  ...
  function endGame(kind) {
    ...
    setHint('R 重开 · 弹层不挡重开', kind === 'win' ? 'hot' : 'warn');
  }

  function startPlay(kind) {
    audio.start();
    G.kind = kind === 'endless' ? 'endless' : 'puzzle';
    G.mode = 'play';
    G.round = 0;
    applyStage();
    G.score = 0;
    ...
    renderHud();
    var spec = currentSpec();
    showToast(G.kind === 'endless' ? '无尽 · 压顶更快' : spec.name, 'gold');
    canvas.focus();
    playHint();
  }
  ...
  function retry() {
    audio.ensure();
    startPlay(G.kind === 'endless' ? 'endless' : 'puzzle');
  }

  function switchKind(kind) {
    audio.ensure();
    if (G.mode === 'play' && overlayKind === 'none' && G.kind === kind) return;
    startPlay(kind);
  }
  ...
  function draw() {
    ...
    var bg = ctx.createRadialGradient(W * 0.5, H * 0.16, 12, W * 0.5, H * 0.42, Math.max(W, H) * 0.72);
    bg.addColorStop(0, 'rgba(255,61,184,0.1)');
    ...
    ctx.fillStyle = 'rgba(16, 6, 18, 0.94)';
    ctx.beginPath();
    ...
    for (i = 0; i < pops.length; i++) {
      ...
      drawBall(p.x, p.y, R * (1 + k * 0.55), pal, 1 + k * 0.4, 1 + k * 0.4, 0.85, 16);
    }
    ...
    if (G.mode === 'play' && !G.shot && overlayKind === 'none') {
      var path = traceAim();
      pal = PAL[G.current] || PAL[0];
    ...
    barrel.addColorStop(0, '#2a0a1c');
    barrel.addColorStop(0.45, '#5c1a44');
    barrel.addColorStop(1, '#ff9ad4');
    ...
    for (i = 0; i < dust.length; i++) {
      p = dust[i];
      a = p.life / p.max;
      ctx.fillStyle = rgba(p.rgb, 0.55 * a);
      ctx.beginPath();
      ctx.arc(p.x, p.y, p.r * a, 0, TAU);
      ctx.fill();
    }
  ...
  function stepFx(dt) {
    ...
    for (key in G.pulse) {
      if (!Object.prototype.hasOwnProperty.call(G.pulse, key)) continue;
      p = G.pulse[key];
    ...
    for (i = falls.length - 1; i >= 0; i--) {
      ...
      if (!p.scored && p.t >= p.delay) {
        ...
        emit(REDUCE ? 2 : 6, {
          x: p.x, y: p.y, j: 8,
          vx0: -90, vx1: 90, vy0: -40, vy1: 80,
          r0: 1.2, r1: 3.2, life: 0.28, rgb: GOLD, g: 600
        });
      }
  ...
  function update(dt) {
    ...
    if (keys.u) {
      if (G.aim > 0) G.aim = Math.max(0, G.aim - AIM_SPD * 0.7 * dt);
      else if (G.aim < 0) G.aim = Math.min(0, G.aim + AIM_SPD * 0.7 * dt);
    }

    if (G.phase === 'fly') stepShot(dt);
    if (G.phase === 'resolve' && G.lock <= 0) afterShotSettled();
    if (G.phase === 'clear' && G.lock <= 0) nextRound();
    if (G.phase === 'ceiling') {
      G.dropT += dt;
      var t = clamp(G.dropT / (REDUCE ? 0.12 : 0.28), 0, 1);
      G.dropY = lerp(G.dropFrom, G.dropTo, easeInOut(t));
      if (t >= 1) finishCeiling();
    }
  ...
  function onKey(ev, down) {
    ...
    if (code === 'ArrowLeft' || code === 'KeyA') {
      ...
      if (down) {
        audio.ensure();
        if (overlayKind === 'title') startPlay(G.kind === 'endless' ? 'endless' : 'puzzle');
        else if (overlayKind !== 'none' || G.mode === 'end') retry();
        else fire();
      }
    } else if (down && (code === 'KeyR')) {
      ev.preventDefault();
      retry();
    } else if (down && (code === 'KeyM')) {
      ev.preventDefault();
      audio.ensure();
  ...
  window.addEventListener('keydown', function (ev) {
    if (ev.repeat && (ev.code === 'Space' || ev.code === 'Enter' || ev.code === 'KeyR' || ev.code === 'KeyM')) {
      ev.preventDefault();
      return;
    }
    onKey(ev, true);
  ...
  canvas.addEventListener('pointermove', function (ev) {
    var w = toWorld(ev.clientX, ev.clientY);
    if (G.mode === 'play' && overlayKind === 'none') aimAt(w.x, w.y);
    else if (overlayKind === 'title') aimAt(w.x, w.y);
  });
  ...
  modePuzzleBtn.addEventListener('click', function () { switchKind('puzzle'); });
  modeEndlessBtn.addEventListener('click', function () { switchKind('endless'); });
  ovPuzzle.addEventListener('click', function () { startPlay('puzzle'); });
  ovEndless.addEventListener('click', function () { startPlay('endless'); });
  ...
  try {
    ...
    if (savedMode === 'endless') G.kind = 'endless';
  ...
  renderHud();
  setOverlay('title');
  setHint('瞄准同色 · 三颗起爆 · 悬空坠落 · 天花板会压下来', '');
  requestAnimationFrame(frame);
})();
