...
(function () {
  ...
  const BOMB_CAP = 6;
  const PWR_MAX = 3;
  const BEST_KEY = 'playbox-sonic-wings-best';
  const MUTE_KEY = 'playbox-sonic-wings-mute';
  const OPS = '方向 / WASD 飞 · 空格机炮 · Shift / Z 空牙 · R 重开 · M 静音';
  ...
  const AMB = [255, 200, 48];
  const HOT = [245, 180, 40];
  const CYN = [78, 232, 255];
  const WHT = [255, 246, 228];
  const PNK = [255, 154, 180];
  const RED = [255, 86, 86];
  const TEAL = [42, 180, 168];
  const ICE = [180, 220, 255];
  const ROCK = [196, 92, 42];
  const INK = [22, 18, 14];

  const PWR_ROMAN = ['', 'Ⅱ', 'Ⅲ', 'MAX'];
  const DROP_GLYPH = { pwr: '火', bomb: '牙' };
  ...
  function plySpd() {
    return (isChaos() ? 318 : 276) + G.pwr * 10;
  }
  function scrollSpd() {
    if (hasBig()) return isChaos() ? 34 : 26;
    const base = isChaos() ? 118 : 86;
    const rush = G.combo >= 8 ? 16 : G.combo >= 4 ? 8 : 0;
    return base + rush + (G.stage - 1) * (isChaos() ? 10 : 8);
  }
  function hash2(n) {
    n |= 0;
  ...
  function shotCap() {
    return isChaos() ? 150 : 96;
  }
  function hpMul() {
    return isChaos() ? 1.22 : 1;
  }
  function biome() {
  ...
  const audio = {
    ...
    shoot() {
      this.ensure();
      this.beep(720 + G.pwr * 55, 0.048, 'square', 0.03, 1680);
    },
    hit(combo) {
      this.ensure();
      const lift = 1 + Math.min(0.5, combo * 0.04);
      this.noise(0.038, 0.034, 1200);
      this.beep(560 * lift, 0.068, 'square', 0.044, 1020 * lift);
    },
    boom(big) {
      this.ensure();
    ...
    bomb() {
      ...
      this.beep(78, 0.46, 'sawtooth', 0.072, 36);
      this.beep(640, 0.34, 'sine', 0.052, 1680);
    },
    pow() {
      this.ensure();
    ...
    rumble() {
      this.ensure();
      this.noise(0.028, 0.014, 180);
      this.beep(72, 0.034, 'sawtooth', 0.014, 48);
    }
  };
  ...
  function syncHud() {
    ...
    if (tagLabel) {
      tagLabel.textContent = isChaos() ? '乱空' : '空牙';
    ...
    if (bombLabel) {
      bombLabel.textContent = '牙 ×' + G.bombs;
    ...
    else if (G.mode === 'win') setHint('空牙尽破 · R 再来一局', 'hot');
    else if (G.lives === 1) setHint('最后一命 · Shift 空牙清弹', 'warn');
    else setHint('空格机炮 · Shift 空牙 · 吃 火/牙 · 撞机扣命', '');
    syncPips();
  }
  ...
  function showOverlay(kind, title, lead) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'SWNG';
    ...
    if (ovModes) {
      ...
      else if (kind === 'win') ovModes.textContent = isChaos() ? '换模式' : '乱空';
    }
  }
  ...
  function spawnStream(dir) {
    ...
    const n = (isChaos() ? 8 : 6) + (Math.random() * 4) | 0;
    for (let i = 0; i < n; i++) {
  ...
  function spawnSams() {
    ...
    for (let i = 0; i < n; i++) {
      ...
      spawnSam(clamp(x, 48, VW - 48), -26 - i * 18);
    }
  }
  ...
  function fireWave(w) {
    ...
    else if (w.kind === 'helo') spawnHelo();
    else if (w.kind === 'stealth') {
      spawnStealth();
      if (isChaos()) spawnStealth();
    } else if (w.kind === 'sams') spawnSams();
    else if (w.kind === 'dishes') spawnDishes();
    else if (w.kind === 'tanker') spawnTanker();
  ...
  function fire() {
    ...
    G.muzzle = 0.05;
    G.fireCd = 0.110 - lv * 0.014;
    const spd = -720;
    const rgb = lv >= 3 ? CYN : lv >= 1 ? GOLD : WHT;
    function gun(ox, oy, vx, vy) {
      addShot({
        x: x + ox, y: y + oy,
        vx: vx || 0,
        vy: vy == null ? spd : vy,
    ...
    function mis(ox, oy, vx) {
      addShot({
        x: x + ox, y: y + oy,
        vx: vx || 0,
        vy: -520,
        r: 4.2, rgb: AMB, dmg: 2,
        ang: Math.atan2(-520, vx || 0),
        kind: 'mis'
      });
    }
    if (lv <= 0) {
  ...
  function doBomb() {
    ...
    hitStop(0.078);
    kick(7.4);
    screenFlash(CYN, 0.62);
    popSpark(G.player.x, G.player.y, CYN, 48);
    rings.push({ x: VW * 0.5, y: VH * 0.62, t: 0, rgb: GOLD, r: 28 });
    emit(28, {
      x: G.player.x, y: G.player.y, j: 70,
      vx0: -180, vx1: 180, vy0: -360, vy1: -40,
      life: 0.55, r0: 1.4, r1: 4.2, rgb: CYN, g: 40
    });
    toast('空牙', false, true);
    if (stageEl && !REDUCE) {
      stageEl.classList.remove('bomb');
  ...
  function fangHit(en) {
    const ly = Math.abs(en.y - G.fangY);
    if (ly > 88) return false;
    const lx = Math.abs(en.x - (G.fangX - 72));
    const rx = Math.abs(en.x - (G.fangX + 72));
    return lx < 58 || rx < 58;
  }

  function tickFang(dt) {
    if (G.fangT <= 0) return;
    G.fangT -= dt;
    G.fangY -= 980 * dt;
    G.fangTick -= dt;
    if (G.fangTick <= 0) {
      ...
      for (let i = 0; i < G.ents.length; i++) {
        const en = G.ents[i];
        if (en.hp <= 0) continue;
        if (fangHit(en)) hurtEnt(en, 2, en.x, en.y);
      }
      G.eShots.length = 0;
    }
    if (G.fangT <= 0) G.fangT = 0;
  }
  ...
  function killEnt(en) {
    ...
    else if (en.type === 'stealth' && Math.random() < 0.2) spawnPow(en.x, en.y, nextDropKind());
    if (en.type === 'boss') {
      G.stageClearT = 2.05;
  ...
  function pickPow(p) {
    ...
    juice(p.x, p.y, p.kind === 'bomb' ? CYN : GOLD, 1.15);
    audio.pow();
    hitStop(0.038);
    floatText(p.x, p.y, DROP_GLYPH[p.kind] || '火', p.kind === 'bomb' ? CYN : GOLD, true);
    syncHud();
  }
  ...
  function winGame() {
    ...
    saveBest();
    audio.win();
    showOverlay('win', '空牙尽破', (isChaos() ? '乱空通关' : '三关打穿') + ' · 分数 ' + G.score);
    syncHud();
  }
  ...
  function chaosThink(dt) {
    ...
    if (G.spawnT <= 0 && livingAir() < 14) {
      ...
      else if (roll < 0.86) spawnHelo();
      else spawnDishes();
      G.spawnT = 2.4 + rand(0, 0.8);
    }
  }

  function bossFire(en, chaos) {
    ...
    if (en.type === 'mid') {
      if (stg === 1) {
        aimShot(en.x, en.y + 10, chaos ? 196 : 164, HOT);
        eShot(en.x - 16, en.y + 8, -40, 170, ROCK);
        eShot(en.x + 16, en.y + 8, 40, 170, ROCK);
        if (mid) ringShot(en.x, en.y + 4, chaos ? 10 : 8, 128, en.spin, GOLD, 3.0);
        en.fireCd = low ? 0.42 : 0.7;
      } else if (stg === 2) {
        ringShot(en.x, en.y + 6, chaos ? 12 : 9, 134, en.t * 1.6, TEAL, 3.05);
        if (mid) aimShot(en.x, en.y + 12, 188, HOT);
        en.fireCd = low ? 0.4 : 0.64;
      } else {
        ringShot(en.x, en.y + 4, chaos ? 14 : 10, 142, en.spin, ICE, 3.1);
        aimShot(en.x - 18, en.y + 8, 176, MAG);
        aimShot(en.x + 18, en.y + 8, 176, MAG);
        en.fireCd = low ? 0.36 : 0.58;
      }
      if (chaos) en.fireCd *= 0.76;
      return;
    }
    if (stg === 1) {
      eShot(en.x - 28, en.y + 12, -50, 196, RED);
      eShot(en.x, en.y + 16, 0, 210, MAG);
      eShot(en.x + 28, en.y + 12, 50, 196, RED);
      if (mid) ringShot(en.x, en.y + 6, chaos ? 12 : 9, 138, en.spin, PNK, 3.1);
      if (low) {
        for (let k = -3; k <= 3; k++) eShot(en.x + k * 14, en.y + 20, k * 42, 208, MAG);
      }
      en.fireCd = low ? 0.28 : mid ? 0.42 : 0.56;
    } else if (stg === 2) {
      ringShot(en.x, en.y + 8, chaos ? 14 : 11, 146, en.spin, MAG, 3.15);
      if (mid) {
        ringShot(en.x, en.y + 8, chaos ? 10 : 8, 118, -en.spin * 1.4, GOLD, 3.0);
      ...
      ringShot(en.x, en.y + 6, chaos ? 16 : 12, 152, en.spin, MAG, 3.2);
      ringShot(en.x, en.y + 6, chaos ? 10 : 8, 108, -en.spin * 0.7, CYN, 2.8);
      if (mid) {
        aimShot(en.x - 20, en.y + 14, 210, PNK);
        aimShot(en.x + 20, en.y + 14, 210, PNK);
      }
      if (low) {
        ringShot(en.x, en.y, chaos ? 18 : 14, 168, en.t * 3.2, GOLD, 3.4);
      }
  ...
  function updateEnts(dt) {
    ...
    for (let i = G.ents.length - 1; i >= 0; i--) {
      ...
      if (en.ground && en.type !== 'mid' && en.type !== 'boss') {
        ...
        en.x += en.vx * dt;
        en.y += en.vy * dt;
      } else if (en.type === 'dish') {
        en.phase += dt * 3.2;
        en.x += Math.sin(en.phase) * 42 * dt + en.vx * dt;
        en.y += en.vy * dt;
      ...
      if (playing && en.y > -10 && en.y < VH + 10) {
        en.fireCd -= dt;
        if (en.type === 'sam' && en.y > 8 && en.y < VH - 70 && en.fireCd > 0 && en.fireCd <= 0.22) {
          en.lock = en.fireCd;
        }
        if (en.fireCd <= 0) {
          if (en.type === 'jet' && en.y > 18 && en.y < VH - 80) {
            eShot(en.x, en.y + 10, 0, chaos ? 198 : 172, MAG);
            if (chaos && Math.random() < 0.45) aimShot(en.x, en.y + 8, 168, PNK);
            en.fireCd = (chaos ? 1.3 : 2.3) + rand(0, 0.55);
          } else if (en.type === 'helo' && en.y > 20 && en.y < VH - 70) {
            aimShot(en.x, en.y + 8, chaos ? 196 : 164, TEAL);
            eShot(en.x - 10, en.y + 6, -28, 150, HOT);
            eShot(en.x + 10, en.y + 6, 28, 150, HOT);
            en.fireCd = chaos ? 0.72 : 1.08;
          } else if (en.type === 'stealth' && en.y > 20 && en.y < VH - 70) {
            eShot(en.x - 10, en.y + 10, -36, 176, ICE);
            eShot(en.x, en.y + 12, 0, 196, ICE);
            eShot(en.x + 10, en.y + 10, 36, 176, ICE);
            if (chaos) aimShot(en.x, en.y + 8, 186, CYN);
            en.fireCd = chaos ? 0.68 : 0.98;
          } else if (en.type === 'sam' && en.y > 8 && en.y < VH - 70) {
            aimShot(en.x, en.y, chaos ? 218 : 176, GOLD);
            if (chaos) {
              eShot(en.x - 8, en.y + 4, -42, 164, HOT);
              eShot(en.x + 8, en.y + 4, 42, 164, HOT);
            }
            en.fireCd = (chaos ? 0.62 : 1.02) + rand(0, 0.24);
          } else if (en.type === 'dish' && en.y > 18 && en.y < VH - 80) {
            eShot(en.x, en.y + 8, Math.sin(en.phase) * 40, chaos ? 168 : 142, AMB);
            en.fireCd = (chaos ? 1.1 : 1.7) + rand(0, 0.4);
          } else if (en.type === 'mid' || en.type === 'boss') {
            if (en.y > 70) bossFire(en, chaos);
            else en.fireCd = 0.4;
          } else {
            en.fireCd = 2;
          }
  ...
  function updateShots(dt) {
    ...
    for (let i = G.shots.length - 1; i >= 0; i--) {
      const s = G.shots[i];
      if (s.kind === 'mis') {
        const tgt = nearestEnt(s.x, s.y);
        if (tgt) {
          const dx = tgt.x - s.x;
          const dy = tgt.y - s.y;
          const len = hypot(dx, dy) || 1;
          s.vx = lerp(s.vx, dx / len * 420, 1 - Math.exp(-dt * 4.2));
          s.vy = lerp(s.vy, dy / len * 420, 1 - Math.exp(-dt * 4.2));
  ...
  function updateWorld(dt) {
    ...
    for (let i = 0; i < marks.length; i++) {
      ...
      if (isl.y - isl.h > VH + 30) {
        ...
        isl.w = 32 + hash2((G.scroll * 0.1) | 0) * 58;
        isl.h = 40 + hash2((G.scroll * 0.13) | 0) * 44;
  ...
  function drawMark(isl, bio) {
    ...
    ctx.save();
    if (bio === 'polar') {
      ctx.fillStyle = 'rgba(28, 44, 62, 0.9)';
      ctx.beginPath();
      ctx.ellipse(x, y, w * 0.42, h * 0.22, 0, 0, TAU);
      ctx.fill();
      ctx.strokeStyle = rgba(ICE, 0.45);
      ctx.lineWidth = 1.4 * scale;
      ctx.beginPath();
      ctx.arc(x, y - h * 0.18, w * 0.22, 0.2, Math.PI - 0.2);
      ctx.stroke();
      ctx.fillStyle = rgba(CYN, 0.35);
      ctx.fillRect(x - 2 * scale, y - h * 0.48, 4 * scale, h * 0.32);
    } else if (bio === 'bay') {
      ctx.fillStyle = 'rgba(12, 28, 32, 0.92)';
      ctx.fillRect(x - w * 0.08, y - h * 0.45, w * 0.08, h * 0.7);
      ctx.fillRect(x + w * 0.18, y - h * 0.45, w * 0.08, h * 0.7);
      ctx.fillStyle = rgba(TEAL, 0.5);
      ctx.fillRect(x - w * 0.28, y - h * 0.52, w * 0.62, h * 0.1);
    } else {
      ctx.fillStyle = 'rgba(42, 22, 10, 0.92)';
      ctx.beginPath();
  ...
  function drawWorld() {
    ...
    if (bio === 'polar') {
      ...
      g.addColorStop(0.45, '#081018');
      g.addColorStop(1, '#06080c');
    } else if (bio === 'bay') {
      g.addColorStop(0, '#061418');
      g.addColorStop(0.5, '#041014');
      g.addColorStop(1, '#040c10');
    } else {
    ...
    if (bio === 'polar' && !REDUCE) {
      const ag = ctx.createLinearGradient(sx(0), sy(0), sx(VW), sy(160));
      ag.addColorStop(0, 'rgba(78, 232, 255, 0.08)');
      ag.addColorStop(0.5, 'rgba(255, 227, 107, 0.05)');
      ag.addColorStop(1, 'rgba(78, 232, 255, 0.07)');
      ctx.fillStyle = ag;
      ctx.fillRect(sx(0), sy(0), VW * scale, 180 * scale);
    }

    ctx.save();
    ctx.strokeStyle = bio === 'polar'
      ? 'rgba(80, 140, 180, 0.22)'
      : bio === 'bay'
        ? 'rgba(30, 90, 100, 0.28)'
        : 'rgba(90, 52, 16, 0.3)';
    ctx.lineWidth = 1;
  ...
  function drawPlane(x, y, bank) {
    ...
    ctx.fillStyle = rgba(CYN, 0.55 * flicker);
    ctx.beginPath();
    ctx.moveTo(-5, 12);
    ctx.lineTo(-2.2, 12);
    ctx.lineTo(-3.6, 26);
    ...
    ctx.closePath();
    ctx.fill();
    ctx.fillStyle = rgba(GOLD, 0.45 * flicker);
    ctx.beginPath();
    ctx.moveTo(-4.2, 12);
    ctx.lineTo(-3, 20);
    ctx.lineTo(-2.6, 12);
  ...
  function drawEnt(en) {
    ...
    if (en.type === 'jet') {
      ...
      ctx.fillRect(-10, 1, 20, 2.2);
      ctx.fillStyle = rgba(CYN, 0.55);
      ctx.fillRect(-2.4, 7, 1.6, 6);
      ctx.fillRect(0.8, 7, 1.6, 6);
    } else if (en.type === 'dive') {
      ...
      ctx.lineWidth = 1.2;
      ctx.beginPath();
      ctx.arc(0, 2, 11, 0.2, Math.PI - 0.2);
      ctx.stroke();
  ...
  function drawFang() {
    if (G.fangT <= 0) return;
    const a = clamp(G.fangT / 0.72, 0, 1);
    const y = G.fangY;
    ctx.save();
    ctx.fillStyle = rgba(CYN, 0.1 * a);
    ctx.fillRect(sx(0), sy(0), VW * scale, VH * scale);
    function fang(fx) {
      ctx.save();
      ctx.translate(sx(fx), sy(y));
      ctx.scale(scale, scale);
      ctx.shadowColor = rgba(CYN, 0.9);
      ctx.shadowBlur = 22;
      ctx.fillStyle = rgba(CYN, 0.92 * a);
      ctx.beginPath();
      ctx.moveTo(0, -70);
      ctx.lineTo(16, 10);
      ctx.lineTo(5, 18);
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = rgba(GOLD, 0.88 * a);
      ctx.beginPath();
      ctx.moveTo(0, -62);
      ctx.lineTo(6, 8);
      ctx.lineTo(0, 70);
  ...
  function drawPows() {
    for (let i = 0; i < G.pows.length; i++) {
      ...
      const rgb = p.kind === 'bomb' ? CYN : GOLD;
      ctx.save();
  ...
  function startGame(kind) {
    ...
    syncHud();
    audio.start();
    toast(isChaos() ? '乱空 · 弹更密' : '空牙 · 第 1 关', false, true);
  }
  ...
  function goTitle() {
    ...
    showOverlay(
      'title',
      '空牙',
      '开喷气机向上打。空格机炮，Shift 空牙。国关先中破再关底。撞机扣命。'
    );
    syncHud();
  }
  ...
  if (ovModes) {
    ovModes.addEventListener('click', function () {
      ...
      else if (G.mode === 'win' && isChaos()) goTitle();
