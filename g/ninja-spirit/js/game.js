...
(function () {
  ...
  const SLASH_T = 0.2;
  const SLASH_CD = 0.08;
  const SLASH_RANGE = 38;
  const THROW_SPD = 340;
  const SPIRIT_MAX = 2;
  const INVULN = 1.18;
  const DIE_T = 0.86;
  const BEST_KEY = 'playbox-ninja-spirit-best';
  const MUTE_KEY = 'playbox-ninja-spirit-mute';
  const AUTO_SPEED_KEY = 'playbox-ninja-spirit-auto-speed';
  const SPEED_LABELS = ['', '慢', '中', '快', '极快'];
  const AUTO_SCALE = [1, 0.52, 0.78, 1, 3.4];
  const OPS = '方向键 / D 走 · ↑ 跳 · 空格斩并丢镖 · A 自动 · R 重开 · M 静音';
  ...
  const BONE = [214, 196, 176];
  const ONI = [168, 42, 52];
  const NIGHT = [48, 18, 36];

  const SCORE = {
    imp: 100, skull: 150, bat: 150, oni: 200, brute: 250,
    boss: 4000, stage: 1500, spirit: 500, star: 80, shot: 50
  };

  const STAGES = [
    {
      name: '赤林', boss: '鬼面', w: 2200, hp: 16, theme: 'woods',
      ...
      drops: [
        [180, GY, 'star'], [300, MY, 'heart'], [760, GY, 'spirit'],
        [1340, MY, 'star'], [1720, GY, 'heart']
      ],
      ents: [
        [260, GY, 'imp', 40, 500],
        [420, GY, 'oni', 80, 500],
        [300, MY, 'skull', 220, 380],
        [720, GY, 'oni', 600, 940],
        [840, GY, 'imp', 610, 940],
        [880, HY, 'bat', 820, 950],
        [1180, GY, 'oni', 1070, 1430],
        [1360, GY, 'brute', 1070, 1440],
        [1380, MY, 'skull', 1280, 1450],
        [1720, GY, 'imp', 1570, 2140],
        [1900, GY, 'oni', 1580, 2160],
        [1840, MY, 'bat', 1760, 1920]
      ]
    },
    {
      name: '魂桥', boss: '天狗', w: 2520, hp: 22, theme: 'bridge',
      ...
      drops: [
        [120, GY, 'heart'], [280, HY, 'spirit'], [760, MY, 'star'],
        [1180, GY, 'heart'], [1720, HY, 'spirit'], [2220, MY, 'heart']
      ],
      ents: [
        [220, GY, 'oni', 20, 410],
        [220, MY, 'skull', 140, 280],
        [280, HY, 'bat', 220, 350],
        [640, GY, 'brute', 540, 800],
        [760, GY, 'oni', 540, 800],
        [820, MY, 'skull', 700, 870],
        [1080, GY, 'imp', 920, 1230],
        [1220, HY, 'bat', 1220, 1370],
        [1480, GY, 'oni', 1370, 1690],
        [1560, MY, 'skull', 1520, 1690],
        [1740, HY, 'bat', 1660, 1800],
        [1960, GY, 'brute', 1850, 2380],
        [2100, GY, 'oni', 1850, 2400],
        [2180, MY, 'skull', 2160, 2320]
      ]
    },
    {
      name: '冥殿', boss: '阎王', w: 2840, hp: 28, theme: 'hell',
      ...
      drops: [
        [160, GY, 'heart'], [240, HY, 'spirit'], [780, MY, 'star'],
        [1220, HY, 'heart'], [1880, GY, 'spirit'], [2220, MY, 'heart'], [2540, GY, 'star']
      ],
      ents: [
        [240, GY, 'oni', 20, 390],
        [200, MY, 'skull', 120, 260],
        [280, HY, 'bat', 220, 350],
        [620, GY, 'brute', 520, 780],
        [740, GY, 'oni', 520, 780],
        [820, MY, 'skull', 660, 830],
        [1080, GY, 'imp', 920, 1200],
        [1160, HY, 'bat', 1120, 1270],
        [1180, GY, 'oni', 920, 1200],
        [1380, MY, 'brute', 1286, 1454],
        [1540, GY, 'oni', 1370, 1650],
        [1620, HY, 'skull', 1560, 1700],
        [1980, GY, 'oni', 1810, 2130],
        [2060, MY, 'bat', 1980, 2130],
        [2440, GY, 'brute', 2330, 2780],
        [2480, HY, 'skull', 2440, 2580],
        [2560, GY, 'oni', 2340, 2800]
      ]
    }
  ];
  ...
  function airDist() {
    return WALK * (2 * JUMP_V / GRAV);
  }
  function spdMul(tide, stage) {
    return (tide ? 1.26 : 1) * (1 + Math.max(0, stage - 1) * 0.08);
  }
  ...
  function selfCheck() {
    ...
    if (SPIRIT_MAX !== 2) throw new Error('spirit max');
    if (SLASH_RANGE < 28) throw new Error('slash short');
    if (THROW_SPD < 200) throw new Error('throw slow');
    if (BEST_KEY !== 'playbox-ninja-spirit-best') throw new Error('best key');
    if (AUTO_SPEED_KEY !== 'playbox-ninja-spirit-auto-speed') throw new Error('auto key');
  ...
  const spiritLabel = document.getElementById('spirit-label');
  ...
  const audio = {
    ...
    throwStar() {
      this.ensure();
      this.beep(880, 0.05, 'square', 0.036, 1480);
      this.beep(1320, 0.07, 'triangle', 0.028, 720);
    },
    spirit() {
      this.ensure();
  ...
  function syncHud() {
    ...
    if (stageLabel) {
      stageLabel.textContent = (isTide() ? '妖潮 ' : '') + spec.name;
      stageLabel.classList.toggle('hot', G.stage >= 3 || (G.boss && G.boss.active));
    }
    if (tagLabel) {
      tagLabel.textContent = isTide() ? '妖潮' : '斩关';
      tagLabel.classList.toggle('warn', isTide());
      tagLabel.classList.toggle('hot', !isTide() && G.stage >= 3);
    }
    if (spiritLabel) {
      spiritLabel.textContent = '魂 ' + G.spirits;
      spiritLabel.className = 'spirit' + (G.spirits <= 0 ? ' off' : G.spirits >= SPIRIT_MAX ? ' hot' : '');
    }
    ...
    if (autoOn) {
      ...
      else if (G.boss && G.boss.active && !G.boss.dead) setHint('托管中 · 守护者 ' + spec.boss + ' · A 停下', 'hot');
      else setHint(isTide() ? '托管中 · 妖潮斩镖 · A 停下' : '托管中 · 斩关斩镖 · A 停下', 'hot');
    } else if (G.mode === 'title') setHint(OPS, '');
    else if (G.mode === 'lose') setHint('R 重开 · 空格近斩远镖 · 魂分身同步出手', 'warn');
    else if (G.mode === 'win') setHint('守护者已灭 · R 再来一局', 'hot');
    else if (G.lives === 1) setHint('最后一命 · ↑跳 · 空格斩并丢镖', 'warn');
    else if (G.boss && G.boss.active && !G.boss.dead) setHint('守护者 · ' + spec.boss, 'hot');
    else setHint('走跳过崖 · 空格近斩远镖 · 拾魂得分身 · A 自动', '');
    syncPips();
    syncModes();
  }
  ...
  function showOverlay(kind, title, lead) {
    ...
    if (ovKicker) {
      ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'SPIR';
    }
    ...
    if (ovMenu) ovMenu.textContent = kind === 'win' ? '妖潮' : '换模式';
  }
  ...
  function kick(mag, cls) {
    ...
    kickTok += 1;
    stageEl.classList.remove('die', 'boom', 'hit', 'thump', 'pickup', 'win-flash', 'spirit');
  ...
  function hpOf(kind) {
    if (kind === 'brute') return 3;
    if (kind === 'oni') return 2;
    return 1;
  }

  function makeEnt(x, y, kind, a, b) {
    const hp = hpOf(kind);
    const fly = kind === 'skull' || kind === 'bat';
    return {
      id: uid++,
      x: x, y: y, vx: 0, vy: 0, face: -1,
      ...
      w: kind === 'brute' ? 20 : kind === 'oni' ? 16 : kind === 'skull' ? 14 : kind === 'bat' ? 16 : 12,
      h: kind === 'brute' ? 28 : kind === 'oni' ? 24 : kind === 'skull' ? 14 : kind === 'bat' ? 12 : 14
    };
  }

  function makeBoss(spec) {
    const hp = (spec.hp * (isTide() ? 1.18 : 1)) | 0;
    const fly = spec.boss === '天狗';
    return {
      id: uid++,
      ...
      hitN: 0, w: fly ? 40 : 38, h: fly ? 32 : 48,
      base: fly ? HY + 20 : GY
    };
  }
  ...
  function loadStage(n, attract) {
    ...
    if (isTide() && !attract) {
      for (i = 0; i < spec.ents.length; i++) {
        if (i % 3 !== 0) continue;
        const e = spec.ents[i];
        if (e[2] === 'brute') continue;
        G.ents.push(makeEnt(e[0] + 36, e[1], e[2] === 'oni' ? 'imp' : e[2], e[3], e[4]));
      }
    }
    G.pickups = [];
    if (!attract) {
  ...
  function takePickup(u) {
    ...
    audio.ping();
    kick(2.2, 'pickup');
    screenFlash(u.kind === 'spirit' ? CYN : GOLD, 0.22);
    popSpark(u.x, u.y, u.kind === 'spirit' ? CYN : GOLD, 16);
    if (u.kind === 'heart') {
      G.hp = Math.min(maxHp(), G.hp + 1);
      toast('生命 +1', false, true);
      floatText(u.x, u.y - 8, '+1', MAG, false);
    } else if (u.kind === 'spirit') {
      if (G.spirits < SPIRIT_MAX) {
        G.spirits += 1;
        seedGhosts(false);
        audio.spirit();
        toast(G.spirits === 1 ? '魂分身' : '双魂齐出', false, true);
        addScore(SCORE.spirit * G.mult);
        kick(2.8, 'spirit');
        screenFlash(CYN, 0.36);
      } else {
        G.hp = Math.min(maxHp(), G.hp + 1);
        toast('魂已满 · 生命 +1', false, true);
      }
    } else {
    ...
    emit(10, {
      ...
      life: 0.32, r0: 1.2, r1: 3, rgb: u.kind === 'spirit' ? CYN : GOLD
    });
    syncHud();
  }
  ...
  function maybeDrop(e) {
    if (!playing()) return;
    const r = Math.random();
    const tank = e.kind === 'brute' || e.kind === 'oni';
    if (tank && r < 0.18) spawnPickup(e.x, e.y - 12, 'spirit', false);
  ...
  function spiritOffset(i, face) {
    if (i === 0) return { dx: -28 * face, dy: -8 };
    return { dx: -20 * face, dy: -38 };
  }

  function seedGhosts(snap) {
    const p = G.player;
    const n = G.spirits;
    while (G.ghosts.length < n) {
      G.ghosts.push({ x: p.x, y: p.y, face: p.face, flash: 0, run: 0 });
    }
    while (G.ghosts.length > n) G.ghosts.pop();
    if (snap) {
      let i, off;
      for (i = 0; i < G.ghosts.length; i++) {
        off = spiritOffset(i, p.face);
        G.ghosts[i].x = p.x + off.dx;
        G.ghosts[i].y = p.y + off.dy;
        G.ghosts[i].face = p.face;
      }
    }
  }
  ...
  function updateSpirits(dt) {
    ...
    for (i = 0; i < G.ghosts.length; i++) {
      ...
      off = spiritOffset(i, p.face);
      tx = p.x + off.dx;
      ty = p.y + off.dy + Math.sin(G.t * 5.2 + i * 1.7) * 3.5;
      g.x = lerp(g.x, tx, 1 - Math.pow(0.0004, dt));
      g.y = lerp(g.y, ty, 1 - Math.pow(0.0008, dt));
      g.face = p.face;
      if (g.flash > 0) g.flash -= dt;
      g.run = p.run;
      if (G.spirits > 0 && Math.random() < 0.18) {
        emit(1, {
          x: g.x, y: g.y - 12, j: 4,
          vx0: -20, vx1: 20, vy0: -40, vy1: -4,
          life: 0.22, r0: 0.8, r1: 1.8, rgb: CYN, g: -40
        });
      }
    }
    if (G.spiritFlash > 0) G.spiritFlash -= dt;
  }

  function slashBoxAt(x, y, face, air) {
    const h = air ? 34 : 30;
    const by = y - (air ? 32 : 28);
    if (face > 0) return { x: x + 2, y: by, w: SLASH_RANGE, h: h };
    return { x: x - 2 - SLASH_RANGE, y: by, w: SLASH_RANGE, h: h };
  }

  function attackOrigins() {
    const p = G.player;
    const list = [{ x: p.x, y: p.y, face: p.face, air: !p.grounded }];
    let i, g;
    for (i = 0; i < G.ghosts.length; i++) {
      g = G.ghosts[i];
      list.push({ x: g.x, y: g.y, face: g.face, air: true });
    }
    return list;
  }

  function spawnThrow(x, y, face) {
    spawnShot({
      x: x + face * 10,
      y: y - 14,
      vx: face * THROW_SPD,
  ...
  function tryAttack() {
    ...
    const p = G.player;
    G.slash = { t: SLASH_T, hit: {}, air: !p.grounded };
    p.pose = SLASH_T;
    audio.slash();
    audio.throwStar();
    G.spiritFlash = 0.18;
    const origins = attackOrigins();
    let i, o, tip;
    for (i = 0; i < origins.length; i++) {
      o = origins[i];
      spawnThrow(o.x, o.y, o.face);
      tip = o.x + o.face * 18;
      emit(6, {
        x: tip, y: o.y - 14, j: 6,
        vx0: o.face * 40, vx1: o.face * 220, vy0: -80, vy1: 50,
        life: 0.16, r0: 1, r1: 2.4, rgb: i ? CYN : GOLD, g: 40
      });
      if (i > 0 && G.ghosts[i - 1]) G.ghosts[i - 1].flash = 0.2;
    }
    if (G.spirits > 0) {
      audio.spirit();
  ...
  function resolveBoxHits(box, hitMap) {
    let i, e, s;
    for (i = 0; i < G.ents.length; i++) {
      e = G.ents[i];
      if (e.dead) continue;
      if (hitMap[e.id]) continue;
      if (overlap(box.x, box.y, box.w, box.h, e.x - e.w * 0.5, e.y - e.h, e.w, e.h)) {
        hitMap[e.id] = 1;
        hitEnemy(e, 1);
      }
    }
    if (G.boss && !G.boss.dead && G.boss.active && !hitMap[G.boss.id]) {
      const b = G.boss;
  ...
  function resolveSlashHits() {
    ...
    for (i = 0; i < origins.length; i++) {
      o = origins[i];
      box = slashBoxAt(o.x, o.y, o.face, o.air);
      resolveBoxHits(box, sl.hit);
    }
  }
  ...
  function updateSlash(dt) {
    ...
    if (Math.random() < 0.55) {
      ...
      for (i = 0; i < origins.length; i++) {
        o = origins[i];
        emit(1, {
          x: o.x + o.face * (12 + SLASH_RANGE * 0.55), y: o.y - 14, j: 4,
          vx0: -30, vx1: 30, vy0: -40, vy1: 20,
          life: 0.12, r0: 0.8, r1: 1.8, rgb: i ? CYN : GOLD, g: 0
        });
      }
    }
    resolveSlashHits();
  ...
  function nextStage() {
    ...
    const keep = { hp: G.hp, spirits: G.spirits, score: G.score, lives: G.lives };
    loadStage(G.stage + 1, false);
    G.hp = keep.hp;
    G.spirits = keep.spirits;
    G.score = keep.score;
    G.lives = keep.lives;
    G.invuln = 1.05;
    seedGhosts(true);
  ...
  function goTitle() {
    ...
    G.deadT = 0;
    loadStage(1, true);
    showOverlay('title', '忍魂', '往右斩妖。空格近斩远镖，魂分身一起出手。小鬼、断崖、关底守护者。生命打空丢一条命。');
    syncHud();
  }
  ...
  function groundAt(x) {
    ...
    for (i = 0; i < G.plats.length; i++) {
      p = G.plats[i];
      if (!p.base) continue;
      if (x >= p.x - 2 && x <= p.x + p.w + 2) return GY;
    }
    return null;
  }
  ...
  function nextPit(x, dir) {
    ...
    const bases = [];
    let i, p, cur = null, curI = -1;
    for (i = 0; i < G.plats.length; i++) {
      p = G.plats[i];
      if (p.base) bases.push(p);
    }
    bases.sort(function (a, b) { return a.x - b.x; });
    ...
    if (dir >= 0) {
      out.edge = cur.x + cur.w;
      out.dist = out.edge - x;
      if (curI + 1 < bases.length) {
        out.next = bases[curI + 1];
        out.width = out.next.x - out.edge;
      }
    } else {
      out.edge = cur.x;
      out.dist = x - out.edge;
      if (curI > 0) {
        out.next = bases[curI - 1];
        out.width = out.edge - (out.next.x + out.next.w);
      }
    }
    return out;
  }
  ...
  function autoIsFly(e) {
    return !!(e && (e.kind === 'skull' || e.kind === 'bat' || e.kind === '天狗'));
  }
  ...
  function autoVertHit(e, p) {
    ...
    const ey0 = e.y - e.h;
    const ey1 = e.y;
    function overlapAt(py, air) {
      const by = py - (air ? 32 : 28);
      const h = air ? 34 : 30;
      return by < ey1 && by + h > ey0;
    }
    if (overlapAt(p.y, !p.grounded)) return 'now';
    if (overlapAt(p.y - jumpHeight() * 0.72, true)) return 'jump';
    return null;
  }
  ...
  function autoShotThreat() {
    ...
    for (i = 0; i < G.shots.length; i++) {
      ...
      if (s.vx === 0) {
        if (Math.abs(s.x - p.x) < 40) return s;
        continue;
      }
      t = (p.x - s.x) / s.vx;
  ...
  function autoPick() {
    ...
    let i, e, d, u, dx, pri;
    if (G.hp <= 3 || (isTide() && G.hp <= 2)) {
      for (i = 0; i < G.pickups.length; i++) {
        u = G.pickups[i];
        if (u.taken || u.kind !== 'heart') continue;
        dx = u.x - p.x;
        if (dx < -70) continue;
        d = hypot(dx, u.y - (p.y - 12));
        if (d > 240) continue;
        pri = (G.hp <= 2 ? 900 : 640) - d * 0.5;
    ...
    if (G.spirits < SPIRIT_MAX) {
      for (i = 0; i < G.pickups.length; i++) {
        u = G.pickups[i];
        if (u.taken || u.kind !== 'spirit') continue;
        dx = u.x - p.x;
        if (dx < -50 || dx > 220) continue;
        d = hypot(dx, u.y - (p.y - 12));
        if (d > 200) continue;
        if (autoPitBetween(p.x, u.x) && autoFloor(u.y + 12) === GY) continue;
        consider(u.x, u.y + 12, 700 - d * 0.45, 'spirit');
      }
    }
    for (i = 0; i < G.ents.length; i++) {
      e = G.ents[i];
      ...
      d = hypot(dx, e.y - p.y);
      if (autoIsFly(e)) {
        if (dx > -24 && dx < 120 && autoVertHit(e, p)) {
          consider(e.x, e.y, 740 - Math.abs(dx) * 0.4, 'fly');
        }
        continue;
      }
    ...
    if (b && b.active && !b.dead) {
      dx = b.x - p.x;
      const side = dx >= 0 ? -1 : 1;
      consider(b.x + side * 32, b.y, 1450, 'boss');
    }
  ...
  function autoThink() {
    ...
    if (moved < 0.7 && p.grounded && !G.slash) autoStuck += STEP;
    else autoStuck = Math.max(0, autoStuck - STEP * 2);
    autoLastX = p.x;
    autoLastY = p.y;
    if (autoBackT > 0) autoBackT -= STEP;

    const reach = SLASH_RANGE;
    const onPlat = p.grounded && p.y < GY - 24;
    const plat = onPlat ? platCovering(p.x, autoFloor(p.y)) : null;
    const goal = autoPick();
    let seekX = goal.x;
    let wantJump = false;
    let wantFire = false;
    ...
    for (i = 0; i < G.ents.length; i++) {
      ...
      dx = e.x - p.x;
      adx = Math.abs(dx);
      if (adx > reach + 200) continue;
      if (dx < 0 && autoPitBetween(p.x, e.x)) continue;
      vert = autoVertHit(e, p);
      if (autoIsFly(e)) {
        if (adx > reach + 28) continue;
        if (!vert) continue;
      } else if (!autoSameBand(e.y, p.y) || Math.abs(e.y - p.y) > 46) {
        continue;
      }
    ...
    if (shot && G.invuln <= 0) wantJump = true;
    ...
    if (close) {
      ...
      const flyer = autoIsFly(close) || close.kind === '天狗';
      const now = vert === 'now' || (flyer && vert);
      if (now && adx <= reach + 8) {
        wantFire = true;
        if (flyer && !onPlat && p.grounded && adx < 50) wantJump = true;
        if (adx < 14 && !flyer && close.kind !== '鬼面' && close.kind !== '阎王') {
          seekX = p.x + (dx > 0 ? -18 : 18);
        } else if (adx > 34) seekX = close.x;
        else {
          seekX = p.x;
          hold = true;
        }
      } else if (flyer && vert === 'jump' && adx <= reach + 24 && !onPlat) {
        wantJump = true;
        seekX = close.x;
        if (adx <= reach + 10) wantFire = true;
      } else if (!flyer && autoSameBand(close.y, p.y) && Math.abs(close.y - p.y) < 42) {
        seekX = close.x;
        if (adx < 200 && adx > 20 && (dx > 0 || adx < 50)) wantFire = true;
      } else if (close.kind === '鬼面' || close.kind === '阎王' || close.kind === '天狗') {
        seekX = close.x + (dx >= 0 ? -32 : 32);
        if (adx < 80) wantFire = true;
        if (close.kind === '天狗' && adx < 70 && p.grounded) wantJump = true;
      }
    }

    if ((goal.kind === 'heart' || goal.kind === 'spirit' || goal.kind === 'fly')
      && goal.y < p.y - 20 && !onPlat) {
      const band = autoFloor(goal.y);
      const up = platCovering(goal.x, band) || platCovering(p.x, MY);
      if (up && Math.abs(p.x - (up.x + up.w * 0.5)) < up.w * 0.5 + 10) {
        wantJump = true;
        seekX = up.x + Math.min(up.w * 0.55, Math.max(12, goal.x - up.x));
      }
    }

    if (onPlat && plat) {
      const grabHere = (goal.kind === 'heart' || goal.kind === 'spirit')
        && goal.x >= plat.x - 4 && goal.x <= plat.x + plat.w + 4;
      const fightHere = close && autoVertHit(close, p) === 'now'
        && Math.abs(close.x - p.x) <= reach + 8;
      if (!grabHere) {
        const end = plat.x + plat.w;
        const fallT = Math.sqrt(2 * Math.max(24, plat.y ? (GY - plat.y) : (GY - MY)) / GRAV);
    ...
    if (!onPlat && pit.width > 14 && pit.dist < 110 && !nearEnd) {
      const cover = platCovering(pit.edge + 8, MY) || platCovering(p.x, MY)
        || (pit.next ? platCovering(pit.next.x + 8, MY) : null);
      const tooWide = pit.width > airDist() * 0.88;
      if (tooWide && cover && p.x < pit.edge + 4) {
        if (p.x >= cover.x - 6 && p.x <= cover.x + cover.w + 6) {
          wantJump = true;
          seekX = cover.x + cover.w * 0.55;
          autoSteer(seekX);
        } else if (cover.x > p.x) {
          autoSteer(cover.x + 20);
        }
        if (pit.dist < 28) wantFire = false;
      } else {
        if (pit.dist < 100) wantFire = false;
        if (dir < 0) {
          autoSteer(p.x + 40);
        } else {
          const air = airDist();
          const lead = clamp(air - pit.width - 8, 10, 36);
          if (pit.dist < lead) {
            wantJump = true;
            pitJump = true;
            wantFire = false;
    ...
    autoIn.fire = wantFire && !G.fireEdge;
  }
  ...
  function toggleAuto() {
    ...
    if (autoOn) {
      audio.ensure();
      if (G.mode === 'title') startGame(G.kind || 'cut');
    }
    syncHud();
  }
  ...
  function enemyShoot(x, y, dx, dy, spd, kind) {
    ...
    spawnShot({
      ...
      vx: (dx / len) * spd,
      vy: (dy / len) * spd,
      from: 'e', kind: kind || 'bone',
      life: 2.4, rgb: kind === 'mask' || kind === 'fire' ? HOT : kind === 'wind' ? CYN : BONE,
      grav: 0,
      spin: 0, dead: false
    });
  }
  ...
  function updateEnt(e, dt) {
    ...
    if (e.kind === 'skull' || e.kind === 'bat') {
      e.x += (e.face || -1) * (e.kind === 'bat' ? 64 : 54) * mul * dt;
      if (e.x < e.a) e.face = 1;
      if (e.x > e.b) e.face = -1;
      e.y = e.base + Math.sin(e.t * (e.kind === 'bat' ? 4.1 : 3.2)) * (e.kind === 'bat' ? 22 : 16);
      e.fire -= dt;
      if (live() && onScreen(e.x, e.y, 20) && e.fire <= 0 && e.kind === 'skull') {
        enemyShoot(e.x, e.y + 4, G.player.x - e.x, (G.player.y - 14) - e.y, 140 * mul, 'bone');
        e.fire = (isTide() ? 1.2 : 1.8) + rand(0, 0.5);
      }
      return;
    }
    if (!onScreen(e.x, e.y, 90)) return;
    const walk = (e.kind === 'brute' ? 24 : e.kind === 'oni' ? 40 : 46) * mul;
    if (e.x < e.a) e.face = 1;
    if (e.x > e.b) e.face = -1;
    ...
    if (e.kind === 'oni' && live()) {
      ...
      if (e.fire <= 0 && onScreen(e.x, e.y, 10)) {
        enemyShoot(e.x + e.face * 12, e.y - 16, e.face, 0, 160 * mul, 'bone');
        e.fire = (isTide() ? 1.25 : 1.85) + rand(0, 0.4);
      }
    }
  }
  ...
  function updateBoss(dt) {
      ...
      if (b.state === 'idle') {
        ...
        if (b.fire <= 0) {
          if (Math.random() < 0.42) {
            ...
            enemyShoot(b.x, b.y + 8, p.x - b.x, (GY - 18) - (b.y + 8), 200 * mul, 'wind');
      ...
      if (b.state === 'idle') {
        ...
        if (b.fire <= 0) {
          if (Math.random() < 0.38) {
            ...
            enemyShoot(b.x + b.face * 16, GY - 18, b.face, 0, 210 * mul, 'fire');
            enemyShoot(b.x + b.face * 16, GY - 48, b.face, 0, 190 * mul, 'wind');
            b.fire = (isTide() ? 0.7 : 1.0) + rand(0, 0.25);
          }
        }
      } else {
        b.fire -= dt;
    ...
    if (b.fire <= 0) {
      if (Math.random() < 0.5) {
        enemyShoot(b.x + b.face * 18, GY - 18, b.face, 0, 180 * mul, 'mask');
        landDust(b.x, b.y, 1.2);
      } else {
        landDust(b.x, b.y, 1.5);
        kick(3, 'thump');
        audio.boom();
        enemyShoot(b.x + b.face * 20, b.y - 10, b.face, 0, 150 * mul, 'bone');
      }
      b.fire = (isTide() ? 0.75 : 1.15) + rand(0, 0.35);
    }
  }
    ...
    for (i = 0; i < G.shots.length; i++) {
      ...
      if (s.from === 'e' && playing() && G.deadT <= 0 && G.invuln <= 0) {
        ...
        if (overlap(s.x - 5, s.y - 5, 10, 10, pb.x, pb.y, pb.w, pb.h)) {
          s.dead = true;
          hurt(s.x, 1, s.kind === 'fire' || s.kind === 'mask' ? 'boss' : 'hit');
        }
      }
      if (s.from === 'p' && !s.dead) {
        if (!s.hit) s.hit = {};
  ...
  function drawSky() {
    ...
    if (spec.theme === 'hell') {
      g.addColorStop(0, '#220610');
      g.addColorStop(0.55, '#14040a');
      g.addColorStop(1, '#0a0206');
    } else if (spec.theme === 'bridge') {
      g.addColorStop(0, '#0c1020');
      g.addColorStop(0.5, '#100814');
      g.addColorStop(1, '#0c0610');
    } else {
      g.addColorStop(0, '#1c0810');
      g.addColorStop(0.5, '#14060c');
      g.addColorStop(1, '#0e0408');
    }
    ctx.fillStyle = g;
    ...
    ctx.fillStyle = rgba(spec.theme === 'bridge' ? CYN : HOT, isTide() ? 0.28 : 0.5);
    ctx.beginPath();
    ctx.arc(mx, my, 22 * scale, 0, TAU);
    ctx.fill();
    ctx.fillStyle = rgba(spec.theme === 'bridge' ? WHT : MAG, 0.18);
    ctx.beginPath();
  ...
  function drawBackdrop() {
    ...
    for (i = -2; i < 28; i++) {
      ...
      if (spec.theme === 'woods') {
        ctx.fillStyle = i % 2 ? '#1a0c10' : '#12080c';
        ctx.fillRect(x, base - h, w, h + 40 * scale);
        ctx.fillStyle = rgba(HOT, 0.35);
        ctx.fillRect(x, base - h, w, 3 * scale);
        ctx.fillStyle = rgba(LEAF, 0.18);
        ctx.beginPath();
        ctx.ellipse(x + w * 0.5, base - h - 6 * scale, 10 * scale, 8 * scale, 0, 0, TAU);
        ctx.fill();
      } else if (spec.theme === 'bridge') {
        ctx.fillStyle = i % 2 ? '#141018' : '#100c16';
        ctx.fillRect(x, base - h * 0.7, w + 8 * scale, h * 0.7 + 40 * scale);
        ctx.strokeStyle = rgba(CYN, 0.28);
        ctx.lineWidth = 1.4 * scale;
        ctx.strokeRect(x + 2 * scale, base - h * 0.7, w + 4 * scale, 16 * scale);
        ctx.fillStyle = rgba(GOLD, 0.16);
        ctx.fillRect(x + 6 * scale, base - h * 0.7 + 4 * scale, 5 * scale, 8 * scale);
      } else {
        ctx.fillStyle = i % 2 ? '#1c0810' : '#14060c';
        ...
        ctx.strokeRect(x + 2 * scale, base - h + 8 * scale, w + 6 * scale, 14 * scale);
      }
    }
    for (i = 0; i < mist.length; i++) {
      const m = mist[i];
      ctx.fillStyle = rgba(spec.theme === 'bridge' ? CYN : HOT2, m.a);
      ctx.beginPath();
  ...
  function drawPickup(u) {
    ...
    else if (u.kind === 'spirit') { rgb = CYN; mark = '魂'; }
  ...
  function drawShot(s) {
    ...
    if (s.kind === 'star') {
      ...
      ctx.fill();
    } else if (s.kind === 'fire' || s.kind === 'mask') {
      ...
      ctx.fillStyle = rgba(s.rgb || BONE, 0.95);
  ...
  function drawSlashArc(x, y, face, air, rgb, alpha) {
    const sl = G.slash;
    if (!sl) return;
    const s = scale;
    ...
    const cx = sx(x);
    const cy = sy(y - (air ? 16 : 14));
    ctx.strokeStyle = rgba(rgb, 0.95 * alpha);
    ctx.lineWidth = 3.2 * s;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.arc(cx, cy, SLASH_RANGE * 0.72 * s, ang - 0.9, ang + 0.35, false);
    ctx.stroke();
    ctx.strokeStyle = rgba(HOT, 0.7 * alpha);
    ctx.lineWidth = 1.5 * s;
    ctx.beginPath();
    ctx.arc(cx, cy, SLASH_RANGE * 0.58 * s, ang - 0.7, ang + 0.2, false);
    ctx.stroke();
    ctx.fillStyle = rgba(WHT, 0.9 * alpha);
    const tx = cx + Math.cos(ang) * SLASH_RANGE * 0.72 * s;
    const ty = cy + Math.sin(ang) * SLASH_RANGE * 0.72 * s;
    ctx.beginPath();
    ctx.arc(tx, ty, 2.4 * s, 0, TAU);
    ctx.fill();
  }

  function drawNinja(x, y, face, opt) {
    const s = scale;
    const sq = opt.squash || 1;
    const rgb = opt.rgb || HOT;
    ...
    ctx.fillRect(-6 * s, -bodyH * s - 5 * s, 12 * s, bodyH * s);
    ctx.fillStyle = rgba(opt.sash || GOLD, 0.9);
    ctx.fillRect(-6 * s, -bodyH * s + 1 * s, 12 * s, 2.2 * s);
    ctx.fillStyle = rgba(CYN, 0.45);
    ctx.fillRect(-6 * s, -bodyH * s - 5 * s, 12 * s, 2 * s);
    const hy = -(bodyH + 10);
    ctx.fillStyle = rgba(opt.skin || SKIN, 0.95);
    ctx.beginPath();
    ctx.ellipse(0, hy * s, 5 * s, 5.2 * s, 0, 0, TAU);
    ctx.fill();
    ctx.fillStyle = rgba(rgb, 0.95);
    ctx.fillRect(-5 * s, (hy - 3) * s, 10 * s, 3.2 * s);
    ctx.fillStyle = '#1a0810';
    ctx.fillRect(1.4 * s, (hy - 1) * s, 2.8 * s, 1.5 * s);
    const swinging = !!G.slash || opt.pose > 0;
    ctx.strokeStyle = rgba(WHT, 0.9);
    ctx.lineWidth = 1.7 * s;
    ctx.beginPath();
    ctx.moveTo(3 * s, -(bodyH + 1) * s);
    ctx.lineTo((swinging ? 13 : 8) * s, -(bodyH + (swinging ? 3 : 1)) * s);
    ctx.stroke();
    ctx.restore();
  }
  ...
  function drawHero(p, opt) {
    ...
    for (i = 0; i < G.ghosts.length; i++) {
      g = G.ghosts[i];
      flash = g.flash > 0 || G.spiritFlash > 0;
      alpha = flash ? 0.92 : 0.42;
      drawNinja(g.x, g.y, g.face, {
        run: g.run, grounded: false, squash: 1,
        rgb: flash ? WHT : CYN, sash: MAG, skin: CYN,
        alpha: alpha, glow: flash, pose: p.pose
      });
      if (G.slash) drawSlashArc(g.x, g.y, g.face, true, CYN, flash ? 1 : 0.7);
    }
    drawNinja(p.x, p.y, p.face, {
      run: opt.run, grounded: opt.grounded, squash: opt.squash,
      rgb: HOT, sash: GOLD, skin: SKIN, alpha: 1, glow: false, pose: p.pose
    });
    if (G.slash) drawSlashArc(p.x, p.y, p.face, !p.grounded, GOLD, 1);
    else {
      const s = scale;
      ctx.save();
      ...
      ctx.lineWidth = 2.2 * s;
      ctx.beginPath();
      ctx.arc(10 * s, -12 * s, 5.2 * s, -0.4, 2.4, false);
      ctx.stroke();
      ctx.restore();
    }
  }
  ...
  function drawEnt(e) {
    ...
    if (e.kind === 'skull') {
      ...
      ctx.fillRect(-3.4 * s, -10 * s, 2.4 * s, 2.2 * s);
      ctx.fillRect(1 * s, -10 * s, 2.4 * s, 2.2 * s);
      ctx.fillStyle = rgba(NIGHT, 0.8);
      ctx.fillRect(-2 * s, -5 * s, 4 * s, 1.6 * s);
    } else if (e.kind === 'bat') {
      const flap = Math.sin(e.t * 14) * 5;
      ctx.fillStyle = rgba(NIGHT, 0.95);
      ctx.fillRect(-5 * s, -8 * s, 10 * s, 7 * s);
      ctx.fillStyle = rgba(MAG, 0.7);
      ctx.beginPath();
      ctx.moveTo(-4 * s, -8 * s);
      ctx.lineTo(-14 * s, (-10 - flap) * s);
      ctx.lineTo(-4 * s, -4 * s);
      ctx.fill();
      ctx.beginPath();
      ctx.moveTo(4 * s, -8 * s);
      ...
      ctx.fillRect(-2 * s, -7 * s, 1.6 * s, 1.4 * s);
      ctx.fillRect(0.6 * s, -7 * s, 1.6 * s, 1.4 * s);
    } else if (e.kind === 'oni') {
      ctx.fillStyle = rgba(ONI, 0.95);
      ctx.fillRect(-7 * s, -18 * s, 14 * s, 14 * s);
      ctx.fillStyle = rgba(GOLD, 0.7);
      ctx.fillRect(-7 * s, -18 * s, 14 * s, 3 * s);
      ctx.fillStyle = rgba(ONI, 0.95);
      ctx.beginPath();
      ctx.arc(0, -24 * s, 6 * s, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(HOT, 0.9);
      ctx.fillRect(-5 * s, -30 * s, 2.2 * s, 6 * s);
      ctx.fillRect(2.8 * s, -30 * s, 2.2 * s, 6 * s);
      ctx.fillStyle = '#1a0810';
      ctx.fillRect(-3 * s, -25 * s, 2.2 * s, 1.6 * s);
      ctx.fillRect(1 * s, -25 * s, 2.2 * s, 1.6 * s);
      ctx.fillStyle = rgba(BONE, 0.9);
      ctx.fillRect(6 * s, -16 * s, 9 * s, 3 * s);
      ctx.strokeStyle = rgba(ONI, 0.9);
      ctx.lineWidth = 2 * s;
      ctx.beginPath();
      ctx.moveTo(-4 * s, -4 * s);
      ctx.lineTo(-6 * s, 0);
      ...
      ctx.fillStyle = rgba(HOT, 0.85);
      ctx.fillRect(-6 * s, -32 * s, 2.4 * s, 2 * s);
      ctx.fillRect(3.4 * s, -32 * s, 2.4 * s, 2 * s);
      ctx.fillStyle = rgba(BONE, 0.85);
      ctx.fillRect(8 * s, -18 * s, 8 * s, 4 * s);
    } else {
      const wob = Math.sin(e.t * 8) * 1.4 * s;
      ctx.fillStyle = rgba(ONI, 0.95);
      ctx.fillRect(-6 * s + wob, -12 * s, 12 * s, 12 * s);
      ctx.fillStyle = rgba(HOT, 0.8);
      ctx.beginPath();
      ctx.arc(wob, -16 * s, 5 * s, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(GOLD, 0.7);
      ctx.fillRect(-3.6 * s + wob, -20 * s, 2 * s, 4 * s);
      ctx.fillRect(1.6 * s + wob, -20 * s, 2 * s, 4 * s);
      ctx.fillStyle = '#1a0810';
      ctx.fillRect(-2.4 * s + wob, -17 * s, 1.8 * s, 1.4 * s);
      ctx.fillRect(0.6 * s + wob, -17 * s, 1.8 * s, 1.4 * s);
    }
    ctx.restore();
  ...
  function drawBoss() {
    ...
    if (b.kind === '天狗') {
      ...
      ctx.fillRect(-2 * s, -20 * s, 10 * s, 4 * s);
      ctx.fillStyle = rgba(CYN, 0.7);
      ctx.fillRect(8 * s, -18 * s, 10 * s, 3 * s);
    } else if (b.kind === '阎王') {
      ctx.fillStyle = rgba(NIGHT, 0.95);
      ...
      ctx.beginPath();
      ctx.arc(0, -46 * s, 11 * s, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(GOLD, 0.8);
      ctx.fillRect(-8 * s, -58 * s, 4 * s, 10 * s);
      ctx.fillRect(4 * s, -58 * s, 4 * s, 10 * s);
      ctx.fillStyle = rgba(CYN, 0.85);
      ctx.beginPath();
      ctx.arc(-4 * s, -48 * s, 2.4 * s, 0, TAU);
      ctx.fill();
      ctx.beginPath();
      ctx.arc(4 * s, -48 * s, 2.4 * s, 0, TAU);
      ctx.fill();
      ...
      ctx.fillStyle = rgba(ONI, 0.95);
      ctx.beginPath();
      ctx.arc(0, -40 * s, 14 * s, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(GOLD, 0.8);
      ctx.fillRect(-10 * s, -52 * s, 5 * s, 10 * s);
      ctx.fillRect(5 * s, -52 * s, 5 * s, 10 * s);
      ctx.fillStyle = '#1a0810';
      ctx.fillRect(-8 * s, -42 * s, 5 * s, 4 * s);
      ctx.fillRect(3 * s, -42 * s, 5 * s, 4 * s);
      ctx.fillStyle = rgba(BONE, 0.85);
