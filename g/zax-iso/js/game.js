...
(function () {
  ...
  const OX = 52;
  const OY = 650;
  const BEST_KEY = 'playbox-zax-iso-best';
  const MUTE_KEY = 'playbox-zax-iso-mute';
  const OPS = '← → 平移 · ↑ ↓ 升降 · 空格开火 · R 重开 · M 静音';
  ...
  const AMB = [255, 154, 40];
  const HOT = [255, 180, 74];
  const WHT = [255, 246, 232];
  const PNK = [255, 154, 212];
  const TOP = [214, 148, 52];
  const LEFT = [92, 50, 22];
  const FRONT = [156, 90, 34];
  const SIDE_T = [168, 108, 40];
  const SIDE_L = [58, 32, 16];
  const SIDE_F = [110, 64, 26];

  const SCORE = { tank: 300, gun: 200, fighter: 150, missile: 80, boss: 2500 };
  ...
  const audio = {
    ...
    shoot() {
      this.ensure();
      this.beep(880, 0.05, 'square', 0.03, 1680);
    },
    hit(kind, combo) {
      this.ensure();
      const base = kind === 'boss' ? 180 : kind === 'tank' ? 420 : kind === 'gun' ? 520 : kind === 'missile' ? 640 : 700;
      const lift = 1 + Math.min(0.5, combo * 0.04);
      this.noise(0.05, 0.038, 1100);
      this.beep(base * lift, 0.09, 'square', 0.046, base * lift * 1.55);
    },
    tank() {
      this.ensure();
  ...
  function syncHud() {
    ...
    if (bestEl) bestEl.textContent = String(G.best);
    if (stageLabel) {
      if (G.mode === 'title') stageLabel.textContent = '斜城';
      else stageLabel.textContent = phaseName();
      stageLabel.classList.toggle('hot', G.mode === 'play' && (G.boss || G.combo >= 8));
    }
    if (tagLabel) {
      tagLabel.textContent = isLow() ? '低空' : '要塞';
    ...
    else if (G.mode === 'lose') setHint('R 重开 · 高度要对，影子贴地就撞', 'warn');
    else if (G.mode === 'win') setHint('R 重开 · 机甲已毁', 'hot');
    else if (G.warn) setHint('拉高！墙在前面', 'warn');
    else if (G.fuel < 22) setHint('油料告急 · 打油罐', 'warn');
    else if (G.lives === 1) setHint('最后一命 · 看影子对高度', 'warn');
    else if (G.boss) setHint('打机甲的眼 · 高度对准', 'hot');
    else if (G.pz >= G.fortEnd) setHint('深空 · 对准战机高度开火', '');
    else setHint('↑ 拉高飞过墙 · ↓ 俯冲打油罐炮台', '');
    syncPips();
  }
  ...
  function showOverlay(kind, title, lead) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'ZAXXON';
  ...
  function buildLevel(low) {
    const walls = [];
    const ents = [];
    const fortEnd = low ? 4000 : 3480;
    const bossZ = fortEnd + (low ? 1320 : 1680);

    function addWall(x, z, w, d, h) {
      walls.push({ x: x, z: z, w: w, d: d, h: Math.max(8, h) });
    }
    ...
    function addGun(x, z) {
      ents.push({
        ...
        cd: 0.5 + hash2((z | 0) + 3) * 0.9
      });
    }

    addWall(28, 360, 140, 18, low ? 24 : 16);
    addTank(64, 450);
    addTank(132, 510);
    addGun(100, 580);
    ...
    let z = 640;
    let i = 0;
    const spacing = low ? 70 : 90;
    while (z < fortEnd - 280) {
      const u = hash2(i * 17 + (low ? 91 : 3));
      const v = hash2(i * 31 + 11);
      const base = low ? 38 : 26;
      const spread = low ? 30 : 24;
      const H = base + v * spread;
      if (u < 0.2) {
        addWall(18, z, FORT_W - 36, 22, H);
        if (v > 0.32) addGun(40 + v * 110, z + 50);
      } else if (u < 0.4) {
        const ww = 78 + v * 50;
        addWall(16, z, ww, 24, H);
        if (v > 0.48) addTank(clamp(16 + ww + 26, 30, 170), z + 10);
        else addGun(clamp(16 + ww + 30, 30, 170), z + 14);
      } else if (u < 0.6) {
        const ww = 78 + v * 50;
        addWall(FORT_W - 16 - ww, z, ww, 24, H);
        if (v > 0.5) addTank(42, z + 10);
        else addGun(50, z + 10);
      } else if (u < 0.74) {
        addWall(14, z, 54, 26, H + 4);
        addWall(FORT_W - 68, z, 54, 26, H);
        addGun(FORT_W * 0.5, z + 8);
      } else if (u < 0.88) {
        addTank(52 + v * 28, z);
        addTank(118 + v * 22, z + 38);
        if (low) addGun(92, z + 66);
        if (v > 0.55) addWall(28, z + 86, 86, 18, base);
      } else {
        addWall(22, z, 72, 16, base);
        addWall(68, z + 26, 92, 16, base + 12);
        addWall(34, z + 52, 124, 16, base + 20);
      }
      z += spacing + v * 26;
      i += 1;
    }
    addTank(100, fortEnd - 220);
    addWall(20, fortEnd - 130, FORT_W - 40, 24, low ? 50 : 36);
    addGun(58, fortEnd - 70);
    addGun(142, fortEnd - 70);
    return { walls: walls, ents: ents, fortEnd: fortEnd, bossZ: bossZ };
  }
  ...
  function spawnBoss() {
    ...
    pushEnt({
      ...
      hp: isLow() ? 26 : 20,
      max: isLow() ? 26 : 20,
      vx: 34, cd: 0.9, phase: 0, flash: 0, alive: true,
      eyeAlt: 50, eyeZ: 16
    });
    toast('机甲出现', false, true);
    audio.check();
    syncHud();
  }
  ...
  function scrollSpd() {
    ...
    if (G.pz >= G.fortEnd) return isLow() ? 168 : 148;
    return isLow() ? 138 : 116;
  }

  function fuelDrain() {
    if (G.pz >= G.fortEnd) return isLow() ? 3.6 : 2.7;
    return isLow() ? 4.2 : 3.1;
  }
  ...
  function goWin() {
    ...
    saveBest();
    audio.win();
    showOverlay('win', isLow() ? '低空通关' : '斜城攻破', '机甲已毁  ·  分数 ' + G.score);
  ...
  function resetRun(kind) {
    G.kind = kind === 'low' ? 'low' : 'fort';
    G.t = 0;
    G.taught = false;
    const level = buildLevel(isLow());
    G.walls = level.walls;
    G.ents = level.ents;
    G.fortEnd = level.fortEnd;
    G.bossZ = level.bossZ;
    G.camZ = 0;
    G.px = 100;
    G.pz = PZ;
    ...
    pointer.sx = iso(G.px, G.pz, G.pal).x;
    pointer.sy = iso(G.px, G.pz, G.pal).y;
  }
  ...
  function startGame(kind) {
    ...
    hideOverlay();
    audio.start();
    toast(isLow() ? '低空 · 墙更高更密' : '要塞 · 先城后空', false, !isLow());
    syncHud();
    if (canvas && canvas.focus) canvas.focus();
  }
  ...
  function goTitle() {
    ...
    G.mode = 'title';
    G.invuln = 99;
    showOverlay('title', '斜城', '高度要对。飞过高墙，影子贴地就撞。打油罐补油，打炮台，再打机甲。');
    syncHud();
  }
  ...
  function movePlayer(dt) {
    if (G.deadT > 0) return;
    const spdX = isLow() ? 228 : 210;
    const spdA = isLow() ? 168 : 158;
  ...
  function updateShots(dt) {
    for (let i = G.shots.length - 1; i >= 0; i--) {
      ...
      s.z += s.vz * dt;
      s.life -= dt;
      if (groundH(s.x, s.z) > s.alt + 1) {
        popSpark(s.x, s.z, s.alt, AMB, 8);
        G.shots.splice(i, 1);
        continue;
      }
      ...
      if (e.type === 'fighter' && e.alive) {
        ...
        if (G.mode === 'play') {
          ...
          if (e.cd <= 0 && dz > 20 && dz < 280) {
            ...
            const spd = low ? 210 : 170;
            enemyShot(e.x, e.z - 6, e.alt, dx / d * spd, dzz / d * spd, da / d * spd);
            e.cd = low ? rand(0.7, 1.2) : rand(0.95, 1.6);
          }
        }
  ...
  function updatePhase() {
    ...
    if (p !== G.lastPhase && G.mode === 'play') {
      if (p === '深空') {
        toast('冲出要塞', false, true);
        audio.check();
      } else if (p === '机甲') {
        /* spawned toast already */
      }
      G.lastPhase = p;
    }
  ...
  function update(dt) {
    ...
    if (G.pz >= G.fortEnd && !G.boss) {
      G.spawnT -= dt;
      if (G.spawnT <= 0) {
        const n = isLow() ? 2 + (hash2((G.pz | 0)) > 0.55 ? 1 : 0) : 1 + (hash2((G.pz | 0)) > 0.4 ? 1 : 0);
        for (let i = 0; i < n; i++) spawnFighter();
        G.spawnT = isLow() ? rand(0.62, 1.05) : rand(0.85, 1.35);
      }
    }
  ...
  function drawFloor() {
    if (G.pz > G.fortEnd + 80 && G.camZ > G.fortEnd - 40) return;
    const z0 = Math.floor((G.camZ - 24) / TILE) * TILE;
    const z1 = Math.min(G.camZ + 460, G.fortEnd + 8);
    for (let z = z0; z < z1; z += TILE) {
      for (let x = 16; x < FORT_W - 16; x += TILE) {
        const parity = (((x / TILE) | 0) + ((z / TILE) | 0)) & 1;
        drawTile(x, z, TILE, parity ? [42, 28, 14] : [32, 20, 10], 1);
      }
    }
    const c = ctx;
    c.save();
    c.globalAlpha = 0.35;
    c.strokeStyle = rgba(AMB, 0.35);
    c.lineWidth = Math.max(1, 0.8 * scale);
    const zg = Math.floor((G.camZ - 24) / (TILE * 2)) * (TILE * 2);
  ...
  function drawSides() {
    const z0 = Math.floor((G.camZ - 20) / 32) * 32;
    const z1 = Math.min(G.camZ + 460, G.fortEnd);
  ...
  function drawWall(w) {
    ...
    const top = danger ? MAG : (w.h > 44 ? HOT : TOP);
    const left = danger ? [90, 22, 48] : LEFT;
    const front = danger ? [150, 40, 70] : FRONT;
    ...
    strokePoly([At, Bt, Ct, Dt], rgba(danger ? MAG : GOLD, danger ? 0.95 : 0.45), Math.max(1, 1.4 * scale));
  }

  function drawTank(e) {
    if (!e.alive) return;
    const top = e.flash > 0 ? WHT : GOLD;
    drawBox(e.x - 8, e.z - 7, 16, 14, 16, top, [120, 80, 20], AMB, 1, 0);
  ...
  function drawGun(e) {
    ...
    drawBox(e.x - 4, e.z - 3, 8, 8, 8, HOT, [90, 20, 50], MAG, 1, 12);
  ...
  function drawBoss(e) {
    ...
    const top = flash ? WHT : [200, 80, 40];
    const left = flash ? [180, 180, 180] : [80, 28, 18];
    const front = flash ? PNK : [160, 50, 32];
    ...
    drawBox(x - 12, z + 8, 24, 16, 14, flash ? WHT : HOT, left, front, 1, 42);
    drawBox(x - 34, z + 10, 12, 10, 10, AMB, left, front, 1, 28);
    drawBox(x + 22, z + 10, 12, 10, 10, AMB, left, front, 1, 28);
    const eye = iso(x, z + e.eyeZ, e.eyeAlt);
    const pulse = 0.55 + 0.45 * Math.sin(G.t * 8);
    ctx.fillStyle = rgba(e.alive ? (flash ? WHT : CYN) : MAG, pulse);
    ctx.beginPath();
  ...
  function drawShip() {
    ...
    fillPoly([nose, tail, wingR], rgba([0, 170, 210], 0.96));
    fillPoly([nose, iso(x - 3, z + 2, a + 4), iso(x + 3, z + 2, a + 4)], rgba(WHT, 0.92));
    fillPoly([tail, wingL, keel], rgba([0, 90, 120], 0.8));
    fillPoly([tail, keel, wingR], rgba([0, 70, 100], 0.8));
    if (G.muzzle > 0) {
  ...
  function drawStars() {
    const spaceT = clamp((G.camZ - (G.fortEnd - 200)) / 400, 0, 1);
    if (spaceT <= 0) return;
    ctx.save();
  ...
  function drawBg() {
    ...
    const spaceT = clamp((G.camZ - (G.fortEnd - 280)) / 500, 0, 1);
    g.addColorStop(0, spaceT > 0.4 ? '#12081c' : '#24140a');
    g.addColorStop(1, spaceT > 0.4 ? '#070312' : '#0c0906');
    c.fillStyle = g;
