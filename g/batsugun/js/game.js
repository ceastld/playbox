...
(function () {
  ...
  const AUTO_SPEED_KEY = 'playbox-batsugun-auto-speed';
  const SPEED_LABELS = ['', '慢', '中', '快', '极快'];
  const AUTO_SCALE = [1, 0.48, 0.72, 1, 2.55];
  const OPS = '←↑↓→ / WSD 移动 · 空格射击 · Shift / Z 爆弹 · A 自动 · R 重开 · M 静音';
  ...
  function syncHud() {
    ...
    else setHint('空格连射 · Shift 战枪爆 · 吃 枪 升火力 · A 自动', '');
    syncPips();
  }
  ...
  function autoDanger(x, y, horizon) {
    ...
    for (let i = 0; i < G.enemies.length; i++) {
      ...
      const r = e.kind === 'boss' ? e.r * 0.62 : e.kind === 'pod' ? e.r * 0.68 : e.r * 0.72;
      const body = hitR + r;
      if (dist < body + 28) {
        const soon = (look - t) / Math.max(0.08, look);
        const w = e.kind === 'dive' ? 36 : e.kind === 'boss' ? 14 : e.kind === 'elite' ? 16 : 18;
  ...
  function autoThink() {
    ...
    for (let i = 0; i < G.enemies.length; i++) {
      ...
      else if (e.kind === 'elite') w = 96;
      else if (e.kind === 'pod') w = 70;
