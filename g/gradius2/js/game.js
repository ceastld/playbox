...
(function () {
  ...
  const BEST_KEY = 'playbox-gradius2-best';
  const MUTE_KEY = 'playbox-gradius2-mute';
  const OPS = '←↑↓→ / WASD 飞 · 空格射击 · Shift / Z 点选 · R 重开 · M 静音';
  const STAGE_NAME = ['炎日', '晶廊', '核门'];
  const BOSS_NAME = ['火核', '晶核', '戈核'];
  const LEAD = '横版巡二。吃胶囊推进武装槽，Shift 点选。涟射扩环，导弹上下分叉。火鸟会从灰里再起。打穿三关后轰核。别当成秘武或沙罗——这是炎日、晶廊、核门，不是火山摩艾，不是血腔巨蛇。';
  ...
  const SUN = [255, 138, 58];
  const ICE = [154, 244, 255];
  const WHT = [232, 248, 255];
  const PNK = [255, 154, 212];
  const STN = [108, 148, 172];
  const RED = [255, 72, 88];
  const DEEP = [8, 28, 40];

  const SCORE = {
    fan: 50, bird: 80, chick: 40, ash: 30, prism: 90,
    gun: 120, shard: 60, flare: 0, core: 2500
  };
  ...
  function fortStep(wx) {
    const p = 280;
    const m = ((wx % p) + p) % p;
    if (m < 70) return 22;
    if (m > 180 && m < 240) return 36;
    return 0;
  }
  function caveAt(wx) {
    const st = stageAt(wx);
    const n1 = fbm(wx * 0.0017, 2);
    const n2 = fbm(wx * 0.0036, 11);
    let top = 10;
    let bot = VH - 10;
    if (st === 1) {
      top = 10 + n1 * 18;
      ...
      top = 16 + n1 * 22 + crystalTooth(wx);
      bot = VH - 16 - n2 * 24 - crystalTooth(wx + 90);
    } else {
      top = 18 + n1 * 28 + fortStep(wx);
      bot = VH - 18 - n2 * 30 - fortStep(wx + 140);
    }
    if (wx < 480) {
      const t = wx / 480;
      top = lerp(10, top, t);
  ...
  const audio = {
    ...
    shoot(ripple) {
      this.ensure();
      if (ripple) {
        this.beep(420, 0.09, 'sine', 0.032, 180);
        this.beep(880, 0.07, 'triangle', 0.028, 440);
      } else {
        this.beep(920, 0.05, 'square', 0.03, 1680);
      }
    },
    missile() {
      this.ensure();
      this.beep(240, 0.1, 'sawtooth', 0.04, 96);
    ...
    laserOn() {
      this.ensure();
      this.beep(180, 0.1, 'sine', 0.04, 720);
      this.beep(540, 0.16, 'triangle', 0.04, 216);
    },
    option() {
      this.ensure();
    ...
    rise() {
      this.ensure();
      this.beep(220, 0.1, 'sawtooth', 0.04, 660);
      this.beep(440, 0.14, 'triangle', 0.032, 880);
    },
    bounce() {
      this.ensure();
      this.beep(1240, 0.05, 'triangle', 0.03, 620);
    },
    check() {
      this.ensure();
  ...
  function syncHud() {
    ...
    if (bestEl) bestEl.textContent = String(G.best);
    if (stageLabel) {
      if (G.mode === 'title') stageLabel.textContent = '巡二';
    ...
    if (tagLabel) {
      tagLabel.textContent = isCore() ? '巡核' : '巡二';
    ...
    if (misEl) misEl.hidden = !(G.mode === 'play' && G.missile);
    if (optEl) {
      optEl.hidden = !(G.mode === 'play' && G.options.length > 0);
      optEl.textContent = '分' + (G.options.length > 1 ? G.options.length : '');
    }
    if (shdEl) {
    ...
    else if (G.mode === 'lose') setHint('R 重开 · 火鸟会再起，灰烬要补一枪', 'warn');
    else if (G.mode === 'win') setHint('R 重开 · 戈核尽破', 'hot');
    else if (G.lives === 1) setHint('最后一命 · 护盾优先挡弹', 'warn');
    else if (G.bar > 0) {
      const s = SLOTS[G.bar - 1];
      setHint('Shift 点选「' + (s ? s.full : '') + '」', 'hot');
    } else if (G.laser) setHint('涟射扩环穿群 · 分身抄射', '');
    else setHint('吃胶囊推进武装槽 · Shift 点选 · 火鸟会再起', '');
    syncPips();
    syncSlots();
  }
  ...
  function showOverlay(kind, title, lead) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'GRD2';
  ...
  function moveSpd() {
    return (isCore() ? 164 : 148) + G.speed * 34;
  }
  ...
  function scrollSpd() {
    ...
    const base = isCore() ? 138 : 102;
    return base + (G.stage - 1) * 8 + Math.min(18, G.combo * 0.55);
  }
  ...
  function spawnShard(wx, ceil, reflect) {
    const cave = caveAt(wx);
    const y = ceil ? cave.top + 20 : cave.bot - 20;
  ...
  function spawnSlice(wx) {
    ...
    if (st === 1) {
      ...
      if (slice % 5 === 2 && h > 0.22 * dens) {
        spawnBird(wx, mid + (h > 0.5 ? 36 : -36), isCore() ? 3 : 2);
      }
      if (slice % 9 === 4) spawnFlare(wx);
      if (slice % 8 === 1 && h > 0.4) spawnGun(wx, h > 0.62);
    } else if (st === 2) {
      if (slice % fanEvery === 1 && h > 0.18 * dens) {
        spawnFan(wx, lerp(cave.top + 42, cave.bot - 42, h), isCore() ? 5 : 4, 0, false);
      }
      if (slice % 4 === 0 && h > 0.2) spawnPrism(wx, mid + (h - 0.5) * 80);
      if (slice % 5 === 2) spawnShard(wx, h > 0.5, h > 0.62);
      if (isCore() && slice % 6 === 4) spawnShard(wx + 40, h <= 0.5, false);
      if (slice % 7 === 3 && h > 0.3) spawnGun(wx, h > 0.45);
    } else {
      ...
      if (slice % 8 === 3 && h > 0.28) spawnBird(wx, mid, 2);
      if (slice % 5 === 1 && h > 0.34) spawnPrism(wx, mid);
    }
    if (slice % 7 === 3 && h > 0.3) {
  ...
  function spawnBoss() {
    G.boss = true;
    const st = G.stage;
    let hp = st === 1 ? 40 : st === 2 ? 56 : 78;
    if (isCore()) hp = (hp * 1.28) | 0;
    const cave = caveAt(G.cam + VW * 0.72);
    const kind = st === 1 ? 'sun' : st === 2 ? 'ice' : 'gof';
    pushEnt({
      type: 'core',
      kind: kind,
      wx: G.cam + VW * 0.78,
    ...
    audio.check();
    kick(3.4);
    screenFlash(st === 1 ? SUN : st === 2 ? ICE : GOLD, 0.32);
    syncHud();
  }
  ...
  function award(kind, x, y) {
    ...
    addScore(n);
    const gold = kind === 'core' || kind === 'bird' || G.mult >= 3;
  ...
  function fire() {
    ...
    G.fireCd = G.laser ? 0.155 : 0.108;
    G.muzzle = 0.055;
    const srcs = sources();
    const ripple = G.laser;
    for (let i = 0; i < srcs.length; i++) {
      const p = srcs[i];
      const wx = G.cam + p.x;
      if (ripple) {
        pushShot({
          type: 'ripple', wx: wx, y: p.y, vx: 420, vy: 0,
          r: 5.2, hw: 8, hh: 8, life: 0.52, hit: {}, ripple: true
        });
      } else if (G.double) {
        pushShot({
        ...
        pushShot({
          type: 'shot', wx: wx, y: p.y - 2, vx: 540, vy: -270,
    ...
    if (!REDUCE) {
      emit(ripple ? 5 : 3, {
        ...
        r0: 1, r1: 2.4, life: 0.12, rgb: ripple ? ICE : WHT, g: 0
      });
    }
    if (G.missile && G.misCd <= 0) {
      ...
      for (let i = 0; i < srcs.length; i++) {
        const p = srcs[i];
        pushShot({
          type: 'mis', wx: G.cam + p.x, y: p.y + 3, vx: 90, vy: 240,
          hw: 3.5, hh: 4, life: 1.55, mis: true, dir: 1, grounded: false
        });
        pushShot({
          type: 'mis', wx: G.cam + p.x, y: p.y - 3, vx: 90, vy: -240,
          hw: 3.5, hh: 4, life: 1.55, mis: true, dir: -1, grounded: false
  ...
  function hurt(e, dmg, hx, hy) {
    ...
    if (e.hp > 0) {
      ...
      if (e.type === 'core' || e.type === 'bird' || e.type === 'shard') hitStop(0.028);
  ...
  function killEnt(e) {
    ...
    if (e.type === 'core') {
      ...
      explode(x + 20, y - 10, e.kind === 'sun' ? SUN : CYN, 22);
      award('core', x, y);
    ...
    if (e.type === 'ash') {
      ...
      audio.hit(G.combo);
      hitStop(0.032);
      floatText(x, y - 12, '断火', GOLD, true);
      return;
    }
    if (e.type === 'bird' && e.rise) {
      explode(x, y, SUN, 18);
      award('bird', x, y);
      audio.hit(G.combo);
      hitStop(clamp(0.034 + G.combo * 0.002, 0.034, 0.06));
      kick(2.2);
      spawnAsh(e.wx, e.y);
      if (e.red || hash2(e.id * 13) > 0.78) spawnCap(e.wx, e.y);
      return;
    }
    const kind = e.red ? 'red' : e.type;
    const rgb = e.type === 'chick' ? SUN
      : e.type === 'prism' || e.type === 'shard' ? ICE
      : e.red ? RED
      : e.type === 'gun' ? STN : MAG;
    explode(x, y, rgb, e.type === 'shard' ? 20 : 16);
    award(kind === 'red' ? 'fan' : kind, x, y);
    audio.hit(G.combo);
    hitStop(clamp(0.03 + G.combo * 0.0022, 0.03, 0.062));
    kick(e.type === 'shard' || e.type === 'gun' ? 3.2 : 1.8);
    if (e.red || (e.type === 'fan' && hash2(e.id * 13) > 0.82) || (e.type === 'prism' && hash2(e.id) > 0.62) || (e.type === 'gun' && hash2(e.id) > 0.7)) {
      spawnCap(e.wx, e.y);
    }
  }
  ...
  function shotHitsEnt(s, e) {
    ...
    if (e.type === 'cap' || e.type === 'flare') return false;
    if (e.type === 'ash' && e.grace > 0) return false;
    const sy0 = s.y;
    if (s.ripple) {
      const dx = s.wx - e.wx;
      const dy = s.y - e.y;
      const rr = s.r + Math.max(e.hw, e.hh);
      if (dx * dx + dy * dy > rr * rr) return false;
      if (e.type === 'core' && !coreOpen(e)) return 'block';
      if (e.type === 'shard' && e.reflect) return 'reflect';
      return true;
    }
    const shw = s.hw;
    const hh = s.hh;
    ...
    if (!aabb(cx, sy0, shw, hh, e.wx, e.y, e.hw, e.hh)) return false;
    if (e.type === 'shard' && e.reflect) return 'reflect';
    return true;
  }

  function bounceShot(s) {
    audio.bounce();
    enemyShot(s.wx - 6, s.y, -280, rand(-40, 40), 3.4);
    emit(4, {
      x: scrX(s.wx), y: s.y, j: 4,
      vx0: -80, vx1: 20, vy0: -50, vy1: 50,
      life: 0.14, r0: 1, r1: 2.2, rgb: ICE, g: 0
    });
  }
  ...
  function updateShots(dt) {
    for (let i = G.shots.length - 1; i >= 0; i--) {
      ...
      s.y += s.vy * dt;
      s.life -= dt;
      if (s.ripple) s.r = Math.min(34, s.r + 52 * dt);
      if (s.mis) {
        const cave = caveAt(s.wx);
        s.vx = Math.min(460, s.vx + 380 * dt);
        if (!s.grounded) {
          s.vy += s.dir * 520 * dt;
          if (s.dir > 0 && s.y >= cave.bot - 7) {
            s.y = cave.bot - 7;
            s.grounded = true;
            s.vy = 0;
            s.vx = 400;
          } else if (s.dir < 0 && s.y <= cave.top + 7) {
            s.y = cave.top + 7;
            s.grounded = true;
            s.vy = 0;
            s.vx = 400;
          ...
          s.y = s.dir > 0 ? cave.bot - 7 : cave.top + 7;
          s.vy = 0;
        }
      }
      const x = scrX(s.wx);
      if (s.life <= 0 || x > VW + 60 || x < -50 || s.y < -28 || s.y > VH + 28) {
        G.shots.splice(i, 1);
        continue;
      }
      if (!s.ripple && !s.mis) {
      ...
      for (let k = 0; k < G.ents.length; k++) {
        ...
        if (hit === 'reflect') {
          bounceShot(s);
          if (!s.ripple) gone = true;
          else if (s.hit[e.id] && G.t - s.hit[e.id] < 0.1) continue;
          else {
            s.hit[e.id] = G.t;
            hurt(e, 1, hx, e.y);
          }
        ...
        if (s.ripple) {
          if (s.hit[e.id] && G.t - s.hit[e.id] < 0.1) continue;
          s.hit[e.id] = G.t;
          hurt(e, 1, hx, e.y);
        } else {
          hurt(e, 1, hx, e.y);
  ...
  function updateEnts(dt) {
    ...
    for (let i = G.ents.length - 1; i >= 0; i--) {
      ...
      if (e.type === 'fan') {
        ...
        e.wx += e.vx * dt;
        e.wing += dt;
        e.y += Math.sin(G.t * 2.4 + e.phase) * 58 * dt;
        const cave = caveAt(e.wx);
        e.y = clamp(e.y, cave.top + 22, cave.bot - 22);
        e.cd -= dt;
        if (e.cd <= 0 && x < VW * 0.8 && x > 50) {
          e.cd = dense ? rand(1.0, 1.7) : rand(1.5, 2.4);
          aimAt(e, dense ? 170 : 140);
          enemyShot(e.wx - 4, e.y, e.vx - 40, 90, 2.8);
          enemyShot(e.wx - 4, e.y, e.vx - 40, -90, 2.8);
        }
      } else if (e.type === 'chick') {
        e.wx += e.vx * dt;
        e.y += Math.sin(G.t * 5 + e.phase) * 70 * dt;
        const cave = caveAt(e.wx);
        e.y = clamp(e.y, cave.top + 16, cave.bot - 16);
        e.cd -= dt;
        if (e.cd <= 0 && x < VW * 0.78) {
        ...
        e.life -= dt;
        e.spin += dt * 4;
        e.y += Math.sin(G.t * 6) * 18 * dt;
        if (e.life <= 0) {
          e.alive = false;
          spawnChick(e.wx, e.y);
          audio.rise();
          popSpark(scrX(e.wx), e.y, SUN, 16);
          floatText(scrX(e.wx), e.y - 10, '再起', SUN, true);
        }
      } else if (e.type === 'prism') {
        e.wx += e.vx * dt;
        e.spin += dt * 2.2;
        e.y += Math.sin(G.t * 2.1 + e.phase) * 36 * dt;
        const cave = caveAt(e.wx);
        ...
        if (e.cd <= 0 && x < VW * 0.8) {
          e.cd = dense ? 1.05 : 1.5;
          for (let k = -1; k <= 1; k++) {
            enemyShot(e.wx - 6, e.y, -150, k * 90, 2.8);
          }
        }
      } else if (e.type === 'gun') {
        const cave = caveAt(e.wx);
        ...
        const cave = caveAt(e.wx);
        e.y = e.ceil ? cave.top + 20 : cave.bot - 20;
        e.spin += dt * (e.reflect ? 2.6 : 0.6);
      } else if (e.type === 'flare') {
        e.pulse += dt;
        e.life -= dt;
        const cave = caveAt(e.wx);
        e.y = (cave.top + cave.bot) * 0.5;
        e.hh = (cave.bot - cave.top) * (0.28 + Math.sin(e.pulse * 5) * 0.08);
        ...
        e.open = u > 0.38 && u < 0.78 ? 1 : Math.max(0, e.open - dt * 1.6);
        if (u > 0.38 && u < 0.78) e.open = Math.min(1, e.open + dt * 2.4);
        e.cd -= dt;
        if (e.cd <= 0) {
          ...
          if (coreOpen(e)) {
            if (e.kind === 'sun') {
              aimAt(e, dense ? 210 : 170);
              enemyShot(e.wx - 20, e.y - 18, -160, -70, 3.6);
              enemyShot(e.wx - 20, e.y + 18, -160, 70, 3.6);
            } else if (e.kind === 'ice') {
              for (let k = -2; k <= 2; k++) {
                enemyShot(e.wx - 18, e.y, -190, k * 55, 3);
              }
            } else {
              aimAt(e, dense ? 230 : 190);
              enemyShot(e.wx - 16, e.y, -140, 0, 5.2);
              if (e.hp < e.max * 0.5) {
                enemyShot(e.wx - 10, e.y - 12, -120, -90, 3);
                enemyShot(e.wx - 10, e.y + 12, -120, 90, 3);
              }
            }
          }
      ...
      if (G.mode === 'play' && G.deadT <= 0 && G.invuln <= 0 && e.alive && e.type !== 'cap') {
        ...
        if (hit) {
          if (e.type === 'ash') {
            if (e.grace <= 0) collectAshCrash(e);
          } else {
            killPlayer();
          }
        }
  ...
  function collectAshCrash(e) {
    e.alive = false;
    const x = scrX(e.wx);
    explode(x, e.y, SUN, 12);
    award('ash', x, e.y);
    audio.hit(G.combo);
    hitStop(0.028);
    floatText(x, e.y - 12, '断火', GOLD, true);
  }
  ...
  function drawSun() {
    ...
    g.addColorStop(0.35, rgba(SUN, 0.55));
  ...
  function drawCave() {
    ...
    c.lineTo(sx(VW), sy(0));
    c.closePath();
    c.fillStyle = st === 2 ? '#0a2434' : st === 3 ? '#081820' : '#0c1c22';
    c.fill();
    c.beginPath();
    c.moveTo(sx(0), sy(VH));
    for (let x = 0; x <= VW; x += step) {
    ...
    c.strokeStyle = st === 1 ? rgba(SUN, 0.45) : st === 2 ? rgba(ICE, 0.55) : rgba(CYN, 0.4);
    ...
    if (st === 2) {
      ...
      for (let x = 0; x < VW; x += 40) {
        const cave = caveAt(G.cam + x);
        c.fillRect(sx(x), sy(cave.top), 6 * scale, 10 * scale);
        c.fillRect(sx(x + 12), sy(cave.bot - 10), 6 * scale, 10 * scale);
      }
    } else if (st === 3) {
      c.strokeStyle = rgba(CYN, 0.12);
      c.lineWidth = Math.max(1, scale);
      for (let x = 0; x < VW; x += 48) {
        const cave = caveAt(G.cam + x);
        c.strokeRect(sx(x), sy(cave.top), 24 * scale, 14 * scale);
        c.strokeRect(sx(x + 8), sy(cave.bot - 14), 24 * scale, 14 * scale);
      }
    }
  }
  ...
  function drawBird(e, x, chick) {
    const c = ctx;
    const s = scale * (chick ? 0.72 : 1);
    const flap = Math.sin((e.wing || G.t) * (chick ? 14 : 9)) * (chick ? 8 : 12);
    c.save();
    c.translate(sx(x), sy(e.y));
  ...
  function drawAsh(e, x) {
    const c = ctx;
    const s = scale;
    c.save();
    c.translate(sx(x), sy(e.y));
    c.rotate(e.spin);
    const a = 0.45 + e.life * 0.5;
    c.fillStyle = rgba(SUN, a);
    c.beginPath();
    c.arc(0, 0, 6 * s, 0, TAU);
    c.fill();
    c.fillStyle = rgba(GOLD, a);
    c.beginPath();
    c.arc(0, 0, 2.6 * s, 0, TAU);
    c.fill();
    c.restore();
  }

  function drawPrism(e, x) {
    const c = ctx;
    const s = scale;
    c.save();
    c.translate(sx(x), sy(e.y));
    c.rotate(e.spin);
    c.fillStyle = rgba(e.flash > 0 ? WHT : ICE, 0.9);
    c.beginPath();
    c.moveTo(0, -13 * s);
    c.lineTo(11 * s, 0);
    c.lineTo(0, 13 * s);
  ...
  function drawGun(e, x) {
    ...
    c.fillStyle = rgba(e.flash > 0 ? WHT : STN, 0.95);
  ...
  function drawShard(e, x) {
    const c = ctx;
    const s = scale;
    c.save();
    c.translate(sx(x), sy(e.y));
    if (e.ceil) c.scale(1, -1);
    c.rotate(e.reflect ? e.spin * 0.15 : 0);
    c.fillStyle = rgba(e.flash > 0 ? WHT : (e.reflect ? ICE : [90, 170, 196]), 0.92);
    c.beginPath();
    c.moveTo(0, -18 * s);
    c.lineTo(12 * s, 14 * s);
    c.lineTo(-12 * s, 14 * s);
  ...
  function drawFlare(e, x) {
    const c = ctx;
    const pulse = 0.35 + Math.sin(e.pulse * 5) * 0.35;
    const g = c.createLinearGradient(sx(x), sy(e.y - e.hh), sx(x), sy(e.y + e.hh));
    g.addColorStop(0, rgba(SUN, 0));
    g.addColorStop(0.5, rgba(SUN, pulse));
    g.addColorStop(1, rgba(SUN, 0));
    c.fillStyle = g;
  ...
  function drawCore(e, x) {
    ...
    const open = e.open;
    if (e.kind === 'sun') {
      const flap = Math.sin(e.wing * 6) * 10;
      c.fillStyle = rgba(SUN, 0.45);
      c.beginPath();
      c.moveTo(-10 * s, 0);
      c.lineTo(-40 * s, (-28 - flap) * s);
      c.lineTo(10 * s, -8 * s);
      c.closePath();
      c.fill();
      c.beginPath();
    ...
    const body = e.flash > 0 ? WHT : e.kind === 'ice' ? [70, 130, 156] : e.kind === 'gof' ? [86, 70, 96] : [80, 96, 108];
    ...
    if (e.kind === 'gof') {
      ...
      c.beginPath();
      c.ellipse(-6 * s, -6 * s, 10 * s, 7 * s, 0, 0, TAU);
      c.fill();
      c.beginPath();
      c.ellipse(-6 * s, 10 * s, 16 * s, (6 + open * 8) * s, 0, 0, TAU);
      c.fill();
    } else {
      c.fillStyle = rgba(DEEP, 1);
      c.beginPath();
    ...
    c.fillStyle = rgba(open > 0.55 ? (e.kind === 'sun' ? SUN : GOLD) : [40, 50, 58], 0.4 + open * 0.6);
    c.beginPath();
    c.arc(-8 * s, 0, (7 + open * 5) * s, 0, TAU);
    c.fill();
    if (open > 0.55) {
      c.strokeStyle = rgba(e.kind === 'ice' ? ICE : GOLD, 0.7);
      c.lineWidth = Math.max(1, 1.4 * s);
      c.beginPath();
      c.arc(-8 * s, 0, 16 * s, 0, TAU);
      c.stroke();
    }
    c.fillStyle = rgba(e.kind === 'ice' ? ICE : MAG, 0.85);
  ...
  function drawEnts() {
    for (let i = 0; i < G.ents.length; i++) {
      ...
      if (e.type === 'flare') drawFlare(e, x);
      else if (e.type === 'fan') drawFan(e, x);
      else if (e.type === 'bird') drawBird(e, x, false);
      else if (e.type === 'chick') drawBird(e, x, true);
      else if (e.type === 'ash') drawAsh(e, x);
      else if (e.type === 'prism') drawPrism(e, x);
      else if (e.type === 'gun') drawGun(e, x);
      else if (e.type === 'shard') drawShard(e, x);
  ...
  function drawShots() {
    ...
    for (let i = 0; i < G.shots.length; i++) {
      ...
      const x = scrX(s.wx);
      if (s.ripple) {
        c.strokeStyle = rgba(ICE, 0.85);
        c.lineWidth = Math.max(1.2, 2.2 * scale);
        c.beginPath();
        c.ellipse(sx(x), sy(s.y), s.r * 1.35 * scale, s.r * 0.72 * scale, 0, 0, TAU);
        c.stroke();
        c.strokeStyle = rgba(WHT, 0.55);
        c.lineWidth = Math.max(1, scale);
        c.beginPath();
        c.ellipse(sx(x), sy(s.y), s.r * 0.7 * scale, s.r * 0.38 * scale, 0, 0, TAU);
        c.stroke();
      } else if (s.mis) {
        c.fillStyle = rgba(s.dir > 0 ? SUN : GOLD, 0.95);
        c.beginPath();
        ...
        c.fillRect(sx(x), sy(s.y - 1.6), 11 * scale, 3.2 * scale);
        if (!REDUCE) {
  ...
  function drawBossBar() {
    ...
    c.fillStyle = rgba(b.kind === 'sun' ? SUN : GOLD, 0.9);
  ...
  function startGame(kind) {
    ...
    hideOverlay();
    audio.start();
    toast(isCore() ? '巡核' : '巡二', false, !isCore());
    trySpawn();
    syncHud();
    if (canvas && canvas.focus) canvas.focus();
  }
