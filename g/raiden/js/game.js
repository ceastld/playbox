...
(function () {
  ...
  const BOMB_CAP = 6;
  const WPN_MAX = 3;
  const BEST_KEY = 'playbox-raiden-best';
  const MUTE_KEY = 'playbox-raiden-mute';
  const AUTO_SPEED_KEY = 'playbox-raiden-auto-speed';
  ...
  const CYN = [0, 232, 255];
  const SKY = [92, 239, 255];
  const GOLD = [255, 227, 107];
  const VIO = [122, 108, 255];
  const WHT = [232, 251, 255];
  const PNK = [255, 154, 212];
  const RED = [255, 86, 110];
  const ORG = [255, 160, 72];

  const WPN_NAME = { spread: '散', laser: '激', homing: '导' };
  const DROP_CYCLE = ['spread', 'laser', 'homing', 'bomb'];
  const DROP_GLYPH = { spread: '散', laser: '激', homing: '导', bomb: '爆' };
  ...
  function plySpd() {
    return (isStorm() ? 318 : 276) + G.wpnLv * 10;
  }
  function scrollSpd() {
    if (hasBig()) return isStorm() ? 36 : 28;
    const base = isStorm() ? 118 : 86;
    const rush = G.combo >= 8 ? 16 : G.combo >= 4 ? 8 : 0;
    return base + rush + (G.stage - 1) * (isStorm() ? 10 : 8);
  }
  function hash2(n) {
    n |= 0;
  ...
  const audio = {
    ...
    shoot(kind) {
      this.ensure();
      if (kind === 'laser') this.beep(880, 0.07, 'sawtooth', 0.034, 420);
      else if (kind === 'homing') this.beep(310, 0.08, 'square', 0.034, 620);
      else this.beep(680, 0.05, 'square', 0.03, 1540);
    },
    hit(combo) {
      this.ensure();
  ...
  function syncHud() {
    ...
    if (tagLabel) {
      tagLabel.textContent = isStorm() ? '弹雨' : '雷击';
    ...
    if (wpnLabel) {
      ...
      wpnLabel.classList.toggle('homing', G.wpn === 'homing');
    }
    if (bombLabel) {
    ...
    else if (G.mode === 'win') setHint('雷核尽破 · R 再来一局', 'hot');
    else if (G.lives === 1) setHint('最后一命 · Shift 爆弹清屏', 'warn');
    else setHint('方向移动 · 空格开火 · Shift 爆弹 · 捡 散/激/导 · A 自动', '');
    syncPips();
  }
  ...
  function showOverlay(kind, title, lead, primary, showSecond) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'THUN';
    ovTitle.textContent = title;
    ovLead.textContent = lead;
    ovOps.textContent = OPS;
    btnRaid.textContent = primary;
    if (btnStorm) {
      btnStorm.classList.toggle('hidden', !showSecond);
      if (kind === 'title') btnStorm.textContent = '弹雨';
      else if (kind === 'lose') btnStorm.textContent = '换模式';
      else btnStorm.textContent = isStorm() ? '换模式' : '弹雨';
    }
  }
  ...
  function spawnBlock(y) {
    ...
    const w = rand(28, 54);
    const h = rand(40, 88);
    const x = side < 0 ? rand(10, 70) : rand(VW - 70, VW - 10);
    G.blocks.push({
      x: x, y: y, w: w, h: h,
  ...
  function fire() {
    ...
    if (G.wpn === 'laser') {
      ...
      if (n === 1) {
        addShot({ x: x, y: y, vy: -820, r: 3.6, rgb: rgb, kind: 'laser', pierce: pierce, dmg: dmg });
      } else if (n === 2) {
        addShot({ x: x - 7, y: y, vy: -820, r: 3.4, rgb: rgb, kind: 'laser', pierce: pierce, dmg: dmg });
        addShot({ x: x + 7, y: y, vy: -820, r: 3.4, rgb: rgb, kind: 'laser', pierce: pierce, dmg: dmg });
      } else {
        addShot({ x: x, y: y - 2, vy: -840, r: 4.2, rgb: rgb, kind: 'laser', pierce: pierce + 1, dmg: dmg });
        addShot({ x: x - gap, y: y, vy: -800, r: 3.4, rgb: rgb, kind: 'laser', pierce: pierce, dmg: dmg });
        addShot({ x: x + gap, y: y, vy: -800, r: 3.4, rgb: rgb, kind: 'laser', pierce: pierce, dmg: dmg });
      }
    } else if (G.wpn === 'homing') {
      G.fireCd = 0.18 - lv * 0.02;
      const n = 1 + lv;
      const spd = -420 - lv * 30;
      for (let i = 0; i < n; i++) {
        const ox = (i - (n - 1) / 2) * 10;
        addShot({
          x: x + ox, y: y + Math.abs(ox) * 0.2,
          vx: ox * 8, vy: spd,
          r: 4.2, rgb: MAG, kind: 'homing', pierce: 0, dmg: 2, hom: true
        });
      }
    } else {
      ...
      function fan(ox, oy, vx, vy) {
        addShot({ x: x + ox, y: y + oy, vx: vx || 0, vy: vy == null ? spd : vy, r: 3.1, rgb: lv >= 2 ? GOLD : WHT, kind: 'spread', dmg: 1 });
      }
      if (lv <= 0) {
        fan(0, 0);
      } else if (lv === 1) {
    ...
    emit(3, {
      ...
      rgb: G.wpn === 'homing' ? MAG : G.wpn === 'laser' ? CYN : GOLD,
      g: 0
    });
  }
  ...
  function killEnt(en) {
    ...
    en.hp = -99;
    bumpCombo();
    const pwr = en.type === 'boss' ? 2.7 : en.type === 'mid' ? 2.1 : en.type === 'gun' ? 1.25 : 0.85;
    ...
    else if ((en.type === 'gun' || en.type === 'tank') && Math.random() < 0.2) spawnPow(en.x, en.y, nextDropKind());
    if (en.type === 'boss') {
      G.stageClearT = 2.05;
  ...
  function pickPow(p) {
    if (p.kind === 'bomb') {
      ...
      const kind = p.kind === 'laser' || p.kind === 'homing' ? p.kind : 'spread';
      if (G.wpn === kind) {
        if (G.wpnLv < WPN_MAX) {
          G.wpnLv += 1;
    ...
    juice(p.x, p.y, p.kind === 'laser' ? CYN : p.kind === 'homing' ? MAG : GOLD, 1.15);
    audio.pow();
    hitStop(0.038);
    floatText(p.x, p.y, DROP_GLYPH[p.kind] || '散', p.kind === 'laser' ? CYN : p.kind === 'homing' ? MAG : GOLD, true);
    syncHud();
  }
  ...
  function winGame() {
    ...
    saveBest();
    audio.win();
    showOverlay('win', '雷核尽破', (isStorm() ? '弹雨通关' : '三关打穿') + ' · 分数 ' + G.score, '再来', true);
    syncHud();
  }
  ...
  function stormThink(dt) {
    ...
    else if (r < 0.52) spawnStream(Math.random() < 0.5 ? -1 : 1);
    else if (r < 0.68) spawnDive(3 + (Math.random() * 4) | 0);
    else if (r < 0.8) spawnGun();
  ...
  function bossFire(en, storm) {
    ...
    if (en.type === 'mid') {
      ...
      eShot(en.x + 18, en.y + 10, 46, 186, PNK);
      if (mid) ringShot(en.x, en.y, storm ? 10 : 8, 150, en.spin, VIO, 3.2);
      if (low) {
      ...
      if (playing && en.y > -10 && en.y < VH + 10) {
        ...
        if (en.fireCd <= 0) {
          if (en.type === 'scout' && en.y > 18 && en.y < VH - 80) {
            ...
            eShot(en.x + 9, en.y + 12, 34, 176, RED);
            if (storm) aimShot(en.x, en.y + 10, 186, ORG);
            en.fireCd = storm ? 0.72 : 1.05;
          } else if (en.type === 'turret' && en.y > 8 && en.y < VH - 70) {
            aimShot(en.x, en.y, storm ? 214 : 174, GOLD);
            if (storm) {
            ...
            aimShot(en.x, en.y, storm ? 200 : 166, ORG);
  ...
  function updateWorld(dt) {
    ...
    if (G.boltT <= 0) {
      ...
      if (!REDUCE && G.mode !== 'lose') {
        ...
        if (Math.random() < 0.35) screenFlash(SKY, 0.12);
      }
    }
  ...
  function autoDanger(x, y, horizon) {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      ...
      let evx = en.vx || 0;
      let evy = en.vy || 0;
      if (en.type === 'dive' && en.t > 0.32) {
        const dx = x - en.x;
        const dy = y - en.y;
        const len = hypot(dx, dy) || 1;
        evx = dx / len * 178;
        evy = dy / len * 178;
      } else if (en.type === 'scout' && en.dive && en.t > 1.28) {
        evx = Math.sign(x - en.x) * 92;
        evy = Math.max(en.vy, 154);
      } else if (en.ground) {
        evx = en.type === 'tank' ? en.vx : 0;
        evy = scrollSpd();
      }
      const relx = en.x - x;
      const rely = en.y - y;
      ...
      const r = en.type === 'boss' ? en.r * 0.62 : en.type === 'mid' ? en.r * 0.68 : en.r * 0.72;
      const body = hitR + r;
      if (dist < body + 28) {
        const soon = (look - t) / Math.max(0.08, look);
        const w = en.type === 'dive' ? 36 : en.type === 'boss' || en.type === 'mid' ? 14 : 18;
        d += Math.max(0.4, body + 14 - dist) * soon * w;
        if (dist < body) d += 250 * soon;
      }
      if (hypot(en.x - x, en.y - y) < body + 8) d += 120;
    }
    return d;
  }
  ...
  function autoThink() {
    ...
    for (let i = 0; i < G.ents.length; i++) {
      const en = G.ents[i];
      if (en.hp <= 0) continue;
      if (en.y < -36 || en.y > py + 20) continue;
      let w = 32;
      if (en.type === 'dive') w = 78;
      else if (en.type === 'carrier') w = 150;
      else if (en.type === 'turret') w = 88;
      else if (en.type === 'gun') w = 96;
      else if (en.type === 'tank') w = 70;
      else if (en.type === 'mid') w = 240 + en.hp * 0.35;
      else if (en.type === 'boss') w = 280 + en.hp * 0.35;
      else w = 36 + (en.hp || 1) * 8;
      w += (en.hp || 1) * 4;
      w -= Math.abs(en.x - px) * 0.22;
      w -= Math.max(0, py - en.y) * 0.06;
      if (en.y > 40 && en.y < py - 10) w += 22;
      if (Math.abs(en.x - px) < 14 && en.y < py) colHp += en.hp || 1;
      if (en.type === 'boss' || en.type === 'mid') boss = en;
      if (w > aimW) {
        aimW = w;
        aimX = en.x;
        aimY = en.y;
    ...
    if (aimY != null) {
      for (let i = 0; i < G.ents.length; i++) {
        const en = G.ents[i];
        if (en.hp <= 0) continue;
        if (Math.abs(en.x - aimX) < 28 && en.y < py) cluster += 1;
      }
    }
  ...
  function drawBolt(b) {
    ...
    ctx.save();
    ctx.strokeStyle = rgba(WHT, a * 0.95);
  ...
  function drawWorld() {
    ...
    if (G.stage === 2) {
      ...
      g.addColorStop(0.5, '#0c1814');
      g.addColorStop(1, '#061018');
    } else if (G.stage === 3) {
      g.addColorStop(0, '#0c0820');
      g.addColorStop(0.5, '#061428');
      g.addColorStop(1, '#031018');
    } else {
      g.addColorStop(0, '#052030');
    ...
    for (let i = 0; i < stars.length; i++) {
      const s = stars[i];
      ctx.fillStyle = 'rgba(200,245,255,' + s.a + ')';
      ctx.fillRect(sx(s.x), sy(s.y), Math.max(1, s.z * scale), Math.max(1, s.z * 1.6 * scale));
    }

    const off = G.scroll % 36;
    ctx.lineWidth = 1.1 * scale;
    for (let y = -36; y < VH + 36; y += 36) {
      const yy = y + (36 - off);
      ctx.strokeStyle = 'rgba(0,232,255,' + (0.04 + ((y / 36 | 0) % 2 === 0 ? 0.025 : 0)) + ')';
      ctx.beginPath();
      for (let x = 0; x <= VW; x += 14) {
        const yy2 = yy + Math.sin((x + G.scroll) * 0.02) * 2.8;
    ...
    for (let i = 0; i < G.blocks.length; i++) {
      ...
      const w = b.w * scale;
      const h = b.h * scale;
      ctx.fillStyle = 'rgba(8, 28, 38, 0.92)';
      ctx.fillRect(x, y, w, h);
      ctx.strokeStyle = rgba(CYN, 0.18 + b.hue * 0.12);
      ctx.lineWidth = 1;
      ctx.strokeRect(x, y, w, h);
      ctx.fillStyle = rgba(b.hue > 0.55 ? GOLD : CYN, 0.22);
    ...
    for (let i = 0; i < bolts.length; i++) drawBolt(bolts[i]);
  }
  ...
  function drawShip(x, y, a) {
    ...
    ctx.shadowColor = rgba(G.wpn === 'homing' ? MAG : G.wpn === 'laser' ? CYN : GOLD, 0.55);
    ctx.shadowBlur = 12;
  ...
  function drawPows() {
    for (let i = 0; i < G.pows.length; i++) {
      ...
      const rgb = p.kind === 'laser' ? CYN : p.kind === 'homing' ? MAG : p.kind === 'bomb' ? WHT : GOLD;
      ctx.save();
  ...
  function startGame(kind) {
    ...
    syncHud();
    audio.start();
    toast(isStorm() ? '弹雨 · 更密更快' : '雷击 · 第 1 关', false, true);
  }
  ...
  function goTitle() {
    ...
    seedStars();
    seedBlocks();
    showOverlay('title', '雷电', '纵向卷轴。捡武装切散弹、激光、追踪，Shift 扔爆弹清屏。每关中 Boss 与关底 Boss。', '雷击', true);
    syncHud();
  }
  ...
  function onKey(e, down) {
    ...
    if (autoOn && (isMove || space || isBomb)) return;
    if (k === '1') {
  ...
  function bindPointer() {
    ...
    canvas.addEventListener('pointerleave', function () {
      pointer.hover = false;
      if (!autoOn && !pointer.down) G.fireHold = false;
    });
  ...
  if (btnStorm) {
    btnStorm.addEventListener('click', function () {
      ...
      else if (G.mode === 'win' && isStorm()) goTitle();
      else startGame('storm');
    });
  }
