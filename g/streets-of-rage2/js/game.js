...
(function () {
  ...
  const HURT_T = 0.36;
  const GRAB_RANGE = 26;
  const BEST_KEY = 'playbox-streets-of-rage2-best';
  const MUTE_KEY = 'playbox-streets-of-rage2-mute';
  const OPS = '方向键 / WASD 走 · 空格出拳 · Shift / Z 升龙 · R 重开 · M 静音';

  const MAG = [255, 61, 184];
  const CYN = [0, 232, 255];
  const GOLD = [255, 227, 107];
  const HOT = [255, 58, 26];
  const HOT2 = [255, 122, 82];
  ...
  const COMBO_PUNCH = [
    { dmg: 8, range: 34, dur: 0.15, hit0: 0.04, hit1: 0.12, kb: 72, stop: 0.04 },
    { dmg: 12, range: 38, dur: 0.17, hit0: 0.04, hit1: 0.13, kb: 92, stop: 0.05 },
    { dmg: 17, range: 44, dur: 0.3, hit0: 0.05, hit1: 0.18, kb: 170, stop: 0.07, stun: true }
  ];
  const SPEC_ATK = { dmg: 22, range: 58, dur: 0.44, hit0: 0.05, hit1: 0.38, kb: 210, stop: 0.08 };
  const THROW_HIT = { dmg: 18, kb: 260, stop: 0.078 };
  const SLAM_HIT = { dmg: 14, kb: 180, stop: 0.06 };

  const FOES = {
    punk: {
      hp: 26, spd: 82, dmg: 11, range: 28, score: 180, w: 16, h: 30,
      think: 0.52, punchDur: 0.3, name: '混混'
    },
    knifer: {
      hp: 22, spd: 88, dmg: 10, range: 86, score: 240, w: 15, h: 29,
      think: 0.7, punchDur: 0.36, name: '刀客', thrower: true
    },
    brute: {
      hp: 48, spd: 62, dmg: 16, range: 32, score: 320, w: 20, h: 36,
      think: 0.62, punchDur: 0.4, name: '壮汉', heavy: true
    },
    biker: {
      hp: 40, spd: 128, dmg: 15, range: 34, score: 280, w: 18, h: 32,
      think: 0.4, punchDur: 0.28, name: '飞车', charge: true
    },
    gate: {
      hp: 160, spd: 86, dmg: 18, range: 38, score: 4000, w: 22, h: 40,
      think: 0.38, punchDur: 0.36, name: '铁门', boss: true
    },
    jet: {
      hp: 220, spd: 118, dmg: 19, range: 40, score: 5200, w: 20, h: 38,
      think: 0.32, punchDur: 0.3, name: '飞影', boss: true, charge: true
    },
    don: {
      hp: 300, spd: 92, dmg: 20, range: 92, score: 9000, w: 22, h: 42,
      think: 0.36, punchDur: 0.34, name: '罪枭', boss: true, shooter: true
    }
  };

  const STAGES = [
    {
      name: '闹市', boss: '铁门', w: 1960, theme: 'street', bossKind: 'gate',
      packs: [
        { x: 200, gate: 480, foes: [['punk', 260, 262], ['punk', 340, 284]] },
        { x: 520, gate: 820, foes: [['punk', 560, 258], ['knifer', 660, 278], ['punk', 740, 268]] },
        { x: 860, gate: 1180, foes: [['punk', 900, 260], ['biker', 1000, 276], ['knifer', 1100, 254]] },
        { x: 1240, gate: 1560, foes: [['biker', 1280, 258], ['punk', 1360, 282], ['punk', 1440, 262], ['knifer', 1500, 274]] }
      ],
      drops: [[420, 268, 'food'], [980, 250, 'food'], [1380, 274, 'food']]
    },
    {
      name: '铁桥', boss: '飞影', w: 2160, theme: 'bridge', bossKind: 'jet',
      packs: [
        { x: 180, gate: 500, foes: [['biker', 240, 262], ['punk', 340, 284]] },
        { x: 540, gate: 880, foes: [['brute', 580, 260], ['biker', 680, 278], ['knifer', 780, 256]] },
        { x: 920, gate: 1280, foes: [['punk', 960, 258], ['biker', 1060, 276], ['brute', 1160, 286], ['knifer', 1220, 254]] },
        { x: 1340, gate: 1740, foes: [['biker', 1380, 258], ['brute', 1480, 282], ['punk', 1580, 264], ['biker', 1680, 276]] }
      ],
      drops: [[400, 270, 'food'], [860, 252, 'food'], [1280, 288, 'food'], [1600, 264, 'food']]
    },
    {
      name: '罪巢', boss: '罪枭', w: 2360, theme: 'nest', bossKind: 'don',
      packs: [
        { x: 160, gate: 500, foes: [['punk', 220, 262], ['biker', 320, 280], ['knifer', 400, 256]] },
        { x: 540, gate: 920, foes: [['brute', 580, 256], ['biker', 680, 278], ['punk', 780, 264], ['knifer', 860, 272]] },
        { x: 980, gate: 1400, foes: [['punk', 1020, 258], ['brute', 1120, 274], ['biker', 1220, 286], ['knifer', 1320, 254]] },
        { x: 1460, gate: 1900, foes: [['biker', 1500, 258], ['brute', 1600, 282], ['punk', 1700, 264], ['biker', 1780, 276], ['knifer', 1860, 256]] }
      ],
      drops: [[380, 268, 'food'], [760, 250, 'food'], [1180, 290, 'food'], [1540, 262, 'food'], [1760, 278, 'food']]
    }
  ];
  ...
  function selfCheck() {
    ...
    if (!FOES.punk || !FOES.knifer || !FOES.brute || !FOES.biker) throw new Error('foes');
    if (!FOES.gate || !FOES.jet || !FOES.don) throw new Error('bosses');
    if (!FOES.knifer.thrower) throw new Error('knifer throw');
    if (!FOES.biker.charge) throw new Error('biker charge');
    if (!FOES.don.shooter) throw new Error('don shoot');
    if (FOES.gate.hp >= FOES.jet.hp || FOES.jet.hp >= FOES.don.hp) throw new Error('boss hp');
    if (STAGES[0].w >= STAGES[1].w || STAGES[1].w >= STAGES[2].w) throw new Error('wider later');
    if (BEST_KEY !== 'playbox-streets-of-rage2-best') throw new Error('best key');
    if (MUTE_KEY !== 'playbox-streets-of-rage2-mute') throw new Error('mute key');
    ...
    if (STAGES[0].bossKind !== 'gate' || STAGES[2].bossKind !== 'don') throw new Error('crime-boss last');
  ...
  function lookOf(kind) {
    ...
    if (kind === 'punk') {
      return { jacket: GOLD, pants: [36, 32, 44], hair: MAG, skin: SKIN, accent: MAG, hairStyle: 'mohawk', size: 1 };
    }
    if (kind === 'knifer') {
      return { jacket: [180, 48, 120], pants: [40, 24, 48], hair: [40, 18, 28], skin: [228, 170, 140], accent: MAG, hairStyle: 'pony', size: 0.96 };
    }
    if (kind === 'brute') {
      return { jacket: [200, 90, 48], pants: [36, 28, 32], hair: [24, 18, 16], skin: [196, 140, 108], accent: GOLD, hairStyle: 'bald', size: 1.22 };
    }
    if (kind === 'biker') {
      return { jacket: [32, 28, 38], pants: [22, 20, 28], hair: [20, 18, 22], skin: [210, 160, 120], accent: HOT, hairStyle: 'helm', size: 1.04 };
    }
    if (kind === 'gate') {
      return { jacket: [200, 40, 48], pants: [28, 28, 36], hair: [24, 20, 18], skin: [196, 140, 108], accent: GOLD, hairStyle: 'bald', size: 1.24 };
    }
    if (kind === 'jet') {
      return { jacket: [24, 140, 150], pants: [20, 28, 36], hair: [16, 18, 22], skin: [210, 160, 128], accent: CYN, hairStyle: 'slick', size: 1.14 };
    }
  ...
  function aliveFoes() {
    ...
    for (i = 0; i < G.enemies.length; i++) {
      if (!G.enemies[i].dead) out.push(G.enemies[i]);
    }
    return out;
  }
  ...
  function hurtEnemy(e, dmg, kb, face, finisher, src) {
    ...
    e.act = e.hp <= 0 ? 'down' : (finisher ? 'hurt' : 'hurt');
    e.t = e.hp <= 0 ? 0.55 : 0.3;
    e.vx = face * kb;
    e.vy = 0;
    e.charge = 0;
  ...
  function specialHits(p) {
    ...
    for (i = 0; i < G.enemies.length; i++) {
      ...
      d = (e.x - p.x) * face;
      if (d < -8 || d > SPEC_ATK.range) continue;
  ...
  function startSpecial(p) {
    ...
    shake(10);
    boomKick();
    ringAt(p.x + p.face * 18, p.y - 18, CYN);
    burst(p.x, p.y - 16, 22, CYN, 220, 0.4);
    ghostAt(p, 0.5);
  }
  ...
  function tickPlayer(dt) {
    ...
    if (p.act === 'special') {
      ...
      if ((G.clock * 18) % 1 < dt * 18) ghostAt(p, 0.28);
      const mid = SPEC_ATK.dur * 0.45;
      if (!p.hit && p.t < SPEC_ATK.dur - SPEC_ATK.hit0 && p.t > SPEC_ATK.dur - SPEC_ATK.hit1) {
        ...
        if (n > 0) {
          ...
          shake(9);
          kick(p.face * 7, 3);
          burst(p.x + p.face * 28, p.y - 22, 16, GOLD, 200, 0.36);
          spark(p.x + p.face * 28, p.y - 22, CYN, 10);
          audio.hit(G.combo, true);
        }
      }
      if (p.hit && p.t < mid) p.hit = false;
      clampBelt(p);
      if (p.t <= 0) p.act = 'idle';
      return;
    }
  ...
  function tickFly(e, dt) {
    ...
    if (G.mode === 'play' && G.player && !G.player.grab && G.deadT <= 0) {
      const p = G.player;
      if (depthHit(e, p) && Math.abs(e.x - p.x) < 18 && G.invuln <= 0 && p.act !== 'special') {
        /* thrown bodies don't hit the thrower */
      }
    }
    if (e.flyT <= 0) {
      e.act = 'hurt';
  ...
  function tickFoe(e, dt) {
    ...
    if (e.act === 'throw') {
      ...
      if (!e.hit && e.t < 0.18) {
        e.hit = true;
        G.shots.push(makeShot(e.x + e.face * 16, e.y, e.face, e.spec.shooter ? 'gun' : 'knife'));
  ...
  function tickShots(dt) {
    ...
    for (i = G.shots.length - 1; i >= 0; i--) {
      ...
      hurtPlayer(s.kind === 'gun' ? 16 : 12, s.face, s.kind === 'gun' ? '中弹了' : '中刀了');
    }
  }
  ...
  function extraFoes(list) {
    ...
    for (i = 0; i < list.length; i++) {
      ...
      out.push([f[0] === 'knifer' ? 'punk' : f[0], f[1] + 36, y]);
    }
    return out;
  }
  ...
  function showTitle() {
    ...
    const d2 = makeFoe('knifer', 360, 300, 1, 1);
    G.enemies = [d1, d2];
    panel.classList.remove('win', 'lose');
    ovKicker.textContent = 'SOR2';
    ovTitle.textContent = '怒二';
    ovLead.innerHTML = '沿街清场。出拳连击，贴近抓住再甩出去。升龙耗能。<br />打穿三街，最后是罪枭。';
    ovOps.textContent = OPS;
    ovStart.classList.remove('gone');
    ovEnd.classList.add('gone');
    showOverlay();
    setHint('走出拳 · 抓住甩 · 升龙清场 · 血空丢命');
    G.hudDirty = true;
  ...
  function startRun(kind) {
    ...
    hideOverlay();
    audio.start();
    toast(G.kind === 'core' ? '怒核' : '怒二', false, G.kind === 'core');
    setHint(G.kind === 'core' ? '怒核更密 · 空格拳 · Shift 升龙' : '空格出拳 · 抓住甩 · Shift 升龙');
    G.hudDirty = true;
  ...
  function showOver(win) {
    ...
    ovTitle.textContent = win ? (G.kind === 'core' ? '核清了' : '罪巢清了') : (G.why || '被打倒了');
    const extra = '第 ' + G.stage + ' 街 · ';
  ...
  function tickDemo(dt) {
    ...
    if (p.grab && p.act === 'grab') {
      ...
      if (p.t <= 0) doThrow(p);
    } else if (p.act === 'punch' || p.act === 'throw' || p.act === 'special') {
      tickPlayer(dt);
    } else {
      p.face = 1;
      p.x += 36 * dt;
      ...
      p.act = 'walk';
      if (((G.clock * 1.15) | 0) !== (((G.clock - dt) * 1.15) | 0)) {
        p.punchBuf = 0.1;
        startPunch(p);
      }
    }
  ...
  function drawSky() {
    ...
    } else if (G.theme === 'nest') {
      g = ctx.createLinearGradient(0, oy, 0, oy + 210 * scale);
      g.addColorStop(0, '#10060a');
      g.addColorStop(1, '#241018');
    } else {
      g = ctx.createLinearGradient(0, oy, 0, oy + 210 * scale);
      g.addColorStop(0, '#0c0412');
      g.addColorStop(1, '#1c0814');
    }
    ctx.fillStyle = g;
    ctx.fillRect(ox, oy, VW * scale, VH * scale);
    ctx.fillStyle = rgba(GOLD, G.theme === 'nest' ? 0.1 : 0.2);
    ctx.beginPath();
    ctx.arc(wx(G.camX * 0.18 + 520), wy(44), 16 * scale, 0, TAU);
    ctx.fill();
  }
  ...
  function drawBuildings() {
    ...
    for (i = start; i < start + 16; i++) {
      hsh = hash2(i * 17 + (G.theme === 'bridge' ? 5 : G.theme === 'nest' ? 11 : 2));
      bx = i * 70 - G.camX * 0.34;
      bw = 48 + hsh * 28;
      bh = 72 + hsh * 88;
      if (G.theme === 'bridge') {
        ctx.fillStyle = '#101820';
        ctx.fillRect(wx(G.camX + bx), wy(210 - bh * 0.45), bw * scale, bh * 0.45 * scale);
        ctx.strokeStyle = 'rgba(0, 232, 255, 0.18)';
        ctx.lineWidth = 1.4 * scale;
        ctx.beginPath();
        ctx.moveTo(wx(G.camX + bx), wy(70));
        ctx.lineTo(wx(G.camX + bx + bw * 0.5), wy(40));
        ctx.lineTo(wx(G.camX + bx + bw), wy(70));
        ctx.stroke();
      } else {
        ctx.fillStyle = G.theme === 'nest' ? '#16080c' : '#120610';
    ...
    if (G.theme === 'bridge') {
      ctx.fillStyle = 'rgba(16, 40, 56, 0.5)';
      ctx.fillRect(ox, wy(186), VW * scale, 30 * scale);
      ctx.strokeStyle = 'rgba(0, 232, 255, 0.14)';
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(ox, wy(198 + Math.sin(G.clock * 1.3) * 2));
      ctx.lineTo(ox + VW * scale, wy(198 + Math.sin(G.clock * 1.3 + 1) * 2));
      ctx.stroke();
    }
    if (G.theme === 'nest') {
  ...
  function drawFighter(e, look, opts) {
    ...
    if (!down && !fly) {
      ctx.fillRect(-6 * s + walk * 3 * s, spec ? -16 * s : -10 * s, 4.2 * s, spec ? 10 * s : 12 * s);
      ctx.fillRect(1.4 * s - walk * 3 * s, spec ? -16 * s : -10 * s, 4.2 * s, spec ? 10 * s : 12 * s);
    } else {
      ctx.fillRect(-10 * s, -6 * s, 14 * s, 5 * s);
    }
    ctx.fillStyle = '#d8dce8';
    if (!down && !fly) {
      ctx.fillRect(-6.2 * s + walk * 3 * s, spec ? -8 * s : 0, 4.4 * s, 3 * s);
      ctx.fillRect(1.2 * s - walk * 3 * s, spec ? -8 * s : 0, 4.4 * s, 3 * s);
    }
    ...
    if (punch) {
      ...
      ctx.fillRect(8 * s, armY - 16 * s, 4.2 * s, 16 * s);
      ctx.fillRect(-12 * s, armY + 2 * s, 7 * s, 3.4 * s);
      ctx.fillStyle = rgba(CYN, 0.85);
      ctx.beginPath();
      ctx.arc(12 * s, armY - 16 * s, 5 * s, 0, TAU);
      ctx.fill();
    } else if (grab && e.kind === 'player') {
  ...
  function drawIntro() {
    ...
    ctx.fillStyle = 'rgba(14,4,6,' + (0.55 * a) + ')';
