...
(function () {
  ...
  const WPN_MAX = 4;
  const LOOP_MAX = 3;
  const CHARGE_TIME = 0.78;
  const CHARGE_AUTO = 0.2;
  const BEST_KEY = 'playbox-n1944-best';
  const MUTE_KEY = 'playbox-n1944-mute';
  const OPS = '方向 / WASD 飞 · 空格射击 · Shift / Z 蓄环环爆 · R 重开 · M 静音';
  ...
  const WHT = [244, 236, 232];
  const PNK = [255, 140, 160];
  const RED = [255, 78, 72];
  const ORG = [255, 110, 80];
  const SEA = [96, 40, 40];
  const REEF = [120, 56, 44];
  const STORM = [56, 24, 32];
  const STEEL = [168, 124, 116];
  const CREAM = [255, 244, 220];
  const SKY = [255, 138, 106];
  ...
  function biome() {
    ...
    return st ? st.biome : 'dusk';
  }
  function stageInfo() {
    return STAGES[clamp(G.stage - 1, 0, STAGES.length - 1)];
  }
  function plySpd() {
    return (isCore() ? 310 : 276) + G.powLv * 8;
  }
  function scrollSpd() {
    if (hasBig()) return isCore() ? 32 : 24;
  ...
  const audio = {
    ...
    chargeTick() {
      this.ensure();
      this.beep(240 + G.charge * 620, 0.038, 'sine', 0.02, 380 + G.charge * 780);
    },
    chargeFull() {
      this.ensure();
    ...
    loopBomb(full) {
      this.ensure();
      this.noise(full ? 0.22 : 0.14, full ? 0.08 : 0.055, full ? 160 : 280);
      this.beep(full ? 140 : 210, full ? 0.32 : 0.2, 'sawtooth', 0.055, 48);
      this.beep(full ? 420 : 560, full ? 0.18 : 0.12, 'square', 0.04, 180);
    },
    graze() {
      this.ensure();
      this.beep(1020, 0.038, 'sine', 0.026, 1560);
    },
    hit(kind, combo) {
      this.ensure();
      ...
      if (kind === 'ground') {
        ...
        this.beep(560 * lift, 0.064, 'square', 0.042, 940 * lift);
      }
    },
    boom(big) {
      this.ensure();
      this.noise(big ? 0.22 : 0.1, big ? 0.08 : 0.048, big ? 200 : 440);
      this.beep(big ? 150 : 240, big ? 0.28 : 0.14, 'sawtooth', 0.05, 50);
    },
    pow() {
      this.ensure();
    ...
    death() {
      ...
      this.beep(130, 0.32, 'sine', 0.044, 40);
    },
    boss() {
      this.ensure();
  ...
  function wpnText() {
    if (G.powLv >= WPN_MAX) return '火 MAX';
    if (G.powLv <= 0) return '火';
    return '火 ' + WPN_ROMAN[G.powLv];
  }
  ...
  function syncChargeHud() {
    const ch = G.charging ? G.charge : (looping() ? G.bombCharge : 0);
    if (chgBar) chgBar.style.transform = 'scaleX(' + clamp(ch, 0, 1) + ')';
    if (chgWrap) chgWrap.classList.toggle('hot', G.charging && G.charge >= 0.92);
  }
  ...
  function syncHud() {
    ...
    if (tagLabel) {
      tagLabel.textContent = isCore() ? '海核' : '四四';
    ...
    const held = G.charging || looping();
    if (btnLoop) btnLoop.classList.toggle('held', held);
    if (btnPadLoop) btnPadLoop.classList.toggle('held', held);
    syncChargeHud();
    if (G.mode === 'title') setHint(OPS, '');
    else if (G.mode === 'lose') setHint('R 重开 · 撞机或中弹扣一命', 'warn');
    else if (G.mode === 'win') setHint('联合舰队击沉 · R 再来一局', 'hot');
    else if (G.lives === 1) setHint('最后一命 · 空格射击 · 按住 Shift 蓄环', 'warn');
    else setHint('空格扫空打舰 · 按住 Shift 蓄环松开环爆 · 吃 火 加宽', '');
    syncPips();
  }
  ...
  function showOverlay(kind, title, lead) {
    ...
    ovKicker.textContent = kind === 'lose' ? 'DOWN' : kind === 'win' ? 'CLEAR' : 'N144';
  ...
  function spawnKami(n) {
    n = n || 3;
    for (let i = 0; i < n; i++) {
      const left = i % 2 === 0;
      spawnEnt({
        type: 'kami',
        x: left ? rand(30, 120) : rand(VW - 120, VW - 30),
        y: -28 - i * 22,
        vx: left ? 40 : -40,
        vy: 70,
        hp: 1, r: 11, score: SCORE.kami,
        rgb: HOT,
        dive: true,
        fireCd: 99
      });
  ...
  function spawnEscort(ox, y, hp) {
    spawnEnt({
      ...
      h: 18,
      fireCd: rand(0.5, 0.9),
      drop: Math.random() < 0.4 ? 'loop' : false
    });
  }
  ...
  function spawnMid() {
    ...
    if (G.stage === 1) {
      spawnEscort(-78, -58, Math.round(8 * hpMul()));
      spawnEscort(78, -58, Math.round(8 * hpMul()));
    } else if (G.stage === 3) {
      spawnEscort(-92, -50, Math.round(12 * hpMul()));
      spawnEscort(92, -50, Math.round(12 * hpMul()));
    }
  ...
  function spawnBoss() {
    ...
    if (G.stage === 1) {
      spawnEscort(-96, -64, Math.round(10 * hpMul()));
      spawnEscort(96, -64, Math.round(10 * hpMul()));
    } else if (G.stage === 2) {
      spawnEscort(-108, -70, Math.round(14 * hpMul()));
      spawnEscort(108, -70, Math.round(14 * hpMul()));
    } else {
      spawnEscort(-118, -62, Math.round(16 * hpMul()));
      spawnEscort(118, -62, Math.round(16 * hpMul()));
      spawnEnt({
        type: 'carrier',
        x: VW * 0.5,
        y: -130,
        vx: 0, vy: 36,
        hp: Math.round(14 * hpMul()),
        r: 28, score: SCORE.carrier,
        rgb: STEEL,
        ground: true,
        fleet: true,
        ox: 0,
  ...
  function fleetLead() {
    for (let i = 0; i < G.ents.length; i++) {
      const t = G.ents[i].type;
      if ((t === 'boss' || t === 'mid') && G.ents[i].hp > 0) return G.ents[i];
    }
    return null;
  }
  ...
  function fireWave(w) {
    ...
    else if (w.kind === 'kami') spawnKami(w.n);
  ...
  function wallShot(y, n, spd, rgb) {
    const gap = VW / (n + 1);
    for (let i = 1; i <= n; i++) {
      eShot(gap * i, y, 0, spd, rgb, 3.2);
    }
  }
  ...
  function releaseLoop() {
    ...
    const ch = clamp(G.charge, 0.12, 1);
    G.charging = false;
    G.loops -= 1;
    G.bombCharge = ch;
    G.loopDur = 0.52 + ch * 0.38;
    G.loopT = G.loopDur;
    G.loopAng = 0;
    G.bombT = G.loopDur * 0.38;
    G.bombKills = 0;
    G.invuln = Math.max(G.invuln, G.loopDur);
    G.charge = 0;
    G.chargeFullT = 0;
    flashLoopHud();
    const full = ch >= 0.92;
    audio.loopBomb(full);
    screenFlash(full ? GOLD : HOT, full ? 0.58 : 0.42);
    hitStop(full ? 0.062 : 0.046);
    kick(full ? 5.8 : 4.2);
    if (stageEl && !REDUCE) {
    ...
    popSpark(G.player.x, G.player.y, full ? GOLD : HOT, 20 + ch * 12);
    toast(full ? '环圣' : (ch >= 0.55 ? '环爆' : '翻环'), false, true);
    syncHud();
  }

  function detonateBomb() {
    const ch = G.bombCharge;
    const full = ch >= 0.92;
    const mid = ch >= 0.55;
    const rad = full ? 280 : (48 + ch * 190);
    const px = G.player.x;
    const py = G.player.y;
    const dmgAir = full ? 6 : mid ? 4 : 2;
    const dmgGround = full ? 8 : mid ? 6 : 3;
    const dmgBoss = full ? 12 : mid ? 7 : 3;
    rings.push({ x: px, y: py, t: 0, rgb: full ? GOLD : HOT, r: 22 + ch * 18 });
    rings.push({ x: px, y: py, t: -0.08, rgb: GOLD, r: 16 });
    emit(18 + (ch * 16) | 0, {
      x: px, y: py, j: 14 + ch * 20,
      vx0: -220, vx1: 220, vy0: -240, vy1: 120,
      life: 0.4 + ch * 0.18, r0: 1.4, r1: 3.6, rgb: full ? GOLD : HOT, g: 0
    });
    for (let k = 0; k < 10; k++) {
      const a = (k / 10) * TAU;
      emit(3, {
        x: px + Math.cos(a) * rad * 0.35,
        y: py + Math.sin(a) * rad * 0.35,
        j: 6,
        vx0: Math.cos(a) * 80, vx1: Math.cos(a) * 180,
        vy0: Math.sin(a) * 80, vy1: Math.sin(a) * 180,
        life: 0.28, r0: 1.2, r1: 2.8, rgb: GOLD, g: 0
      });
    }
    screenFlash(full ? GOLD : HOT, full ? 0.55 : 0.32);
    hitStop(full ? 0.072 : 0.048);
    kick(full ? 6.4 : 4.6);
    ...
    if (mid) {
      for (let i = G.eShots.length - 1; i >= 0; i--) {
        ...
        const dx = s.x - px;
        const dy = s.y - py;
        if (dx * dx + dy * dy < rad * rad) G.eShots.splice(i, 1);
      }
    }
    ...
    for (let i = 0; i < G.ents.length; i++) {
      ...
      const dx = en.x - px;
      const dy = en.y - py;
      if (!full && dx * dx + dy * dy > rad * rad) continue;
      let dmg = en.ground ? dmgGround : dmgAir;
      if (en.type === 'boss' || en.type === 'mid') dmg = dmgBoss;
      const before = en.hp;
      hurtEnt(en, dmg, en.x, en.y);
      if (before > 0 && en.hp <= 0) G.bombKills += 1;
    }
    if (G.bombKills >= 4) {
      floatText(px, py - 40, '环圣 ×' + G.bombKills, GOLD, true);
      audio.combo(Math.min(5, 1 + (G.bombKills / 3) | 0));
    }
  }

  function finishLoop() {
    if (G.bombKills >= 3) {
      toast('环爆 ×' + G.bombKills, false, true);
    }
    G.loopT = 0;
    G.bombT = 0;
    syncHud();
  ...
  function killFleetEscorts() {
    for (let i = 0; i < G.ents.length; i++) {
      const en = G.ents[i];
      if (en.hp > 0 && en.fleet && en.type !== 'boss' && en.type !== 'mid') {
        en.hp = 0;
        killEnt(en);
      }
    }
  ...
  function killEnt(en) {
    ...
    en.hp = -99;
    bumpCombo();
    const pwr = en.type === 'boss' ? 2.8 : en.type === 'mid' ? 2.2 : en.ground ? 1.2 : 0.85;
    ...
    else if ((en.type === 'ship' || en.type === 'carrier' || en.type === 'escort') && Math.random() < 0.22) {
      spawnPow(en.x, en.y, 'p');
    }
    if (en.type === 'boss') {
      killFleetEscorts();
      G.stageClearT = 2.05;
  ...
  function pickPow(p) {
    if (p.kind === 'loop') {
      ...
      G.powLv += 1;
      toast(G.powLv >= WPN_MAX ? '火 MAX' : '火加宽', false, true);
    ...
    audio.pow();
    hitStop(0.038);
    floatText(p.x, p.y, p.kind === 'loop' ? '环' : '火', GOLD, true);
    if (stageEl && !REDUCE) {
  ...
  function winGame() {
    ...
    saveBest();
    audio.win();
    showOverlay('win', isCore() ? '海核尽破' : '联合舰队击沉', (isCore() ? '海核通关' : '三关打穿') + ' · 分数 ' + G.score);
    syncHud();
  }
  ...
  function coreThink(dt) {
    ...
    G.spawnT = clamp(1.32 / (1 + G.stage * 0.12), 0.34, 1.32);
    if (livingAir() > 24) return;
    const r = Math.random();
    if (r < 0.2) spawnV(5 + (Math.random() * 6) | 0);
    else if (r < 0.34) spawnStream(Math.random() < 0.5 ? -1 : 1);
    else if (r < 0.44) spawnDive(3 + (Math.random() * 4) | 0);
    else if (r < 0.56) spawnKami(3 + (Math.random() * 3) | 0);
    else if (r < 0.66) spawnBoats();
    else if (r < 0.76) spawnTurretWave();
    else if (r < 0.84) spawnShip();
  ...
  function bossFire(en, dens) {
    ...
    if (en.type === 'mid') {
      aimShot(en.x, en.y + 10, dens ? 200 : 168, HOT);
      eShot(en.x - 20, en.y + 6, -40, 176, PNK);
      eShot(en.x + 20, en.y + 6, 40, 176, PNK);
      if (mid) ringShot(en.x, en.y, dens ? 9 : 7, 142, en.spin, GOLD, 3.2);
      if (low) {
        aimShot(en.x - 24, en.y + 4, 190, MAG);
        aimShot(en.x + 24, en.y + 4, 190, MAG);
      }
      en.fireCd = low ? 0.36 : mid ? 0.5 : 0.66;
    } else if (stg === 1) {
      aimShot(en.x, en.y + 12, 204, MAG);
      eShot(en.x - 30, en.y + 8, -46, 186, RED);
      eShot(en.x + 30, en.y + 8, 46, 186, RED);
      if (mid) ringShot(en.x, en.y + 4, dens ? 11 : 8, 132, en.spin, PNK, 3.1);
      if (low) wallShot(en.y + 16, dens ? 9 : 7, 198, HOT);
      en.fireCd = low ? 0.3 : mid ? 0.44 : 0.58;
    } else if (stg === 2) {
      ringShot(en.x, en.y + 6, dens ? 13 : 10, 140, en.spin, MAG, 3.15);
      if (mid) {
        ringShot(en.x, en.y + 6, dens ? 9 : 7, 112, -en.spin * 1.4, GOLD, 3.0);
        aimShot(en.x, en.y + 12, 192, ORG);
      }
      if (low) {
        wallShot(en.y + 14, dens ? 11 : 8, 188, HOT);
        aimShot(en.x - 28, en.y + 8, 210, RED);
        aimShot(en.x + 28, en.y + 8, 210, RED);
      }
      en.fireCd = low ? 0.32 : mid ? 0.46 : 0.6;
    } else {
      ringShot(en.x, en.y + 4, dens ? 15 : 11, 148, en.spin, MAG, 3.2);
      ringShot(en.x, en.y + 4, dens ? 9 : 7, 104, -en.spin * 0.7, HOT, 2.8);
      if (mid) {
        aimShot(en.x - 22, en.y + 10, 200, PNK);
        aimShot(en.x + 22, en.y + 10, 200, PNK);
        wallShot(en.y + 18, dens ? 8 : 6, 176, ORG);
      }
      if (low) {
        if (!en.volleyTold) {
          en.volleyTold = true;
          toast('舰队齐射', true, false);
        }
        wallShot(en.y + 12, dens ? 13 : 10, 186, GOLD);
        ringShot(en.x, en.y, dens ? 16 : 12, 160, en.t * 3.2, MAG, 3.4);
      }
  ...
  function updateEnts(dt) {
    ...
    for (let i = G.ents.length - 1; i >= 0; i--) {
      ...
      if (en.fleet && en.type !== 'mid' && en.type !== 'boss' && lead) {
        const park = lead.type === 'boss' ? 118 : 132;
        const ty = lead.y + (en.type === 'carrier' ? -42 : 18);
        if (lead.y < park) en.y += lead.vy * dt;
        else {
          en.y = lerp(en.y, ty, 1 - Math.exp(-dt * 4));
          en.x = lerp(en.x, lead.x + en.ox, 1 - Math.exp(-dt * 3.2));
        }
      } else if (en.ground && en.type !== 'mid' && en.type !== 'boss') {
        en.y += scr * dt;
        if (en.type === 'boat' || en.type === 'ship' || en.type === 'carrier' || en.type === 'escort') {
          en.x += en.vx * dt;
          const pad = en.type === 'carrier' ? 70 : en.type === 'ship' || en.type === 'escort' ? 54 : 40;
      ...
      } else if (en.type === 'kami') {
        if (en.t > 0.22) {
          const dx = px - en.x;
          const dy = py - en.y;
          const len = hypot(dx, dy) || 1;
          const asp = 248;
          en.vx = lerp(en.vx, dx / len * asp, 1 - Math.exp(-dt * 4.2));
          en.vy = lerp(en.vy, dy / len * asp, 1 - Math.exp(-dt * 4.2));
        }
        en.x += en.vx * dt;
        en.y += en.vy * dt;
      ...
      if (en.y > VH + 52 || en.x < -72 || en.x > VW + 72 || (en.ground && en.y > VH + 42 && !en.fleet)) {
        G.ents.splice(i, 1);
        continue;
      }
      ...
      if (playing && en.y > -10 && en.y < VH + 10) {
        ...
        if (en.fireCd <= 0) {
          if (en.type === 'fighter' && en.y > 18 && en.y < VH - 80) {
            ...
            eShot(en.x - 14, en.y + 6, -28, 150, HOT);
            eShot(en.x + 14, en.y + 6, 28, 150, HOT);
          ...
          } else if ((en.type === 'carrier' || en.type === 'escort') && en.y > 8 && en.y < VH - 70) {
            eShot(en.x - 16, en.y + 8, -28, 164, STEEL);
            eShot(en.x + 16, en.y + 8, 28, 164, STEEL);
      ...
      if (looping()) {
        ...
        if (dx * dx + dy * dy < 40 * 40) {
          emit(3, {
            ...
            life: 0.14, r0: 1, r1: 2.2, rgb: GOLD, g: 0
          });
          audio.graze();
          G.eShots.splice(i, 1);
          continue;
  ...
  function updateFx(dt) {
    ...
    for (let i = orbs.length - 1; i >= 0; i--) {
      orbs[i].t += dt;
      if (orbs[i].t >= orbs[i].life) orbs.splice(i, 1);
    }
  ...
  function updateCharge(dt) {
    if (!G.charging || G.mode !== 'play' || G.deadT > 0) return;
    const prev = G.charge;
    G.charge = clamp(G.charge + dt / CHARGE_TIME, 0, 1);
    if (G.charge >= 0.22 && ((G.t * 14) | 0) !== (((G.t - dt) * 14) | 0)) audio.chargeTick();
    ...
    if (G.charge >= 1) {
      G.chargeFullT += dt;
      if (G.chargeFullT >= CHARGE_AUTO) releaseLoop();
    }
    if (!REDUCE) {
      orbs.push({
        x: G.player.x + Math.cos(G.t * 8) * (16 + G.charge * 14),
        y: G.player.y + Math.sin(G.t * 8) * (12 + G.charge * 10),
        t: 0, life: 0.28, rgb: G.charge >= 0.92 ? GOLD : HOT
      });
      capArr(orbs, 16);
    }
    syncChargeHud();
  ...
  function updatePlayer(dt) {
    ...
    if (G.deadT > 0) return;
    const spd = plySpd() * (looping() ? 0.55 : G.charging ? 0.82 : 1);
    let dx = 0;
    let dy = 0;
    if (keys.l) dx -= 1;
    if (keys.r) dx += 1;
    ...
    if (looping() && !REDUCE) {
      const p = 1 - G.loopT / G.loopDur;
      const rad = (18 + G.bombCharge * 16) * Math.sin(p * Math.PI);
    ...
    if ((looping() || G.charging) && !REDUCE) {
      ghosts.push({
        x: G.player.x, y: G.player.y,
        ang: looping() ? G.loopAng : 0, t: 0, life: 0.2, bank: G.player.bank
      });
      capArr(ghosts, 10);
      trails.push({
        x: G.player.x, y: G.player.y,
        t: 0, life: 0.34, rgb: G.bombCharge >= 0.92 || G.charge >= 0.92 ? GOLD : HOT
      });
      capArr(trails, 18);
    }
  }
  ...
  function update(dt) {
    ...
    if (G.mode === 'title') {
      ...
      G.charge = 0.45 + Math.sin(G.t * 2.2) * 0.2;
      G.spawnT -= dt;
    ...
    if (G.loopT > 0) {
      G.loopT -= dt;
      G.loopAng += dt * (REDUCE ? 7 : 15);
      if (G.bombT > 0) {
        G.bombT -= dt;
        if (G.bombT <= 0) detonateBomb();
      }
  ...
  function drawIsland(isl, bio) {
    ...
    ctx.save();
    if (bio === 'storm') {
      ctx.fillStyle = 'rgba(36, 14, 18, 0.94)';
      ctx.beginPath();
      ctx.ellipse(x, y, w * 0.55, h * 0.5, 0, 0, TAU);
      ctx.fill();
      ctx.fillStyle = rgba(STORM, 0.5);
      ctx.beginPath();
      ...
      ctx.fillStyle = 'rgba(48, 22, 18, 0.9)';
      ctx.beginPath();
      ctx.ellipse(x, y, w * 0.52, h * 0.4, 0, 0, TAU);
      ctx.fill();
      ctx.strokeStyle = rgba(REEF, 0.55);
      ctx.lineWidth = 3 * scale;
      ctx.beginPath();
      ctx.ellipse(x, y, w * 0.38, h * 0.26, 0, 0, TAU);
      ctx.stroke();
      ctx.fillStyle = 'rgba(210, 150, 110, 0.28)';
      ctx.beginPath();
      ctx.ellipse(x, y, w * 0.22, h * 0.14, 0, 0, TAU);
      ctx.fill();
    } else {
      ctx.fillStyle = 'rgba(42, 16, 16, 0.88)';
      ctx.beginPath();
  ...
  function drawWorld() {
    ...
    if (bio === 'storm') {
      ...
      g.addColorStop(0, '#28100c');
      g.addColorStop(0.5, '#1a0a08');
      g.addColorStop(1, '#120606');
    } else {
      g.addColorStop(0, '#2a0c0a');
      g.addColorStop(0.55, '#180808');
      g.addColorStop(1, '#140404');
    }
    ctx.fillStyle = g;
    ctx.fillRect(sx(0), sy(0), VW * scale, VH * scale);

    ctx.save();
    ctx.strokeStyle = rgba(bio === 'storm' ? STORM : HOT, bio === 'storm' ? 0.14 : 0.12);
    ctx.lineWidth = 1;
    ...
    for (let i = 0; i < foam.length; i++) {
      const s = foam[i];
      ctx.fillStyle = rgba(WHT, s.a * (bio === 'storm' ? 0.18 : 0.38));
    ...
    for (let i = 0; i < clouds.length; i++) {
      const c = clouds[i];
      ctx.fillStyle = rgba(bio === 'storm' ? PNK : WHT, c.a * (isCore() ? 1.25 : 0.9) * (bio === 'storm' ? 0.45 : 1));
      ctx.beginPath();
    ...
    for (let i = 0; i < wash.length; i++) {
      const w = wash[i];
      ctx.strokeStyle = rgba(HOT, (1 - w.t) * 0.28);
  ...
  function drawAce(x, y, a, flashHit, ang, bank) {
    ...
    const lp = looping();
    const ch = G.charging;
    const sc = lp && !REDUCE ? 1.18 + Math.sin(G.loopAng * 2) * 0.1 : 1;
    ctx.scale(scale * sc, scale * sc);
    ctx.globalAlpha = a == null ? 1 : a;
    const flash = flashHit || G.muzzle > 0;
    const body = flash ? WHT : (ch && G.charge >= 0.92 ? GOLD : HOT);
    ctx.shadowColor = rgba(lp || ch ? GOLD : body, lp ? 0.85 : 0.55);
    ctx.shadowBlur = lp || ch ? 16 : 11;
    ctx.strokeStyle = rgba(body, 0.96);
    ctx.lineWidth = 2.1;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.moveTo(-16, 2);
    ctx.quadraticCurveTo(-10, -6, -4, 0);
    ctx.moveTo(16, 2);
    ctx.quadraticCurveTo(10, -6, 4, 0);
    ctx.stroke();
    ctx.lineWidth = 2.3;
    ...
    ctx.beginPath();
    ctx.ellipse(0, -6, 2.1, 3.1, 0, 0, TAU);
    ctx.fill();
    ...
    ctx.beginPath();
    ctx.arc(-1.6, 8, 1.1, 0, TAU);
    ctx.arc(1.6, 8, 1.1, 0, TAU);
    ctx.fill();
    if (G.powLv >= 2) {
      ctx.fillStyle = rgba(SKY, 0.9);
      ctx.beginPath();
      ctx.arc(-14, 4, 2.2, 0, TAU);
      ctx.arc(14, 4, 2.2, 0, TAU);
      ctx.fill();
    }
    if (G.muzzle > 0) {
    ...
    if (lp || ch) {
      const rad = 16 + (ch ? G.charge * 10 : G.bombCharge * 8);
      ctx.strokeStyle = rgba(G.charge >= 0.92 || G.bombCharge >= 0.92 ? GOLD : HOT, 0.4 + (ch ? G.charge : 0.3) * 0.4);
      ctx.lineWidth = 1.6;
      ctx.beginPath();
  ...
  function drawEnt(en) {
    ...
    if (en.type === 'fighter' || en.type === 'dive' || en.type === 'kami') {
      ctx.beginPath();
      ctx.moveTo(0, 11);
      ctx.lineTo(10, 0);
      ctx.lineTo(3, 1);
      ...
      ctx.closePath();
      ctx.fill();
      ctx.fillStyle = en.type === 'kami' ? rgba(GOLD, 0.7) : rgba(WHT, 0.32);
    ...
    } else if (en.type === 'ship' || en.type === 'escort') {
      ctx.beginPath();
      ctx.ellipse(0, 4, en.type === 'escort' ? 22 : 28, 12, 0, 0, TAU);
      ctx.fill();
      ctx.fillRect(-18, -8, 9, 18);
      ctx.fillRect(-4, -14, 8, 26);
      ctx.fillRect(10, -6, 9, 14);
      ctx.fillStyle = rgba(WHT, 0.22);
      ctx.fillRect(-8, -2, 16, 6);
    } else if (en.type === 'turret') {
      const dx = G.player.x - en.x;
      const dy = G.player.y - en.y;
      ctx.fillStyle = 'rgba(42, 20, 18, 0.95)';
      ctx.beginPath();
  ...
  function drawPows() {
    for (let i = 0; i < G.pows.length; i++) {
      ...
      ctx.rotate(p.t * 2.2);
      ctx.scale(scale, scale);
      ctx.fillStyle = rgba(p.kind === 'loop' ? HOT : GOLD, 0.95);
      ctx.shadowColor = rgba(p.kind === 'loop' ? HOT : GOLD, 0.8);
      ctx.shadowBlur = 12;
      ctx.beginPath();
      ctx.moveTo(0, -11);
      ctx.lineTo(11, 0);
      ...
      ctx.rotate(-p.t * 2.2);
      ctx.shadowBlur = 0;
      ctx.fillText(p.kind === 'loop' ? '环' : '火', 0, 1);
      ctx.restore();
    }
  }
  ...
  function drawParticles() {
    ...
    for (let i = 0; i < orbs.length; i++) {
      ...
      ctx.fillStyle = rgba(o.rgb, a * 0.85);
      ctx.beginPath();
  ...
  function drawLoopRing() {
    ...
    if (G.charging) {
      const p = G.charge;
      ctx.save();
      ctx.strokeStyle = rgba(p >= 0.92 ? GOLD : HOT, 0.28 + p * 0.45);
      ctx.lineWidth = (2 + p * 2.4) * scale;
      ctx.shadowColor = rgba(p >= 0.92 ? GOLD : HOT, 0.55);
      ctx.shadowBlur = 12 * scale;
      ctx.beginPath();
      ctx.arc(sx(G.player.x), sy(G.player.y), (18 + p * 26) * scale, 0, TAU);
      ctx.stroke();
      ctx.restore();
    }
    if (!looping()) return;
    const p = 1 - G.loopT / G.loopDur;
    ctx.save();
    ctx.strokeStyle = rgba(G.bombCharge >= 0.92 ? GOLD : HOT, 0.35 + (1 - p) * 0.35);
    ctx.lineWidth = 2.4 * scale;
    ctx.shadowColor = rgba(HOT, 0.6);
    ctx.shadowBlur = 12 * scale;
    ctx.beginPath();
    ctx.arc(sx(G.player.x), sy(G.player.y), (22 + p * 32 + G.bombCharge * 18) * scale, 0, TAU);
    ctx.stroke();
    ctx.restore();
  }
  ...
  function startGame(kind) {
    ...
    syncHud();
    audio.start();
    toast(isCore() ? '海核 · 编队更密' : '四四 · 第 1 关', false, true);
  }
  ...
  function goTitle() {
    ...
    showOverlay(
      'title',
      '四四',
      '环圣。空格连射扫空打舰，按住 Shift 蓄环，松开环爆清弹炸舰队。莱特、帛琉、冲绳联合舰队。'
    );
    syncHud();
  }
  ...
  function onKey(e, down) {
    ...
    if (loopKey) {
      if (!G.loopHold) {
        G.loopHold = true;
        audio.ensure();
        if (!overlayOpen() && G.mode === 'play') beginCharge();
      }
      return;
    }
    if (space || k === 'Enter') {
